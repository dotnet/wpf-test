...
/***********************************************************
*
*   Program:   Tests for TextDecorationCollection (generated)
*
...
namespace                       Microsoft.Test.Graphics.Generated
{
    ...
    public partial class        TextDecorationCollectionTest : CoreGraphicsTest
    {
        // - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        ...
        public override void    Init( Variation v )
        {
            ...
            _collection      = NewTextDecorationCollection();
            _list            = NewTextDecorationCollection();
            _objects         = new List<TextDecoration>();
            _bogusCollection = new ArrayList();

            _objects.Add( new TextDecoration() );
            _objects.Add( new TextDecoration(TextDecorationLocation.Underline, null, 0 , TextDecorationUnit.Pixel, TextDecorationUnit.Pixel) );
            _objects.Add( new TextDecoration(TextDecorationLocation.OverLine, new Pen(Brushes.Red, 1), 1 , TextDecorationUnit.FontRecommended, TextDecorationUnit.FontRecommended) );
            _objects.Add( new TextDecoration(TextDecorationLocation.Baseline, new Pen(Brushes.Blue, 2), -1 , TextDecorationUnit.FontRecommended, TextDecorationUnit.Pixel) );
        ...
        private TextDecorationCollection NewTextDecorationCollection()
        {
        ...
        private TextDecorationCollection NewTextDecorationCollection( IEnumerable<TextDecoration> collection )
        {
            return new TextDecorationCollection( collection );
        }

        // - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

        private TextDecorationCollection NewTextDecorationCollection( int capacity )
        {
            return new TextDecorationCollection( capacity );
        }
        ...
        {
            _collection = NewTextDecorationCollection();
            foreach ( TextDecoration obj in _objects )
            {
                _collection.Add( obj );
            }
        }
        ...
        private TextDecoration[]    TextDecorationArray
        {
            get
            {
                TextDecoration[] array = new TextDecoration[ _objects.Count ];
        ...
        private void            TestConstructor()
        {
            ...
            TextDecorationCollection empty = NewTextDecorationCollection();
            if ( empty.Count != 0 || failOnPurpose )
            {
                AddFailure( "TextDecorationCollection.ctor() failed" );
                Log( "*** Expected: Default constructor creates empty collection" );
                Log( "*** Actual:   {0} objects in \"empty\" collection", empty.Count );
            }

            PopulateCollection();
            TextDecorationCollection theirAnswer = NewTextDecorationCollection( _collection );

            if ( theirAnswer.Count != _objects.Count || failOnPurpose )
            {
                AddFailure( "TextDecorationCollection.ctor( IEnumerable<TextDecoration> ) failed" );
            ...
            TestConstructorWith( new List<TextDecoration>() );
        ...
        private void            TestConstructorWith( IEnumerable<TextDecoration> e )
        {
            TextDecorationCollection theirAnswer = NewTextDecorationCollection( e );
        ...
        {
            TextDecorationCollection theirAnswer = NewTextDecorationCollection( capacity );

            if ( theirAnswer.Count != 0 || failOnPurpose )
            {
                AddFailure( "TextDecorationCollection.ctor( int ) failed" );
                Log( "*** Expected: Count == 0" );
                Log( "*** Actual:   Count == {0}", theirAnswer.Count );
            }
        }
        ...
        private class           FakeEnumerable : IEnumerable<TextDecoration>, ICollection<TextDecoration>
        {
            public void         Add( TextDecoration p )
            {
            }
            public void         Clear()
            {
            }
            public bool         Contains( TextDecoration p )
            {
                return false;
            }
            public void         CopyTo( TextDecoration[] points, int index )
            {
            }
            public bool         Remove( TextDecoration p )
            {
                return false;
            }
            public int          Count
            ...
            private class       Enumerator : IEnumerator<TextDecoration>
            {
        ...
        private class           NonCollectionEnumerable : IEnumerable<TextDecoration>
        {
            public IEnumerator<TextDecoration> GetEnumerator()
            {
                return new Enumerator();
        ...
        {
            TextDecorationCollection c = NewTextDecorationCollection( null );
        }

        private void            ConstructWithNullInArray()
        {
            TextDecoration[] array = new TextDecoration[ _objects.Count+1 ];
            _objects.CopyTo( array );
            array[ _objects.Count ] = null;
            TextDecorationCollection c = NewTextDecorationCollection( array );
        }
        ...
        private void            TestAddWith( object o )
        {
            ...
            _collection.Add( (TextDecoration)o );

            if ( oldCount+1 != _collection.Count || position != oldCount ||
                    !_collection[ position ].Equals( (TextDecoration)o ) || failOnPurpose )
            {
                AddFailure( "TextDecorationCollection.Add failed" );
                Log( "*** Expected: count = {0}, position = {1}, TextDecoration added = {2}", oldCount+1, oldCount, o );
                Log( "***   Actual: count = {0}, position = {1}, TextDecoration added = {2}", _collection.Count, position, _collection[ position ] );
            }
        }
        ...
        {
            Log( "Testing TextDecorationCollection.Changed..." );
        ...
        private void            TestChangedForConstructor()
        {
            ...
            TextDecorationCollection temp = _collection.Clone();
            temp = NewTextDecorationCollection( _collection );
            VerifyCallbacksProcessed();
        ...
        private void            TestContains()
        {
            ...
            Log( "Testing TextDecorationCollection.Contains..." );
            PopulateCollection();
            ...
            foreach ( TextDecoration obj in _objects )
            {
                ...
                {
                    AddFailure( "TextDecorationCollection.Contains failed" );
                    Log( "Could not locate {0} in my collection", obj );
                }
            }
        ...
        private void            TestContains2()
        {
            ...
            foreach ( object o in _bogusCollection )
            {
                ...
                Type listType = typeof( TextDecoration );
        ...
        private void            TestCopyTo()
        {
            ...
            Log( "Testing TextDecorationCollection.CopyTo..." );
            PopulateCollection();
            TextDecoration[] typedArray = new TextDecoration[ _objects.Count ];
            ...
            for ( int i = 0; i < _collection.Count; i++ )
            {
                ...
                {
                    AddFailure( "CopyTo( TextDecoration[],int ) failed" );
        ...
        private void            TestCopyTo2()
        {
            ...
            Log( "Testing TextDecorationCollection.CopyTo with bad parameters..." );
        ...
        {
            PopulateList();
            TextDecoration[,] array = new TextDecoration[ _objects.Count,_objects.Count ];
            _list.CopyTo( array, 0 );
        }
        ...
        {
            Log( "Testing TextDecorationEnumerator..." );
            PopulateCollection();

            TestEnumeratorWith( NewTextDecorationCollection() );
            TestEnumeratorWith( _collection );
        }

        // - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

        private void            TestEnumeratorWith( TextDecorationCollection collection )
        {
        ...
        private void            TestMoveNextWith( IEnumerator enumerator, TextDecorationCollection collection )
        {
            int count = 0;
            while ( enumerator.MoveNext() )
            {
                TextDecoration current = (TextDecoration)enumerator.Current;
        ...
        private void            TestEnumerator2()
        {
            ...
            try
            {
                foreach ( TextDecoration o in _collection )
                {
            ...
            {
                PopulateCollection();
                enumerator = ((IEnumerable)NewTextDecorationCollection()).GetEnumerator();
                object foo = enumerator.Current;
        ...
        private void            TestGetAsFrozen()
        {
            ...
            TextDecorationCollection copy = _collection.Clone();
            ...
            TextDecorationCollection frozen = (TextDecorationCollection)copy.GetAsFrozen();
            ...
            copy.Freeze();
            frozen = (TextDecorationCollection)copy.GetAsFrozen();
        ...
        private void            TestGetCurrentValueAsFrozen()
        {
            ...
            TextDecorationCollection frozen = (TextDecorationCollection)copy.GetCurrentValueAsFrozen();
            if ( object.ReferenceEquals( frozen, copy ) ||
                 !ObjectUtils.DeepEqualsToAnimatable( frozen, copy ) ||
                 failOnPurpose )
            {
            ...
            copy.Freeze();
            frozen = (TextDecorationCollection)copy.GetCurrentValueAsFrozen();
            if ( !object.ReferenceEquals( frozen, copy ) || failOnPurpose )
            {
        ...
        private void            TestInsert()
        {
            ...
            Log( "Testing TextDecorationCollection.Insert..." );
            PopulateCollection();
        ...
        private void            TestInsert2()
        {
            ...
            Log( "Testing TextDecorationCollection.Insert with bad parameters..." );
            PopulateCollection();
        ...
        private void            TestItem()
        {
            ...
            Log( "Testing TextDecorationCollection.get/set_Item..." );
            PopulateCollection();

            _collection[ 0 ] = _objects[ 3 ];
            if ( !_collection[ 0 ].Equals( _objects[ 3 ] ) || failOnPurpose )
            {
                AddFailure( "TextDecorationCollection.get/set_Item failed" );
                Log( "*** Expected: item[0] = {0}", _objects[ 3 ] );
        ...
        private void            TestItem2()
        {
            ...
            Log( "Testing TextDecorationCollection.get/set_Item with bad parameters..." );
            PopulateCollection();
        ...
        {
            TextDecoration item = _collection[ -1 ];
        }

        private void            CollectionGetItemIndexTooLarge()
        {
            TextDecoration item = _collection[ _objects.Count ];
        }

        private void            CollectionSetItemNegativeIndex()
        {
            _collection[ -1 ] = (TextDecoration)_objects[ 0 ];
        }

        private void            CollectionSetItemIndexTooLarge()
        {
            _collection[ _objects.Count ] = (TextDecoration)_objects[ 0 ];
        }


        private void            CollectionSetItemNull()
        {
            _collection[ 0 ] = (TextDecoration)null;
        }
        ...
        private void            TestRemove()
        {
            ...
            Log( "Testing TextDecorationCollection.Remove..." );
            PopulateCollection();
        ...
        private void            TestRemove2()
        {
            ...
            Log( "Testing TextDecorationCollection.Remove with bad parameters..." );
        ...
        {
            PopulateCollection();
            _collection.Remove( (TextDecoration)_objects[ 0 ] );
            _collection.Remove( (TextDecoration)_objects[ 0 ] );
        }
        ...
        private void            RemoveFromEmptyCollection()
        {
            ...
            {
                AddFailure( "TextDecorationCollection.Remove failed with empty collection" );
        ...
        {
            Log( "Testing TextDecorationCollection.RemoveAt with bad parameters..." );
        ...
        private TextDecorationCollection _collection;
        private IList _list;
        private List<TextDecoration> _objects;
