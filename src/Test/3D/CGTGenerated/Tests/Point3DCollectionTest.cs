...
/***********************************************************
*
*   Program:   Tests for Point3DCollection (generated)
*
...
namespace                       Microsoft.Test.Graphics.Generated
{
    ...
    public partial class        Point3DCollectionTest : CoreGraphicsTest
    {
        // - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        ...
        public override void    Init( Variation v )
        {
            ...
            _collection      = NewPoint3DCollection();
            _list            = NewPoint3DCollection();
            _objects         = new List<Point3D>();
            _bogusCollection = new ArrayList();

            _objects.Add( new Point3D( 0,0,0 ) );
            _objects.Add( new Point3D( 1,1,1 ) );
            _objects.Add( new Point3D( 2,2,2 ) );
            _objects.Add( new Point3D( 3,3,3 ) );
            _objects.Add( new Point3D( 4,4,4 ) );
            _objects.Add( new Point3D( 5,5,5 ) );
        ...
        private Point3DCollection NewPoint3DCollection()
        {
        ...
        private Point3DCollection NewPoint3DCollection( IEnumerable<Point3D> collection )
        {
            return new Point3DCollection( collection );
        }

        // - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

        private Point3DCollection NewPoint3DCollection( int capacity )
        {
            return new Point3DCollection( capacity );
        }
        ...
        {
            _collection = NewPoint3DCollection();
            foreach ( Point3D obj in _objects )
            {
                _collection.Add( obj );
            }
        }
        ...
        private Point3D[]       Point3DArray
        {
            get
            {
                Point3D[] array = new Point3D[ _objects.Count ];
        ...
        private void            TestConstructor()
        {
            ...
            Point3DCollection empty = NewPoint3DCollection();
            if ( empty.Count != 0 || failOnPurpose )
            {
                AddFailure( "Point3DCollection.ctor() failed" );
                Log( "*** Expected: Default constructor creates empty collection" );
                Log( "*** Actual:   {0} objects in \"empty\" collection", empty.Count );
            }

            PopulateCollection();
            Point3DCollection theirAnswer = NewPoint3DCollection( _collection );

            if ( theirAnswer.Count != _objects.Count || failOnPurpose )
            {
                AddFailure( "Point3DCollection.ctor( IEnumerable<Point3D> ) failed" );
            ...
            TestConstructorWith( new List<Point3D>() );
        ...
        private void            TestConstructorWith( IEnumerable<Point3D> e )
        {
            Point3DCollection theirAnswer = NewPoint3DCollection( e );
        ...
        {
            Point3DCollection theirAnswer = NewPoint3DCollection( capacity );

            if ( theirAnswer.Count != 0 || failOnPurpose )
            {
                AddFailure( "Point3DCollection.ctor( int ) failed" );
                Log( "*** Expected: Count == 0" );
                Log( "*** Actual:   Count == {0}", theirAnswer.Count );
            }
        }
        ...
        private class           FakeEnumerable : IEnumerable<Point3D>, ICollection<Point3D>
        {
            ...
            public void         CopyTo( Point3D[] points, int index )
            {
            }
            ...
            private class       Enumerator : IEnumerator<Point3D>
            {
        ...
        private class           NonCollectionEnumerable : IEnumerable<Point3D>
        {
            public IEnumerator<Point3D> GetEnumerator()
            {
                return new Enumerator();
        ...
        {
            Point3DCollection c = NewPoint3DCollection( null );
        }
        ...
        private void            TestAddWith( object o )
        {
            ...
            _collection.Add( (Point3D)o );

            if ( oldCount+1 != _collection.Count || position != oldCount ||
                    !_collection[ position ].Equals( (Point3D)o ) || failOnPurpose )
            {
                AddFailure( "Point3DCollection.Add failed" );
                Log( "*** Expected: count = {0}, position = {1}, Point3D added = {2}", oldCount+1, oldCount, o );
                Log( "***   Actual: count = {0}, position = {1}, Point3D added = {2}", _collection.Count, position, _collection[ position ] );
            }
        }
        ...
        private void            TestAdd2()
        {
            ...
            // Skipping null tests for Point3DCollection.Add( Point3D ) because it's a value-type
        }
        ...
        {
            Log( "Testing Point3DCollection.Changed..." );
        ...
        private void            TestChangedForConstructor()
        {
            ...
            Point3DCollection temp = _collection.Clone();
            temp = NewPoint3DCollection( _collection );
            VerifyCallbacksProcessed();
        ...
        private void            TestContains()
        {
            ...
            Log( "Testing Point3DCollection.Contains..." );
            PopulateCollection();
            ...
            foreach ( Point3D obj in _objects )
            {
                ...
                {
                    AddFailure( "Point3DCollection.Contains failed" );
                    Log( "Could not locate {0} in my collection", obj );
                }
            }
        ...
        private void            TestCopyTo()
        {
            ...
            Log( "Testing Point3DCollection.CopyTo..." );
            PopulateCollection();
            Point3D[] typedArray = new Point3D[ _objects.Count ];
            ...
            for ( int i = 0; i < _collection.Count; i++ )
            {
                ...
                {
                    AddFailure( "CopyTo( Point3D[],int ) failed" );
        ...
        private void            TestCopyTo2()
        {
            ...
            Log( "Testing Point3DCollection.CopyTo with bad parameters..." );
        ...
        {
            PopulateList();
            Point3D[,] array = new Point3D[ _objects.Count,_objects.Count ];
            _list.CopyTo( array, 0 );
        }
        ...
        {
            PopulateCollection();
            Point3D[] array = new Point3D[ _objects.Count-1 ];
            _collection.CopyTo( array, 0 );
        }
        ...
        {
            Log( "Testing Point3DEnumerator..." );
            PopulateCollection();

            TestEnumeratorWith( NewPoint3DCollection() );
            TestEnumeratorWith( _collection );
        }

        // - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

        private void            TestEnumeratorWith( Point3DCollection collection )
        {
        ...
        private void            TestMoveNextWith( IEnumerator enumerator, Point3DCollection collection )
        {
            int count = 0;
            while ( enumerator.MoveNext() )
            {
                Point3D current = (Point3D)enumerator.Current;
        ...
        private void            TestEnumerator2()
        {
            ...
            try
            {
                foreach ( Point3D o in _collection )
                {
            ...
            {
                PopulateCollection();
                enumerator = ((IEnumerable)NewPoint3DCollection()).GetEnumerator();
                object foo = enumerator.Current;
        ...
        private void            TestGetAsFrozen()
        {
            ...
            Point3DCollection copy = _collection.Clone();
            ...
            Point3DCollection frozen = (Point3DCollection)copy.GetAsFrozen();
            ...
            copy.Freeze();
            frozen = (Point3DCollection)copy.GetAsFrozen();
        ...
        private void            TestGetCurrentValueAsFrozen()
        {
            ...
            Point3DCollection frozen = (Point3DCollection)copy.GetCurrentValueAsFrozen();
            if ( object.ReferenceEquals( frozen, copy ) ||
                 !ObjectUtils.DeepEqualsToAnimatable( frozen, copy ) ||
                 failOnPurpose )
            {
            ...
            copy.Freeze();
            frozen = (Point3DCollection)copy.GetCurrentValueAsFrozen();
            if ( !object.ReferenceEquals( frozen, copy ) || failOnPurpose )
            {
        ...
        private void            TestInsert()
        {
            ...
            Log( "Testing Point3DCollection.Insert..." );
            PopulateCollection();
        ...
        private void            TestInsert2()
        {
            ...
            Log( "Testing Point3DCollection.Insert with bad parameters..." );
            PopulateCollection();

            // Skipping null tests for Point3DCollection.Insert because it's a value-type
        ...
        private void            TestItem()
        {
            ...
            Log( "Testing Point3DCollection.get/set_Item..." );
            PopulateCollection();

            _collection[ 0 ] = _objects[ 3 ];
            if ( !_collection[ 0 ].Equals( _objects[ 3 ] ) || failOnPurpose )
            {
                AddFailure( "Point3DCollection.get/set_Item failed" );
                Log( "*** Expected: item[0] = {0}", _objects[ 3 ] );
        ...
        private void            TestItem2()
        {
            ...
            Log( "Testing Point3DCollection.get/set_Item with bad parameters..." );
            PopulateCollection();
            ...
            // Skipping null tests for Point3DCollection.set_Item( int, Point3D ) because it's a value-type
        }
        ...
        {
            Point3D item = _collection[ -1 ];
        }

        private void            CollectionGetItemIndexTooLarge()
        {
            Point3D item = _collection[ _objects.Count ];
        }

        private void            CollectionSetItemNegativeIndex()
        {
            _collection[ -1 ] = (Point3D)_objects[ 0 ];
        }

        private void            CollectionSetItemIndexTooLarge()
        {
            _collection[ _objects.Count ] = (Point3D)_objects[ 0 ];
        }
        ...
        private void            TestRemove()
        {
            ...
            Log( "Testing Point3DCollection.Remove..." );
            PopulateCollection();
        ...
        private void            TestRemove2()
        {
            ...
            Log( "Testing Point3DCollection.Remove with bad parameters..." );

            // Skipping null tests for Point3DCollection.Remove because it's a value-type
        ...
        {
            PopulateCollection();
            _collection.Remove( (Point3D)_objects[ 0 ] );
            _collection.Remove( (Point3D)_objects[ 0 ] );
        }
        ...
        private void            RemoveFromEmptyCollection()
        {
            ...
            {
                AddFailure( "Point3DCollection.Remove failed with empty collection" );
        ...
        {
            Log( "Testing Point3DCollection.RemoveAt with bad parameters..." );
        ...
        private void            TestToStringWith( Point3DCollection collection )
        {
            ...
            foreach ( Point3D obj in collection )
            {
                // I don't like calling into another Avalon method, but since test coverage
                //  for ToString should already exist for Point3D, this is okay.
        ...
        private void            TestToString2()
        {
            ...
            TestToStringWith( NewPoint3DCollection() );
        }
