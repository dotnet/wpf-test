...
/***********************************************************
*
*   Program:   Tests for Transform3DCollection (generated)
*
...
namespace                       Microsoft.Test.Graphics.Generated
{
    ...
    public partial class        Transform3DCollectionTest : CoreGraphicsTest
    {
        // - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        ...
        public override void    Init( Variation v )
        {
            ...
            _collection      = NewTransform3DCollection();
            _list            = NewTransform3DCollection();
            _objects         = new List<Transform3D>();
            _bogusCollection = new ArrayList();

            _objects.Add( Const.tt10 );
            _objects.Add( Const.ttNeg1 );
            _objects.Add( Const.rtq );
            _objects.Add( Const.rtX45 );
            _objects.Add( Const.rtZ135 );
            _objects.Add( Const.st1 );
            _objects.Add( Const.st10 );
            _objects.Add( Const.mtNAffine );
            _objects.Add( Const.mtIdent );
            _objects.Add( Transform3D.Identity );
        ...
        private Transform3DCollection NewTransform3DCollection()
        {
        ...
        private Transform3DCollection NewTransform3DCollection( IEnumerable<Transform3D> collection )
        {
            return new Transform3DCollection( collection );
        }

        // - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

        private Transform3DCollection NewTransform3DCollection( int capacity )
        {
            return new Transform3DCollection( capacity );
        }
        ...
        {
            _collection = NewTransform3DCollection();
            foreach ( Transform3D obj in _objects )
            {
                _collection.Add( obj );
            }
        }
        ...
        private Transform3D[]   Transform3DArray
        {
            get
            {
                Transform3D[] array = new Transform3D[ _objects.Count ];
        ...
        private void            TestConstructor()
        {
            ...
            Transform3DCollection empty = NewTransform3DCollection();
            if ( empty.Count != 0 || failOnPurpose )
            {
                AddFailure( "Transform3DCollection.ctor() failed" );
                Log( "*** Expected: Default constructor creates empty collection" );
                Log( "*** Actual:   {0} objects in \"empty\" collection", empty.Count );
            }

            PopulateCollection();
            Transform3DCollection theirAnswer = NewTransform3DCollection( _collection );

            if ( theirAnswer.Count != _objects.Count || failOnPurpose )
            {
                AddFailure( "Transform3DCollection.ctor( IEnumerable<Transform3D> ) failed" );
            ...
            TestConstructorWith( new List<Transform3D>() );
        ...
        private void            TestConstructorWith( IEnumerable<Transform3D> e )
        {
            Transform3DCollection theirAnswer = NewTransform3DCollection( e );
        ...
        {
            Transform3DCollection theirAnswer = NewTransform3DCollection( capacity );

            if ( theirAnswer.Count != 0 || failOnPurpose )
            {
                AddFailure( "Transform3DCollection.ctor( int ) failed" );
                Log( "*** Expected: Count == 0" );
                Log( "*** Actual:   Count == {0}", theirAnswer.Count );
            }
        }
        ...
        private class           FakeEnumerable : IEnumerable<Transform3D>, ICollection<Transform3D>
        {
            public void         Add( Transform3D p )
            {
            }
            public void         Clear()
            {
            }
            public bool         Contains( Transform3D p )
            {
                return false;
            }
            public void         CopyTo( Transform3D[] points, int index )
            {
            }
            public bool         Remove( Transform3D p )
            {
                return false;
            }
            public int          Count
            ...
            private class       Enumerator : IEnumerator<Transform3D>
            {
        ...
        private class           NonCollectionEnumerable : IEnumerable<Transform3D>
        {
            public IEnumerator<Transform3D> GetEnumerator()
            {
                return new Enumerator();
        ...
        {
            Transform3DCollection c = NewTransform3DCollection( null );
        }

        private void            ConstructWithNullInArray()
        {
            Transform3D[] array = new Transform3D[ _objects.Count+1 ];
            _objects.CopyTo( array );
            array[ _objects.Count ] = null;
            Transform3DCollection c = NewTransform3DCollection( array );
        }
        ...
        private void            TestAddWith( object o )
        {
            ...
            _collection.Add( (Transform3D)o );

            if ( oldCount+1 != _collection.Count || position != oldCount ||
                    !_collection[ position ].Equals( (Transform3D)o ) || failOnPurpose )
            {
                AddFailure( "Transform3DCollection.Add failed" );
                Log( "*** Expected: count = {0}, position = {1}, Transform3D added = {2}", oldCount+1, oldCount, o );
                Log( "***   Actual: count = {0}, position = {1}, Transform3D added = {2}", _collection.Count, position, _collection[ position ] );
            }
        }
        ...
        private void            TestAdd2()
        {
            ...
            foreach ( object o in _bogusCollection )
            {
                ...
                Type addType = typeof( Transform3D );
                try
                {
                    _list.Add( o );
            ...
            Log( "Testing Transform3DCollection.Add with bad parameters..." );
        ...
        {
            Log( "Testing Transform3DCollection.Changed..." );
        ...
        private void            TestChangedForConstructor()
        {
            ...
            Transform3DCollection temp = _collection.Clone();
            temp = NewTransform3DCollection( _collection );
            VerifyCallbacksProcessed();
        ...
        private void            TestContains()
        {
            ...
            Log( "Testing Transform3DCollection.Contains..." );
            PopulateCollection();
            ...
            foreach ( Transform3D obj in _objects )
            {
                ...
                {
                    AddFailure( "Transform3DCollection.Contains failed" );
                    Log( "Could not locate {0} in my collection", obj );
                }
            }
        ...
        private void            TestContains2()
        {
            ...
            {
                Type itemType = ( o == null ) ? null : o.GetType();
                Type listType = typeof( Transform3D );
        ...
        private void            TestCopyTo()
        {
            ...
            Log( "Testing Transform3DCollection.CopyTo..." );
            PopulateCollection();
            Transform3D[] typedArray = new Transform3D[ _objects.Count ];
            ...
            for ( int i = 0; i < _collection.Count; i++ )
            {
                ...
                {
                    AddFailure( "CopyTo( Transform3D[],int ) failed" );
        ...
        private void            TestCopyTo2()
        {
            ...
            Log( "Testing Transform3DCollection.CopyTo with bad parameters..." );
        ...
        {
            PopulateList();
            Transform3D[,] array = new Transform3D[ _objects.Count,_objects.Count ];
            _list.CopyTo( array, 0 );
        }
        ...
        {
            Log( "Testing Transform3DEnumerator..." );
            PopulateCollection();

            TestEnumeratorWith( NewTransform3DCollection() );
            TestEnumeratorWith( _collection );
        }

        // - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

        private void            TestEnumeratorWith( Transform3DCollection collection )
        {
        ...
        private void            TestMoveNextWith( IEnumerator enumerator, Transform3DCollection collection )
        {
            int count = 0;
            while ( enumerator.MoveNext() )
            {
                Transform3D current = (Transform3D)enumerator.Current;
        ...
        private void            TestEnumerator2()
        {
            ...
            try
            {
                foreach ( Transform3D o in _collection )
                {
            ...
            {
                PopulateCollection();
                enumerator = ((IEnumerable)NewTransform3DCollection()).GetEnumerator();
                object foo = enumerator.Current;
        ...
        private void            TestGetAsFrozen()
        {
            ...
            Transform3DCollection copy = _collection.Clone();
            ...
            Transform3DCollection frozen = (Transform3DCollection)copy.GetAsFrozen();
            ...
            copy.Freeze();
            frozen = (Transform3DCollection)copy.GetAsFrozen();
        ...
        private void            TestGetCurrentValueAsFrozen()
        {
            ...
            Transform3DCollection frozen = (Transform3DCollection)copy.GetCurrentValueAsFrozen();
            if ( object.ReferenceEquals( frozen, copy ) ||
                 !ObjectUtils.DeepEqualsToAnimatable( frozen, copy ) ||
                 failOnPurpose )
            {
            ...
            copy.Freeze();
            frozen = (Transform3DCollection)copy.GetCurrentValueAsFrozen();
            if ( !object.ReferenceEquals( frozen, copy ) || failOnPurpose )
            {
        ...
        private void            TestInsert()
        {
            ...
            Log( "Testing Transform3DCollection.Insert..." );
            PopulateCollection();
        ...
        private void            TestInsert2()
        {
            ...
            Log( "Testing Transform3DCollection.Insert with bad parameters..." );
            PopulateCollection();
        ...
        private void            TestItem()
        {
            ...
            Log( "Testing Transform3DCollection.get/set_Item..." );
            PopulateCollection();

            _collection[ 0 ] = _objects[ 3 ];
            if ( !_collection[ 0 ].Equals( _objects[ 3 ] ) || failOnPurpose )
            {
                AddFailure( "Transform3DCollection.get/set_Item failed" );
                Log( "*** Expected: item[0] = {0}", _objects[ 3 ] );
        ...
        private void            TestItem2()
        {
            ...
            Log( "Testing Transform3DCollection.get/set_Item with bad parameters..." );
            PopulateCollection();
        ...
        {
            Transform3D item = _collection[ -1 ];
        }

        private void            CollectionGetItemIndexTooLarge()
        {
            Transform3D item = _collection[ _objects.Count ];
        }

        private void            CollectionSetItemNegativeIndex()
        {
            _collection[ -1 ] = (Transform3D)_objects[ 0 ];
        }

        private void            CollectionSetItemIndexTooLarge()
        {
            _collection[ _objects.Count ] = (Transform3D)_objects[ 0 ];
        }


        private void            CollectionSetItemNull()
        {
            _collection[ 0 ] = (Transform3D)null;
        }
        ...
        private void            TestRemove()
        {
            ...
            Log( "Testing Transform3DCollection.Remove..." );
            PopulateCollection();
        ...
        private void            TestRemove2()
        {
            ...
            Log( "Testing Transform3DCollection.Remove with bad parameters..." );
        ...
        {
            PopulateCollection();
            _collection.Remove( (Transform3D)_objects[ 0 ] );
            _collection.Remove( (Transform3D)_objects[ 0 ] );
        }
        ...
        private void            RemoveFromEmptyCollection()
        {
            ...
            {
                AddFailure( "Transform3DCollection.Remove failed with empty collection" );
        ...
        {
            Log( "Testing Transform3DCollection.RemoveAt with bad parameters..." );
        ...
        private Transform3DCollection _collection;
        private IList _list;
        private List<Transform3D> _objects;
