...
/***********************************************************
*
*   Program:   Tests for MaterialCollection (generated)
*
...
namespace                       Microsoft.Test.Graphics.Generated
{
    ...
    public partial class        MaterialCollectionTest : CoreGraphicsTest
    {
        // - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        ...
        public override void    Init( Variation v )
        {
            ...
            _collection      = NewMaterialCollection();
            _list            = NewMaterialCollection();
            _objects         = new List<Material>();
            _bogusCollection = new ArrayList();

            _objects.Add( MaterialFactory.Blue );
            _objects.Add( MaterialFactory.Gray );
            _objects.Add( MaterialFactory.Yellow );
            _objects.Add( MaterialFactory.Red );
            _objects.Add( MaterialFactory.Black );
            _objects.Add( MaterialFactory.White );
            _objects.Add( MaterialFactory.Green );
            _objects.Add( MaterialFactory.Default );
            _objects.Add( MaterialFactory.DefaultEmissive );
            _objects.Add( MaterialFactory.DefaultSpecular );
            _objects.Add( MaterialFactory.CompoundMaterial );
        ...
        private MaterialCollection NewMaterialCollection()
        {
        ...
        private MaterialCollection NewMaterialCollection( IEnumerable<Material> collection )
        {
        ...
        private MaterialCollection NewMaterialCollection( int capacity )
        {
        ...
        {
            _collection = NewMaterialCollection();
            foreach ( Material obj in _objects )
            {
                _collection.Add( obj );
            }
        }
        ...
        private Material[]      MaterialArray
        {
            get
            {
                Material[] array = new Material[ _objects.Count ];
        ...
        private void            TestConstructor()
        {
            ...
            MaterialCollection empty = NewMaterialCollection();
            if ( empty.Count != 0 || failOnPurpose )
            {
                AddFailure( "MaterialCollection.ctor() failed" );
                Log( "*** Expected: Default constructor creates empty collection" );
                Log( "*** Actual:   {0} objects in \"empty\" collection", empty.Count );
            }

            PopulateCollection();
            MaterialCollection theirAnswer = NewMaterialCollection( _collection );

            if ( theirAnswer.Count != _objects.Count || failOnPurpose )
            {
                AddFailure( "MaterialCollection.ctor( IEnumerable<Material> ) failed" );
            ...
            TestConstructorWith( new List<Material>() );
        ...
        private void            TestConstructorWith( IEnumerable<Material> e )
        {
            MaterialCollection theirAnswer = NewMaterialCollection( e );
        ...
        {
            MaterialCollection theirAnswer = NewMaterialCollection( capacity );

            if ( theirAnswer.Count != 0 || failOnPurpose )
            {
                AddFailure( "MaterialCollection.ctor( int ) failed" );
                Log( "*** Expected: Count == 0" );
                Log( "*** Actual:   Count == {0}", theirAnswer.Count );
            }
        }
        ...
        private class           FakeEnumerable : IEnumerable<Material>, ICollection<Material>
        {
            ...
            public bool         Contains( Material p )
            {
                return false;
            }
            public void         CopyTo( Material[] points, int index )
            {
            }
            public bool         Remove( Material p )
            {
                return false;
            }
            public int          Count
            ...
            private class       Enumerator : IEnumerator<Material>
            {
        ...
        private class           NonCollectionEnumerable : IEnumerable<Material>
        {
            public IEnumerator<Material> GetEnumerator()
            {
                return new Enumerator();
        ...
        {
            MaterialCollection c = NewMaterialCollection( null );
        }

        private void            ConstructWithNullInArray()
        {
            Material[] array = new Material[ _objects.Count+1 ];
            _objects.CopyTo( array );
            array[ _objects.Count ] = null;
            MaterialCollection c = NewMaterialCollection( array );
        }
        ...
        private void            TestAddWith( object o )
        {
            ...
            _collection.Add( (Material)o );

            if ( oldCount+1 != _collection.Count || position != oldCount ||
                    !_collection[ position ].Equals( (Material)o ) || failOnPurpose )
            {
                AddFailure( "MaterialCollection.Add failed" );
                Log( "*** Expected: count = {0}, position = {1}, Material added = {2}", oldCount+1, oldCount, o );
                Log( "***   Actual: count = {0}, position = {1}, Material added = {2}", _collection.Count, position, _collection[ position ] );
            }
        }
        ...
        private void            TestAdd2()
        {
            ...
            Log( "Testing MaterialCollection.Add with bad parameters..." );
        ...
        {
            Log( "Testing MaterialCollection.Changed..." );
        ...
        private void            TestChangedForConstructor()
        {
            ...
            MaterialCollection temp = _collection.Clone();
            temp = NewMaterialCollection( _collection );
            VerifyCallbacksProcessed();
        ...
        private void            TestContains()
        {
            ...
            Log( "Testing MaterialCollection.Contains..." );
            PopulateCollection();
            ...
            foreach ( Material obj in _objects )
            {
                ...
                {
                    AddFailure( "MaterialCollection.Contains failed" );
                    Log( "Could not locate {0} in my collection", obj );
                }
            }
        ...
        private void            TestCopyTo()
        {
            ...
            Log( "Testing MaterialCollection.CopyTo..." );
            PopulateCollection();
            Material[] typedArray = new Material[ _objects.Count ];
            ...
            for ( int i = 0; i < _collection.Count; i++ )
            {
                ...
                {
                    AddFailure( "CopyTo( Material[],int ) failed" );
        ...
        private void            TestCopyTo2()
        {
            ...
            Log( "Testing MaterialCollection.CopyTo with bad parameters..." );
        ...
        {
            PopulateList();
            Material[,] array = new Material[ _objects.Count,_objects.Count ];
            _list.CopyTo( array, 0 );
        }
        ...
        {
            Log( "Testing MaterialEnumerator..." );
            PopulateCollection();

            TestEnumeratorWith( NewMaterialCollection() );
            TestEnumeratorWith( _collection );
        }

        // - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

        private void            TestEnumeratorWith( MaterialCollection collection )
        {
        ...
        private void            TestMoveNextWith( IEnumerator enumerator, MaterialCollection collection )
        {
            int count = 0;
        ...
        private void            TestEnumerator2()
        {
            ...
            try
            {
                foreach ( Material o in _collection )
                {
            ...
            {
                PopulateCollection();
                enumerator = ((IEnumerable)NewMaterialCollection()).GetEnumerator();
                object foo = enumerator.Current;
        ...
        private void            TestGetAsFrozen()
        {
            ...
            MaterialCollection copy = _collection.Clone();
            ...
            MaterialCollection frozen = (MaterialCollection)copy.GetAsFrozen();
            ...
            copy.Freeze();
            frozen = (MaterialCollection)copy.GetAsFrozen();
        ...
        private void            TestGetCurrentValueAsFrozen()
        {
            ...
            MaterialCollection frozen = (MaterialCollection)copy.GetCurrentValueAsFrozen();
            if ( object.ReferenceEquals( frozen, copy ) ||
                 !ObjectUtils.DeepEqualsToAnimatable( frozen, copy ) ||
                 failOnPurpose )
            {
            ...
            copy.Freeze();
            frozen = (MaterialCollection)copy.GetCurrentValueAsFrozen();
            if ( !object.ReferenceEquals( frozen, copy ) || failOnPurpose )
            {
        ...
        private void            TestInsert()
        {
            ...
            Log( "Testing MaterialCollection.Insert..." );
            PopulateCollection();
        ...
        private void            TestInsert2()
        {
            ...
            Log( "Testing MaterialCollection.Insert with bad parameters..." );
            PopulateCollection();
        ...
        private void            TestItem()
        {
            ...
            Log( "Testing MaterialCollection.get/set_Item..." );
            PopulateCollection();

            _collection[ 0 ] = _objects[ 3 ];
            if ( !_collection[ 0 ].Equals( _objects[ 3 ] ) || failOnPurpose )
            {
                AddFailure( "MaterialCollection.get/set_Item failed" );
                Log( "*** Expected: item[0] = {0}", _objects[ 3 ] );
        ...
        private void            TestItem2()
        {
            ...
            Log( "Testing MaterialCollection.get/set_Item with bad parameters..." );
            PopulateCollection();
        ...
        {
            Material item = _collection[ -1 ];
        }

        private void            CollectionGetItemIndexTooLarge()
        {
            Material item = _collection[ _objects.Count ];
        }

        private void            CollectionSetItemNegativeIndex()
        {
            _collection[ -1 ] = (Material)_objects[ 0 ];
        }

        private void            CollectionSetItemIndexTooLarge()
        {
            _collection[ _objects.Count ] = (Material)_objects[ 0 ];
        }


        private void            CollectionSetItemNull()
        {
            _collection[ 0 ] = (Material)null;
        }
        ...
        private void            TestRemove()
        {
            ...
            Log( "Testing MaterialCollection.Remove..." );
            PopulateCollection();
        ...
        private void            TestRemove2()
        {
            ...
            Log( "Testing MaterialCollection.Remove with bad parameters..." );
        ...
        {
            PopulateCollection();
            _collection.Remove( (Material)_objects[ 0 ] );
            _collection.Remove( (Material)_objects[ 0 ] );
        }
        ...
        private void            RemoveFromEmptyCollection()
        {
            ...
            {
                AddFailure( "MaterialCollection.Remove failed with empty collection" );
        ...
        {
            Log( "Testing MaterialCollection.RemoveAt with bad parameters..." );
        ...
        private IList _list;
        private List<Material> _objects;
