...
/***********************************************************
*
*   Program:   Tests for PointCollection (generated)
*
...
namespace                       Microsoft.Test.Graphics.Generated
{
    ...
    public partial class        PointCollectionTest : CoreGraphicsTest
    {
        // - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        ...
        public override void    Init( Variation v )
        {
            ...
            _collection      = NewPointCollection();
            _list            = NewPointCollection();
            _objects         = new List<Point>();
            _bogusCollection = new ArrayList();

            _objects.Add( new Point( 0,0 ) );
            _objects.Add( new Point( 1,1 ) );
            _objects.Add( new Point( 2,2 ) );
            _objects.Add( new Point( 3,3 ) );
            _objects.Add( new Point( 4,4 ) );
            _objects.Add( new Point( 5,5 ) );
        ...
        private PointCollection NewPointCollection()
        {
        ...
        private PointCollection NewPointCollection( IEnumerable<Point> collection )
        {
        ...
        private PointCollection NewPointCollection( int capacity )
        {
            return new PointCollection( capacity );
        }
        ...
        private Point[]         PointArray
        {
            get
            {
                Point[] array = new Point[ _objects.Count ];
        ...
        private void            TestConstructor()
        {
            ...
            PointCollection empty = NewPointCollection();
            if ( empty.Count != 0 || failOnPurpose )
            {
                AddFailure( "PointCollection.ctor() failed" );
                Log( "*** Expected: Default constructor creates empty collection" );
                Log( "*** Actual:   {0} objects in \"empty\" collection", empty.Count );
            }

            PopulateCollection();
            PointCollection theirAnswer = NewPointCollection( _collection );

            if ( theirAnswer.Count != _objects.Count || failOnPurpose )
            {
                AddFailure( "PointCollection.ctor( IEnumerable<Point> ) failed" );
            ...
            TestConstructorWith( new List<Point>() );
        ...
        private void            TestConstructorWith( IEnumerable<Point> e )
        {
            PointCollection theirAnswer = NewPointCollection( e );
        ...
        {
            PointCollection theirAnswer = NewPointCollection( capacity );

            if ( theirAnswer.Count != 0 || failOnPurpose )
            {
                AddFailure( "PointCollection.ctor( int ) failed" );
                Log( "*** Expected: Count == 0" );
                Log( "*** Actual:   Count == {0}", theirAnswer.Count );
            }
        }
        ...
        private class           FakeEnumerable : IEnumerable<Point>, ICollection<Point>
        {
        ...
        private class           NonCollectionEnumerable : IEnumerable<Point>
        {
            public IEnumerator<Point> GetEnumerator()
            {
                return new Enumerator();
        ...
        {
            PointCollection c = NewPointCollection( null );
        }
        ...
        private void            TestAddWith( object o )
        {
            ...
            _collection.Add( (Point)o );

            if ( oldCount+1 != _collection.Count || position != oldCount ||
                    !_collection[ position ].Equals( (Point)o ) || failOnPurpose )
            {
                AddFailure( "PointCollection.Add failed" );
                Log( "*** Expected: count = {0}, position = {1}, Point added = {2}", oldCount+1, oldCount, o );
                Log( "***   Actual: count = {0}, position = {1}, Point added = {2}", _collection.Count, position, _collection[ position ] );
            }
        }
        ...
        private void            TestAdd2()
        {
            ...
            // Skipping null tests for PointCollection.Add( Point ) because it's a value-type
        }
        ...
        {
            Log( "Testing PointCollection.Changed..." );
        ...
        private void            TestChangedForConstructor()
        {
            ...
            PointCollection temp = _collection.Clone();
            temp = NewPointCollection( _collection );
            VerifyCallbacksProcessed();
        ...
        private void            TestContains()
        {
            ...
            Log( "Testing PointCollection.Contains..." );
            PopulateCollection();
            ...
            foreach ( Point obj in _objects )
            {
                ...
                {
                    AddFailure( "PointCollection.Contains failed" );
                    Log( "Could not locate {0} in my collection", obj );
                }
            }
        ...
        private void            TestCopyTo()
        {
            ...
            Log( "Testing PointCollection.CopyTo..." );
            PopulateCollection();
            Point[] typedArray = new Point[ _objects.Count ];
            ...
            for ( int i = 0; i < _collection.Count; i++ )
            {
                ...
                {
                    AddFailure( "CopyTo( Point[],int ) failed" );
        ...
        private void            TestCopyTo2()
        {
            ...
            Log( "Testing PointCollection.CopyTo with bad parameters..." );
        ...
        {
            PopulateList();
            Point[,] array = new Point[ _objects.Count,_objects.Count ];
            _list.CopyTo( array, 0 );
        }
        ...
        {
            PopulateCollection();
            Point[] array = new Point[ _objects.Count-1 ];
            _collection.CopyTo( array, 0 );
        }
        ...
        {
            Log( "Testing PointEnumerator..." );
            PopulateCollection();

            TestEnumeratorWith( NewPointCollection() );
            TestEnumeratorWith( _collection );
        }

        // - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

        private void            TestEnumeratorWith( PointCollection collection )
        {
        ...
        private void            TestMoveNextWith( IEnumerator enumerator, PointCollection collection )
        {
            int count = 0;
            while ( enumerator.MoveNext() )
            {
                Point current = (Point)enumerator.Current;
        ...
        private void            TestEnumerator2()
        {
            ...
            try
            {
                foreach ( Point o in _collection )
                {
            ...
            {
                PopulateCollection();
                enumerator = ((IEnumerable)NewPointCollection()).GetEnumerator();
                object foo = enumerator.Current;
        ...
        private void            TestGetAsFrozen()
        {
            ...
            PointCollection copy = _collection.Clone();
            ...
            PointCollection frozen = (PointCollection)copy.GetAsFrozen();
            ...
            copy.Freeze();
            frozen = (PointCollection)copy.GetAsFrozen();
        ...
        private void            TestGetCurrentValueAsFrozen()
        {
            ...
            PointCollection frozen = (PointCollection)copy.GetCurrentValueAsFrozen();
            if ( object.ReferenceEquals( frozen, copy ) ||
                 !ObjectUtils.DeepEqualsToAnimatable( frozen, copy ) ||
                 failOnPurpose )
            {
            ...
            copy.Freeze();
            frozen = (PointCollection)copy.GetCurrentValueAsFrozen();
            if ( !object.ReferenceEquals( frozen, copy ) || failOnPurpose )
            {
        ...
        private void            TestInsert()
        {
            ...
            Log( "Testing PointCollection.Insert..." );
            PopulateCollection();
        ...
        private void            TestInsert2()
        {
            ...
            Log( "Testing PointCollection.Insert with bad parameters..." );
            PopulateCollection();

            // Skipping null tests for PointCollection.Insert because it's a value-type
        ...
        private void            TestItem()
        {
            ...
            Log( "Testing PointCollection.get/set_Item..." );
            PopulateCollection();

            _collection[ 0 ] = _objects[ 3 ];
            if ( !_collection[ 0 ].Equals( _objects[ 3 ] ) || failOnPurpose )
            {
                AddFailure( "PointCollection.get/set_Item failed" );
                Log( "*** Expected: item[0] = {0}", _objects[ 3 ] );
        ...
        private void            TestItem2()
        {
            ...
            Log( "Testing PointCollection.get/set_Item with bad parameters..." );
            PopulateCollection();
            ...
            // Skipping null tests for PointCollection.set_Item( int, Point ) because it's a value-type
        }
        ...
        {
            Point item = _collection[ -1 ];
        }

        private void            CollectionGetItemIndexTooLarge()
        {
            Point item = _collection[ _objects.Count ];
        }

        private void            CollectionSetItemNegativeIndex()
        {
            _collection[ -1 ] = (Point)_objects[ 0 ];
        }

        private void            CollectionSetItemIndexTooLarge()
        {
            _collection[ _objects.Count ] = (Point)_objects[ 0 ];
        }
        ...
        private void            TestRemove()
        {
            ...
            Log( "Testing PointCollection.Remove..." );
            PopulateCollection();
        ...
        private void            TestRemove2()
        {
            ...
            Log( "Testing PointCollection.Remove with bad parameters..." );

            // Skipping null tests for PointCollection.Remove because it's a value-type
        ...
        {
            PopulateCollection();
            _collection.Remove( (Point)_objects[ 0 ] );
            _collection.Remove( (Point)_objects[ 0 ] );
        }
        ...
        private void            RemoveFromEmptyCollection()
        {
            ...
            {
                AddFailure( "PointCollection.Remove failed with empty collection" );
        ...
        {
            Log( "Testing PointCollection.RemoveAt with bad parameters..." );
        ...
        private void            TestToStringWith( PointCollection collection )
        {
            ...
            foreach ( Point obj in collection )
            {
                // I don't like calling into another Avalon method, but since test coverage
                //  for ToString should already exist for Point, this is okay.
        ...
        private void            TestToString2()
        {
            ...
            TestToStringWith( NewPointCollection() );
        }
