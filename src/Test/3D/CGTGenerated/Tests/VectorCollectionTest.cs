...
/***********************************************************
*
*   Program:   Tests for VectorCollection (generated)
*
...
namespace                       Microsoft.Test.Graphics.Generated
{
    ...
    public partial class        VectorCollectionTest : CoreGraphicsTest
    {
        // - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        ...
        public override void    Init( Variation v )
        {
            ...
            _collection      = NewVectorCollection();
            _list            = NewVectorCollection();
            _objects         = new List<Vector>();
            _bogusCollection = new ArrayList();

            _objects.Add( new Vector( -2,-2 ) );
            _objects.Add( new Vector( -1,-1 ) );
            _objects.Add( new Vector( 0,0 ) );
            _objects.Add( new Vector( 1,1 ) );
            _objects.Add( new Vector( 2,2 ) );
            _objects.Add( new Vector( 3,3 ) );
        ...
        private VectorCollection NewVectorCollection()
        {
        ...
        private VectorCollection NewVectorCollection( IEnumerable<Vector> collection )
        {
            return new VectorCollection( collection );
        }

        // - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

        private VectorCollection NewVectorCollection( int capacity )
        {
            return new VectorCollection( capacity );
        }
        ...
        {
            _collection = NewVectorCollection();
            foreach ( Vector obj in _objects )
            {
                _collection.Add( obj );
            }
        }
        ...
        private Vector[]        VectorArray
        {
            get
            {
                Vector[] array = new Vector[ _objects.Count ];
        ...
        private void            TestConstructor()
        {
            ...
            VectorCollection empty = NewVectorCollection();
            if ( empty.Count != 0 || failOnPurpose )
            {
                AddFailure( "VectorCollection.ctor() failed" );
                Log( "*** Expected: Default constructor creates empty collection" );
                Log( "*** Actual:   {0} objects in \"empty\" collection", empty.Count );
            }

            PopulateCollection();
            VectorCollection theirAnswer = NewVectorCollection( _collection );

            if ( theirAnswer.Count != _objects.Count || failOnPurpose )
            {
                AddFailure( "VectorCollection.ctor( IEnumerable<Vector> ) failed" );
            ...
            TestConstructorWith( new List<Vector>() );
        ...
        private void            TestConstructorWith( IEnumerable<Vector> e )
        {
            VectorCollection theirAnswer = NewVectorCollection( e );
        ...
        {
            VectorCollection theirAnswer = NewVectorCollection( capacity );

            if ( theirAnswer.Count != 0 || failOnPurpose )
            {
                AddFailure( "VectorCollection.ctor( int ) failed" );
                Log( "*** Expected: Count == 0" );
                Log( "*** Actual:   Count == {0}", theirAnswer.Count );
            }
        }
        ...
        private class           FakeEnumerable : IEnumerable<Vector>, ICollection<Vector>
        {
            ...
            public void         CopyTo( Vector[] points, int index )
            {
            }
            public bool         Remove( Vector p )
            {
                return false;
            }
            public int          Count
            ...
            private class       Enumerator : IEnumerator<Vector>
            {
        ...
        private class           NonCollectionEnumerable : IEnumerable<Vector>
        {
            public IEnumerator<Vector> GetEnumerator()
            {
                return new Enumerator();
        ...
        {
            VectorCollection c = NewVectorCollection( null );
        }
        ...
        private void            TestAddWith( object o )
        {
            ...
            _collection.Add( (Vector)o );

            if ( oldCount+1 != _collection.Count || position != oldCount ||
                    !_collection[ position ].Equals( (Vector)o ) || failOnPurpose )
            {
                AddFailure( "VectorCollection.Add failed" );
                Log( "*** Expected: count = {0}, position = {1}, Vector added = {2}", oldCount+1, oldCount, o );
                Log( "***   Actual: count = {0}, position = {1}, Vector added = {2}", _collection.Count, position, _collection[ position ] );
            }
        }
        ...
        private void            TestAdd2()
        {
            ...
            // Skipping null tests for VectorCollection.Add( Vector ) because it's a value-type
        }
        ...
        {
            Log( "Testing VectorCollection.Changed..." );
        ...
        private void            TestChangedForConstructor()
        {
            ...
            VectorCollection temp = _collection.Clone();
            temp = NewVectorCollection( _collection );
            VerifyCallbacksProcessed();
        ...
        private void            TestContains()
        {
            ...
            Log( "Testing VectorCollection.Contains..." );
            PopulateCollection();
            ...
            foreach ( Vector obj in _objects )
            {
                ...
                {
                    AddFailure( "VectorCollection.Contains failed" );
                    Log( "Could not locate {0} in my collection", obj );
                }
            }
        ...
        private void            TestCopyTo()
        {
            ...
            Log( "Testing VectorCollection.CopyTo..." );
            PopulateCollection();
            Vector[] typedArray = new Vector[ _objects.Count ];
            ...
            for ( int i = 0; i < _collection.Count; i++ )
            {
                ...
                {
                    AddFailure( "CopyTo( Vector[],int ) failed" );
        ...
        private void            TestCopyTo2()
        {
            ...
            Log( "Testing VectorCollection.CopyTo with bad parameters..." );
        ...
        {
            PopulateList();
            Vector[,] array = new Vector[ _objects.Count,_objects.Count ];
            _list.CopyTo( array, 0 );
        }
        ...
        {
            PopulateCollection();
            Vector[] array = new Vector[ _objects.Count-1 ];
            _collection.CopyTo( array, 0 );
        }
        ...
        {
            Log( "Testing VectorEnumerator..." );
            PopulateCollection();

            TestEnumeratorWith( NewVectorCollection() );
            TestEnumeratorWith( _collection );
        }

        // - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

        private void            TestEnumeratorWith( VectorCollection collection )
        {
        ...
        private void            TestMoveNextWith( IEnumerator enumerator, VectorCollection collection )
        {
            int count = 0;
            while ( enumerator.MoveNext() )
            {
                Vector current = (Vector)enumerator.Current;
        ...
        private void            TestEnumerator2()
        {
            ...
            try
            {
                foreach ( Vector o in _collection )
                {
            ...
            {
                PopulateCollection();
                enumerator = ((IEnumerable)NewVectorCollection()).GetEnumerator();
                object foo = enumerator.Current;
        ...
        private void            TestGetAsFrozen()
        {
            ...
            VectorCollection copy = _collection.Clone();
            ...
            VectorCollection frozen = (VectorCollection)copy.GetAsFrozen();
            ...
            copy.Freeze();
            frozen = (VectorCollection)copy.GetAsFrozen();
        ...
        private void            TestGetCurrentValueAsFrozen()
        {
            ...
            VectorCollection frozen = (VectorCollection)copy.GetCurrentValueAsFrozen();
            if ( object.ReferenceEquals( frozen, copy ) ||
                 !ObjectUtils.DeepEqualsToAnimatable( frozen, copy ) ||
                 failOnPurpose )
            {
            ...
            copy.Freeze();
            frozen = (VectorCollection)copy.GetCurrentValueAsFrozen();
            if ( !object.ReferenceEquals( frozen, copy ) || failOnPurpose )
            {
        ...
        private void            TestInsert()
        {
            ...
            Log( "Testing VectorCollection.Insert..." );
            PopulateCollection();
        ...
        private void            TestInsert2()
        {
            ...
            Log( "Testing VectorCollection.Insert with bad parameters..." );
            PopulateCollection();

            // Skipping null tests for VectorCollection.Insert because it's a value-type
        ...
        private void            TestItem()
        {
            ...
            Log( "Testing VectorCollection.get/set_Item..." );
            PopulateCollection();

            _collection[ 0 ] = _objects[ 3 ];
            if ( !_collection[ 0 ].Equals( _objects[ 3 ] ) || failOnPurpose )
            {
                AddFailure( "VectorCollection.get/set_Item failed" );
                Log( "*** Expected: item[0] = {0}", _objects[ 3 ] );
        ...
        private void            TestItem2()
        {
            ...
            Log( "Testing VectorCollection.get/set_Item with bad parameters..." );
            PopulateCollection();
            ...
            // Skipping null tests for VectorCollection.set_Item( int, Vector ) because it's a value-type
        }
        ...
        {
            Vector item = _collection[ -1 ];
        }

        private void            CollectionGetItemIndexTooLarge()
        {
            Vector item = _collection[ _objects.Count ];
        }

        private void            CollectionSetItemNegativeIndex()
        {
            _collection[ -1 ] = (Vector)_objects[ 0 ];
        }

        private void            CollectionSetItemIndexTooLarge()
        {
            _collection[ _objects.Count ] = (Vector)_objects[ 0 ];
        }
        ...
        private void            TestRemove()
        {
            ...
            Log( "Testing VectorCollection.Remove..." );
            PopulateCollection();
        ...
        private void            TestRemove2()
        {
            ...
            Log( "Testing VectorCollection.Remove with bad parameters..." );

            // Skipping null tests for VectorCollection.Remove because it's a value-type
        ...
        {
            PopulateCollection();
            _collection.Remove( (Vector)_objects[ 0 ] );
            _collection.Remove( (Vector)_objects[ 0 ] );
        }
        ...
        private void            RemoveFromEmptyCollection()
        {
            ...
            {
                AddFailure( "VectorCollection.Remove failed with empty collection" );
        ...
        {
            Log( "Testing VectorCollection.RemoveAt with bad parameters..." );
        ...
        private void            TestToStringWith( VectorCollection collection )
        {
            ...
            foreach ( Vector obj in collection )
            {
                // I don't like calling into another Avalon method, but since test coverage
                //  for ToString should already exist for Vector, this is okay.
        ...
        private void            TestToString2()
        {
            ...
            TestToStringWith( NewVectorCollection() );
        }
