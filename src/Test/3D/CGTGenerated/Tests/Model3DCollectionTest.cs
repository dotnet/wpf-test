...
/***********************************************************
*
*   Program:   Tests for Model3DCollection (generated)
*
...
namespace                       Microsoft.Test.Graphics.Generated
{
    ...
    public partial class        Model3DCollectionTest : CoreGraphicsTest
    {
        // - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        ...
        public override void    Init( Variation v )
        {
            ...
            _collection      = NewModel3DCollection();
            _list            = NewModel3DCollection();
            _objects         = new List<Model3D>();
            _bogusCollection = new ArrayList();

            _objects.Add( LightFactory.WhiteDirectionalNegZ );
            _objects.Add( new GeometryModel3D( MeshFactory.SimpleCubeMesh, MaterialFactory.Default ) );
            _objects.Add( LightFactory.WhiteAmbient );
            _objects.Add( new GeometryModel3D( MeshFactory.DoubleTriangleMesh, MaterialFactory.Default ) );
            _objects.Add( new PointLight( Colors.Red, new Point3D( 0,0,0 ) ) );
            _objects.Add( LightFactory.WhiteSpot );
        ...
        private Model3DCollection NewModel3DCollection()
        {
        ...
        private Model3DCollection NewModel3DCollection( IEnumerable<Model3D> collection )
        {
            return new Model3DCollection( collection );
        }

        // - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

        private Model3DCollection NewModel3DCollection( int capacity )
        {
            return new Model3DCollection( capacity );
        }
        ...
        {
            _collection = NewModel3DCollection();
            foreach ( Model3D obj in _objects )
            {
                _collection.Add( obj );
            }
        }
        ...
        private Model3D[]       Model3DArray
        {
            get
            {
                Model3D[] array = new Model3D[ _objects.Count ];
        ...
        private void            TestConstructor()
        {
            ...
            Model3DCollection empty = NewModel3DCollection();
            if ( empty.Count != 0 || failOnPurpose )
            {
                AddFailure( "Model3DCollection.ctor() failed" );
                Log( "*** Expected: Default constructor creates empty collection" );
                Log( "*** Actual:   {0} objects in \"empty\" collection", empty.Count );
            }

            PopulateCollection();
            Model3DCollection theirAnswer = NewModel3DCollection( _collection );

            if ( theirAnswer.Count != _objects.Count || failOnPurpose )
            {
                AddFailure( "Model3DCollection.ctor( IEnumerable<Model3D> ) failed" );
            ...
            TestConstructorWith( new List<Model3D>() );
        ...
        private void            TestConstructorWith( IEnumerable<Model3D> e )
        {
            Model3DCollection theirAnswer = NewModel3DCollection( e );
        ...
        {
            Model3DCollection theirAnswer = NewModel3DCollection( capacity );

            if ( theirAnswer.Count != 0 || failOnPurpose )
            {
                AddFailure( "Model3DCollection.ctor( int ) failed" );
                Log( "*** Expected: Count == 0" );
                Log( "*** Actual:   Count == {0}", theirAnswer.Count );
            }
        }
        ...
        private class           FakeEnumerable : IEnumerable<Model3D>, ICollection<Model3D>
        {
            public void         Add( Model3D p )
            {
            }
            public void         Clear()
            {
            }
            public bool         Contains( Model3D p )
            {
                return false;
            }
            public void         CopyTo( Model3D[] points, int index )
            {
            }
            public bool         Remove( Model3D p )
            {
                return false;
            }
            public int          Count
            ...
            private class       Enumerator : IEnumerator<Model3D>
            {
        ...
        private class           NonCollectionEnumerable : IEnumerable<Model3D>
        {
            public IEnumerator<Model3D> GetEnumerator()
            {
                return new Enumerator();
        ...
        {
            Model3DCollection c = NewModel3DCollection( null );
        }

        private void            ConstructWithNullInArray()
        {
            Model3D[] array = new Model3D[ _objects.Count+1 ];
            _objects.CopyTo( array );
            array[ _objects.Count ] = null;
            Model3DCollection c = NewModel3DCollection( array );
        }
        ...
        private void            TestAddWith( object o )
        {
            ...
            _collection.Add( (Model3D)o );

            if ( oldCount+1 != _collection.Count || position != oldCount ||
                    !_collection[ position ].Equals( (Model3D)o ) || failOnPurpose )
            {
                AddFailure( "Model3DCollection.Add failed" );
                Log( "*** Expected: count = {0}, position = {1}, Model3D added = {2}", oldCount+1, oldCount, o );
                Log( "***   Actual: count = {0}, position = {1}, Model3D added = {2}", _collection.Count, position, _collection[ position ] );
            }
        }
        ...
        private void            TestAdd2()
        {
            ...
            Log( "Testing Model3DCollection.Add with bad parameters..." );
        ...
        {
            Log( "Testing Model3DCollection.Changed..." );
        ...
        private void            TestChangedForConstructor()
        {
            ...
            Model3DCollection temp = _collection.Clone();
            temp = NewModel3DCollection( _collection );
            VerifyCallbacksProcessed();
        ...
        private void            TestContains()
        {
            ...
            Log( "Testing Model3DCollection.Contains..." );
            PopulateCollection();
            ...
            foreach ( Model3D obj in _objects )
            {
                ...
                {
                    AddFailure( "Model3DCollection.Contains failed" );
                    Log( "Could not locate {0} in my collection", obj );
                }
            }
        ...
        private void            TestCopyTo()
        {
            ...
            Log( "Testing Model3DCollection.CopyTo..." );
            PopulateCollection();
            Model3D[] typedArray = new Model3D[ _objects.Count ];
            ...
            for ( int i = 0; i < _collection.Count; i++ )
            {
                ...
                {
                    AddFailure( "CopyTo( Model3D[],int ) failed" );
        ...
        private void            TestCopyTo2()
        {
            ...
            Log( "Testing Model3DCollection.CopyTo with bad parameters..." );
        ...
        {
            PopulateList();
            Model3D[,] array = new Model3D[ _objects.Count,_objects.Count ];
            _list.CopyTo( array, 0 );
        }
        ...
        {
            Log( "Testing Model3DEnumerator..." );
            PopulateCollection();

            TestEnumeratorWith( NewModel3DCollection() );
            TestEnumeratorWith( _collection );
        }

        // - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

        private void            TestEnumeratorWith( Model3DCollection collection )
        {
        ...
        private void            TestMoveNextWith( IEnumerator enumerator, Model3DCollection collection )
        {
            int count = 0;
            while ( enumerator.MoveNext() )
            {
                Model3D current = (Model3D)enumerator.Current;
        ...
        private void            TestEnumerator2()
        {
            ...
            try
            {
                foreach ( Model3D o in _collection )
                {
            ...
            {
                PopulateCollection();
                enumerator = ((IEnumerable)NewModel3DCollection()).GetEnumerator();
                object foo = enumerator.Current;
        ...
        private void            TestGetAsFrozen()
        {
            ...
            Model3DCollection copy = _collection.Clone();
            ...
            Model3DCollection frozen = (Model3DCollection)copy.GetAsFrozen();
            ...
            copy.Freeze();
            frozen = (Model3DCollection)copy.GetAsFrozen();
        ...
        private void            TestGetCurrentValueAsFrozen()
        {
            ...
            Model3DCollection frozen = (Model3DCollection)copy.GetCurrentValueAsFrozen();
            if ( object.ReferenceEquals( frozen, copy ) ||
                 !ObjectUtils.DeepEqualsToAnimatable( frozen, copy ) ||
                 failOnPurpose )
            {
            ...
            copy.Freeze();
            frozen = (Model3DCollection)copy.GetCurrentValueAsFrozen();
            if ( !object.ReferenceEquals( frozen, copy ) || failOnPurpose )
            {
        ...
        private void            TestInsert()
        {
            ...
            Log( "Testing Model3DCollection.Insert..." );
            PopulateCollection();
        ...
        private void            TestInsert2()
        {
            ...
            Log( "Testing Model3DCollection.Insert with bad parameters..." );
            PopulateCollection();
        ...
        private void            TestItem()
        {
            ...
            Log( "Testing Model3DCollection.get/set_Item..." );
            PopulateCollection();

            _collection[ 0 ] = _objects[ 3 ];
            if ( !_collection[ 0 ].Equals( _objects[ 3 ] ) || failOnPurpose )
            {
                AddFailure( "Model3DCollection.get/set_Item failed" );
                Log( "*** Expected: item[0] = {0}", _objects[ 3 ] );
        ...
        private void            TestItem2()
        {
            ...
            Log( "Testing Model3DCollection.get/set_Item with bad parameters..." );
            PopulateCollection();
        ...
        {
            Model3D item = _collection[ -1 ];
        }

        private void            CollectionGetItemIndexTooLarge()
        {
            Model3D item = _collection[ _objects.Count ];
        }

        private void            CollectionSetItemNegativeIndex()
        {
            _collection[ -1 ] = (Model3D)_objects[ 0 ];
        }

        private void            CollectionSetItemIndexTooLarge()
        {
            _collection[ _objects.Count ] = (Model3D)_objects[ 0 ];
        }


        private void            CollectionSetItemNull()
        {
            _collection[ 0 ] = (Model3D)null;
        }
        ...
        private void            TestRemove()
        {
            ...
            Log( "Testing Model3DCollection.Remove..." );
            PopulateCollection();
        ...
        private void            TestRemove2()
        {
            ...
            Log( "Testing Model3DCollection.Remove with bad parameters..." );
        ...
        {
            PopulateCollection();
            _collection.Remove( (Model3D)_objects[ 0 ] );
            _collection.Remove( (Model3D)_objects[ 0 ] );
        }
        ...
        private void            RemoveFromEmptyCollection()
        {
            ...
            {
                AddFailure( "Model3DCollection.Remove failed with empty collection" );
        ...
        {
            Log( "Testing Model3DCollection.RemoveAt with bad parameters..." );
        ...
        private Model3DCollection _collection;
        private IList _list;
        private List<Model3D> _objects;
