...
/***********************************************************
*
*   Program:   Tests for Vector3DCollection (generated)
*
...
namespace                       Microsoft.Test.Graphics.Generated
{
    ...
    public partial class        Vector3DCollectionTest : CoreGraphicsTest
    {
        // - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        ...
        public override void    Init( Variation v )
        {
            ...
            _collection      = NewVector3DCollection();
            _list            = NewVector3DCollection();
            _objects         = new List<Vector3D>();
            _bogusCollection = new ArrayList();

            _objects.Add( new Vector3D( 0,0,0 ) );
            _objects.Add( new Vector3D( 1,1,1 ) );
            _objects.Add( new Vector3D( 2,2,2 ) );
            _objects.Add( new Vector3D( 3,3,3 ) );
            _objects.Add( new Vector3D( 4,4,4 ) );
            _objects.Add( new Vector3D( 5,5,5 ) );
        ...
        private Vector3DCollection NewVector3DCollection()
        {
        ...
        private Vector3DCollection NewVector3DCollection( IEnumerable<Vector3D> collection )
        {
            return new Vector3DCollection( collection );
        }

        // - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

        private Vector3DCollection NewVector3DCollection( int capacity )
        {
            return new Vector3DCollection( capacity );
        }
        ...
        {
            _collection = NewVector3DCollection();
            foreach ( Vector3D obj in _objects )
            {
                _collection.Add( obj );
            }
        }
        ...
        private Vector3D[]      Vector3DArray
        {
            get
            {
                Vector3D[] array = new Vector3D[ _objects.Count ];
        ...
        private void            TestConstructor()
        {
            ...
            Vector3DCollection empty = NewVector3DCollection();
            if ( empty.Count != 0 || failOnPurpose )
            {
                AddFailure( "Vector3DCollection.ctor() failed" );
                Log( "*** Expected: Default constructor creates empty collection" );
                Log( "*** Actual:   {0} objects in \"empty\" collection", empty.Count );
            }

            PopulateCollection();
            Vector3DCollection theirAnswer = NewVector3DCollection( _collection );

            if ( theirAnswer.Count != _objects.Count || failOnPurpose )
            {
                AddFailure( "Vector3DCollection.ctor( IEnumerable<Vector3D> ) failed" );
            ...
            TestConstructorWith( new List<Vector3D>() );
        ...
        private void            TestConstructorWith( IEnumerable<Vector3D> e )
        {
            Vector3DCollection theirAnswer = NewVector3DCollection( e );
        ...
        {
            Vector3DCollection theirAnswer = NewVector3DCollection( capacity );

            if ( theirAnswer.Count != 0 || failOnPurpose )
            {
                AddFailure( "Vector3DCollection.ctor( int ) failed" );
                Log( "*** Expected: Count == 0" );
                Log( "*** Actual:   Count == {0}", theirAnswer.Count );
            }
        }
        ...
        private class           FakeEnumerable : IEnumerable<Vector3D>, ICollection<Vector3D>
        {
            ...
            public void         CopyTo( Vector3D[] points, int index )
            {
            }
            public bool         Remove( Vector3D p )
            {
                return false;
            }
            public int          Count
            ...
            private class       Enumerator : IEnumerator<Vector3D>
            {
        ...
        private class           NonCollectionEnumerable : IEnumerable<Vector3D>
        {
            public IEnumerator<Vector3D> GetEnumerator()
            {
                return new Enumerator();
        ...
        {
            Vector3DCollection c = NewVector3DCollection( null );
        }
        ...
        private void            TestAddWith( object o )
        {
            ...
            _collection.Add( (Vector3D)o );

            if ( oldCount+1 != _collection.Count || position != oldCount ||
                    !_collection[ position ].Equals( (Vector3D)o ) || failOnPurpose )
            {
                AddFailure( "Vector3DCollection.Add failed" );
                Log( "*** Expected: count = {0}, position = {1}, Vector3D added = {2}", oldCount+1, oldCount, o );
                Log( "***   Actual: count = {0}, position = {1}, Vector3D added = {2}", _collection.Count, position, _collection[ position ] );
            }
        }
        ...
        private void            TestAdd2()
        {
            ...
            // Skipping null tests for Vector3DCollection.Add( Vector3D ) because it's a value-type
        }
        ...
        {
            Log( "Testing Vector3DCollection.Changed..." );
        ...
        private void            TestChangedForConstructor()
        {
            ...
            Vector3DCollection temp = _collection.Clone();
            temp = NewVector3DCollection( _collection );
            VerifyCallbacksProcessed();
        ...
        private void            TestContains()
        {
            ...
            Log( "Testing Vector3DCollection.Contains..." );
            PopulateCollection();
            ...
            foreach ( Vector3D obj in _objects )
            {
                ...
                {
                    AddFailure( "Vector3DCollection.Contains failed" );
                    Log( "Could not locate {0} in my collection", obj );
                }
            }
        ...
        private void            TestCopyTo()
        {
            ...
            Log( "Testing Vector3DCollection.CopyTo..." );
            PopulateCollection();
            Vector3D[] typedArray = new Vector3D[ _objects.Count ];
            ...
            for ( int i = 0; i < _collection.Count; i++ )
            {
                ...
                {
                    AddFailure( "CopyTo( Vector3D[],int ) failed" );
        ...
        private void            TestCopyTo2()
        {
            ...
            Log( "Testing Vector3DCollection.CopyTo with bad parameters..." );
        ...
        {
            PopulateList();
            Vector3D[,] array = new Vector3D[ _objects.Count,_objects.Count ];
            _list.CopyTo( array, 0 );
        }
        ...
        {
            PopulateCollection();
            Vector3D[] array = new Vector3D[ _objects.Count-1 ];
            _collection.CopyTo( array, 0 );
        }
        ...
        {
            Log( "Testing Vector3DEnumerator..." );
            PopulateCollection();

            TestEnumeratorWith( NewVector3DCollection() );
            TestEnumeratorWith( _collection );
        }

        // - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

        private void            TestEnumeratorWith( Vector3DCollection collection )
        {
        ...
        private void            TestMoveNextWith( IEnumerator enumerator, Vector3DCollection collection )
        {
            int count = 0;
            while ( enumerator.MoveNext() )
            {
                Vector3D current = (Vector3D)enumerator.Current;
        ...
        private void            TestEnumerator2()
        {
            ...
            try
            {
                foreach ( Vector3D o in _collection )
                {
            ...
            {
                PopulateCollection();
                enumerator = ((IEnumerable)NewVector3DCollection()).GetEnumerator();
                object foo = enumerator.Current;
        ...
        private void            TestGetAsFrozen()
        {
            ...
            Vector3DCollection copy = _collection.Clone();
            ...
            Vector3DCollection frozen = (Vector3DCollection)copy.GetAsFrozen();
            ...
            copy.Freeze();
            frozen = (Vector3DCollection)copy.GetAsFrozen();
        ...
        private void            TestGetCurrentValueAsFrozen()
        {
            ...
            Vector3DCollection frozen = (Vector3DCollection)copy.GetCurrentValueAsFrozen();
            if ( object.ReferenceEquals( frozen, copy ) ||
                 !ObjectUtils.DeepEqualsToAnimatable( frozen, copy ) ||
                 failOnPurpose )
            {
            ...
            copy.Freeze();
            frozen = (Vector3DCollection)copy.GetCurrentValueAsFrozen();
            if ( !object.ReferenceEquals( frozen, copy ) || failOnPurpose )
            {
        ...
        private void            TestInsert()
        {
            ...
            Log( "Testing Vector3DCollection.Insert..." );
            PopulateCollection();
        ...
        private void            TestInsert2()
        {
            ...
            Log( "Testing Vector3DCollection.Insert with bad parameters..." );
            PopulateCollection();

            // Skipping null tests for Vector3DCollection.Insert because it's a value-type
        ...
        private void            TestItem()
        {
            ...
            Log( "Testing Vector3DCollection.get/set_Item..." );
            PopulateCollection();

            _collection[ 0 ] = _objects[ 3 ];
            if ( !_collection[ 0 ].Equals( _objects[ 3 ] ) || failOnPurpose )
            {
                AddFailure( "Vector3DCollection.get/set_Item failed" );
                Log( "*** Expected: item[0] = {0}", _objects[ 3 ] );
        ...
        private void            TestItem2()
        {
            ...
            Log( "Testing Vector3DCollection.get/set_Item with bad parameters..." );
            PopulateCollection();
            ...
            // Skipping null tests for Vector3DCollection.set_Item( int, Vector3D ) because it's a value-type
        }
        ...
        {
            Vector3D item = _collection[ -1 ];
        }

        private void            CollectionGetItemIndexTooLarge()
        {
            Vector3D item = _collection[ _objects.Count ];
        }

        private void            CollectionSetItemNegativeIndex()
        {
            _collection[ -1 ] = (Vector3D)_objects[ 0 ];
        }

        private void            CollectionSetItemIndexTooLarge()
        {
            _collection[ _objects.Count ] = (Vector3D)_objects[ 0 ];
        }
        ...
        private void            TestRemove()
        {
            ...
            Log( "Testing Vector3DCollection.Remove..." );
            PopulateCollection();
        ...
        private void            TestRemove2()
        {
            ...
            Log( "Testing Vector3DCollection.Remove with bad parameters..." );

            // Skipping null tests for Vector3DCollection.Remove because it's a value-type
        ...
        {
            PopulateCollection();
            _collection.Remove( (Vector3D)_objects[ 0 ] );
            _collection.Remove( (Vector3D)_objects[ 0 ] );
        }
        ...
        private void            RemoveFromEmptyCollection()
        {
            ...
            {
                AddFailure( "Vector3DCollection.Remove failed with empty collection" );
        ...
        {
            Log( "Testing Vector3DCollection.RemoveAt with bad parameters..." );
        ...
        private void            TestToStringWith( Vector3DCollection collection )
        {
            ...
            foreach ( Vector3D obj in collection )
            {
                // I don't like calling into another Avalon method, but since test coverage
                //  for ToString should already exist for Vector3D, this is okay.
        ...
        private void            TestToString2()
        {
            ...
            TestToStringWith( NewVector3DCollection() );
        }

        // - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

        private Vector3DCollection _collection;
        private IList _list;
        private List<Vector3D> _objects;
