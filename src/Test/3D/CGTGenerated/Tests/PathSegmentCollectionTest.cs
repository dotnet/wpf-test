...
/***********************************************************
*
*   Program:   Tests for PathSegmentCollection (generated)
*
...
namespace                       Microsoft.Test.Graphics.Generated
{
    ...
    public partial class        PathSegmentCollectionTest : CoreGraphicsTest
    {
        // - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        ...
        public override void    Init( Variation v )
        {
            ...
            _collection      = NewPathSegmentCollection();
            _list            = NewPathSegmentCollection();
            _objects         = new List<PathSegment>();
            _bogusCollection = new ArrayList();

            _objects.Add( new LineSegment( new Point( 90, 10.223 ), true ) );
            _objects.Add( new ArcSegment( new Point( 180, 23 ), new Size( 20, 40 ), 49, true, SweepDirection.Counterclockwise, true) );
            _objects.Add( new BezierSegment( new Point( -10, 32 ), new Point( 2.33, 10 ), new Point( -10, 33 ), true) );
            _objects.Add( new PolyLineSegment(new Point[]{ new Point( 10, 10 ),new Point( 0, -12.2 ) }, false) );
            _objects.Add( new PolyBezierSegment() );
            _objects.Add( new PolyQuadraticBezierSegment() );
        ...
        private PathSegmentCollection NewPathSegmentCollection()
        {
        ...
        private PathSegmentCollection NewPathSegmentCollection( IEnumerable<PathSegment> collection )
        {
            return new PathSegmentCollection( collection );
        }

        // - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

        private PathSegmentCollection NewPathSegmentCollection( int capacity )
        {
            return new PathSegmentCollection( capacity );
        }
        ...
        {
            _collection = NewPathSegmentCollection();
            foreach ( PathSegment obj in _objects )
            {
                _collection.Add( obj );
            }
        }
        ...
        private PathSegment[]   PathSegmentArray
        {
            get
            {
                PathSegment[] array = new PathSegment[ _objects.Count ];
        ...
        private void            TestConstructor()
        {
            ...
            PathSegmentCollection empty = NewPathSegmentCollection();
            if ( empty.Count != 0 || failOnPurpose )
            {
                AddFailure( "PathSegmentCollection.ctor() failed" );
                Log( "*** Expected: Default constructor creates empty collection" );
                Log( "*** Actual:   {0} objects in \"empty\" collection", empty.Count );
            }

            PopulateCollection();
            PathSegmentCollection theirAnswer = NewPathSegmentCollection( _collection );

            if ( theirAnswer.Count != _objects.Count || failOnPurpose )
            {
                AddFailure( "PathSegmentCollection.ctor( IEnumerable<PathSegment> ) failed" );
            ...
            TestConstructorWith( new List<PathSegment>() );
        ...
        private void            TestConstructorWith( IEnumerable<PathSegment> e )
        {
            PathSegmentCollection theirAnswer = NewPathSegmentCollection( e );
        ...
        {
            PathSegmentCollection theirAnswer = NewPathSegmentCollection( capacity );

            if ( theirAnswer.Count != 0 || failOnPurpose )
            {
                AddFailure( "PathSegmentCollection.ctor( int ) failed" );
                Log( "*** Expected: Count == 0" );
                Log( "*** Actual:   Count == {0}", theirAnswer.Count );
            }
        }
        ...
        private class           FakeEnumerable : IEnumerable<PathSegment>, ICollection<PathSegment>
        {
            ...
            public bool         Contains( PathSegment p )
            {
                return false;
            }
            public void         CopyTo( PathSegment[] points, int index )
            {
            }
            public bool         Remove( PathSegment p )
            {
                return false;
            }
            public int          Count
            ...
            private class       Enumerator : IEnumerator<PathSegment>
            {
        ...
        private class           NonCollectionEnumerable : IEnumerable<PathSegment>
        {
            public IEnumerator<PathSegment> GetEnumerator()
            {
                return new Enumerator();
        ...
        {
            PathSegmentCollection c = NewPathSegmentCollection( null );
        }

        private void            ConstructWithNullInArray()
        {
            PathSegment[] array = new PathSegment[ _objects.Count+1 ];
            _objects.CopyTo( array );
            array[ _objects.Count ] = null;
            PathSegmentCollection c = NewPathSegmentCollection( array );
        }
        ...
        private void            TestAddWith( object o )
        {
            ...
            _collection.Add( (PathSegment)o );

            if ( oldCount+1 != _collection.Count || position != oldCount ||
                    !_collection[ position ].Equals( (PathSegment)o ) || failOnPurpose )
            {
                AddFailure( "PathSegmentCollection.Add failed" );
                Log( "*** Expected: count = {0}, position = {1}, PathSegment added = {2}", oldCount+1, oldCount, o );
                Log( "***   Actual: count = {0}, position = {1}, PathSegment added = {2}", _collection.Count, position, _collection[ position ] );
            }
        }
        ...
        private void            TestAdd2()
        {
            ...
            foreach ( object o in _bogusCollection )
            {
                ...
                Type addType = typeof( PathSegment );
                try
                {
                    _list.Add( o );
            ...
            Log( "Testing PathSegmentCollection.Add with bad parameters..." );
        ...
        {
            Log( "Testing PathSegmentCollection.Changed..." );
        ...
        private void            TestChangedForConstructor()
        {
            ...
            PathSegmentCollection temp = _collection.Clone();
            temp = NewPathSegmentCollection( _collection );
            VerifyCallbacksProcessed();
        ...
        private void            TestContains()
        {
            ...
            Log( "Testing PathSegmentCollection.Contains..." );
            PopulateCollection();
            ...
            foreach ( PathSegment obj in _objects )
            {
                ...
                {
                    AddFailure( "PathSegmentCollection.Contains failed" );
                    Log( "Could not locate {0} in my collection", obj );
                }
            }
        ...
        private void            TestContains2()
        {
            ...
            {
                Type itemType = ( o == null ) ? null : o.GetType();
                Type listType = typeof( PathSegment );
        ...
        private void            TestCopyTo()
        {
            ...
            Log( "Testing PathSegmentCollection.CopyTo..." );
            PopulateCollection();
            PathSegment[] typedArray = new PathSegment[ _objects.Count ];
            ...
            for ( int i = 0; i < _collection.Count; i++ )
            {
                ...
                {
                    AddFailure( "CopyTo( PathSegment[],int ) failed" );
        ...
        private void            TestCopyTo2()
        {
            ...
            Log( "Testing PathSegmentCollection.CopyTo with bad parameters..." );
        ...
        {
            PopulateList();
            PathSegment[,] array = new PathSegment[ _objects.Count,_objects.Count ];
            _list.CopyTo( array, 0 );
        }
        ...
        {
            Log( "Testing PathSegmentEnumerator..." );
            PopulateCollection();

            TestEnumeratorWith( NewPathSegmentCollection() );
            TestEnumeratorWith( _collection );
        }

        // - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

        private void            TestEnumeratorWith( PathSegmentCollection collection )
        {
        ...
        private void            TestMoveNextWith( IEnumerator enumerator, PathSegmentCollection collection )
        {
            int count = 0;
            while ( enumerator.MoveNext() )
            {
                PathSegment current = (PathSegment)enumerator.Current;
        ...
        private void            TestEnumerator2()
        {
            ...
            try
            {
                foreach ( PathSegment o in _collection )
                {
            ...
            {
                PopulateCollection();
                enumerator = ((IEnumerable)NewPathSegmentCollection()).GetEnumerator();
                object foo = enumerator.Current;
        ...
        private void            TestGetAsFrozen()
        {
            ...
            PathSegmentCollection copy = _collection.Clone();
            ...
            PathSegmentCollection frozen = (PathSegmentCollection)copy.GetAsFrozen();
            ...
            copy.Freeze();
            frozen = (PathSegmentCollection)copy.GetAsFrozen();
        ...
        private void            TestGetCurrentValueAsFrozen()
        {
            ...
            PathSegmentCollection frozen = (PathSegmentCollection)copy.GetCurrentValueAsFrozen();
            if ( object.ReferenceEquals( frozen, copy ) ||
                 !ObjectUtils.DeepEqualsToAnimatable( frozen, copy ) ||
                 failOnPurpose )
            {
            ...
            copy.Freeze();
            frozen = (PathSegmentCollection)copy.GetCurrentValueAsFrozen();
            if ( !object.ReferenceEquals( frozen, copy ) || failOnPurpose )
            {
        ...
        private void            TestInsert()
        {
            ...
            Log( "Testing PathSegmentCollection.Insert..." );
            PopulateCollection();
        ...
        private void            TestInsert2()
        {
            ...
            Log( "Testing PathSegmentCollection.Insert with bad parameters..." );
            PopulateCollection();
        ...
        private void            TestItem()
        {
            ...
            Log( "Testing PathSegmentCollection.get/set_Item..." );
            PopulateCollection();

            _collection[ 0 ] = _objects[ 3 ];
            if ( !_collection[ 0 ].Equals( _objects[ 3 ] ) || failOnPurpose )
            {
                AddFailure( "PathSegmentCollection.get/set_Item failed" );
                Log( "*** Expected: item[0] = {0}", _objects[ 3 ] );
        ...
        private void            TestItem2()
        {
            ...
            Log( "Testing PathSegmentCollection.get/set_Item with bad parameters..." );
            PopulateCollection();
        ...
        {
            PathSegment item = _collection[ -1 ];
        }

        private void            CollectionGetItemIndexTooLarge()
        {
            PathSegment item = _collection[ _objects.Count ];
        }

        private void            CollectionSetItemNegativeIndex()
        {
            _collection[ -1 ] = (PathSegment)_objects[ 0 ];
        }

        private void            CollectionSetItemIndexTooLarge()
        {
            _collection[ _objects.Count ] = (PathSegment)_objects[ 0 ];
        }


        private void            CollectionSetItemNull()
        {
            _collection[ 0 ] = (PathSegment)null;
        }
        ...
        private void            TestRemove()
        {
            ...
            Log( "Testing PathSegmentCollection.Remove..." );
            PopulateCollection();
        ...
        private void            TestRemove2()
        {
            ...
            Log( "Testing PathSegmentCollection.Remove with bad parameters..." );
        ...
        {
            PopulateCollection();
            _collection.Remove( (PathSegment)_objects[ 0 ] );
            _collection.Remove( (PathSegment)_objects[ 0 ] );
        }
        ...
        private void            RemoveFromEmptyCollection()
        {
            ...
            {
                AddFailure( "PathSegmentCollection.Remove failed with empty collection" );
        ...
        {
            Log( "Testing PathSegmentCollection.RemoveAt with bad parameters..." );
        ...
        private PathSegmentCollection _collection;
        private IList _list;
        private List<PathSegment> _objects;
