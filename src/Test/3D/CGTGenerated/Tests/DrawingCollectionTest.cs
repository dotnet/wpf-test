...
/***********************************************************
*
*   Program:   Tests for DrawingCollection (generated)
*
...
namespace                       Microsoft.Test.Graphics.Generated
{
    ...
    public partial class        DrawingCollectionTest : CoreGraphicsTest
    {
        // - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        ...
        public override void    Init( Variation v )
        {
            ...
            _collection      = NewDrawingCollection();
            _list            = NewDrawingCollection();
            _objects         = new List<Drawing>();
            _bogusCollection = new ArrayList();

            _objects.Add( Const2D.geometryDrawing1 );
            _objects.Add( Const2D.imageDrawing1 );
            _objects.Add( Const2D.glyphRunDrawing1 );
            _objects.Add( Const2D.drawingGroup1 );
        ...
        private DrawingCollection NewDrawingCollection()
        {
        ...
        private DrawingCollection NewDrawingCollection( IEnumerable<Drawing> collection )
        {
            return new DrawingCollection( collection );
        }

        // - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

        private DrawingCollection NewDrawingCollection( int capacity )
        {
            return new DrawingCollection( capacity );
        }
        ...
        {
            _collection = NewDrawingCollection();
            foreach ( Drawing obj in _objects )
            {
                _collection.Add( obj );
            }
        }
        ...
        private Drawing[]       DrawingArray
        {
            get
            {
                Drawing[] array = new Drawing[ _objects.Count ];
        ...
        private void            TestConstructor()
        {
            ...
            DrawingCollection empty = NewDrawingCollection();
            if ( empty.Count != 0 || failOnPurpose )
            {
                AddFailure( "DrawingCollection.ctor() failed" );
                Log( "*** Expected: Default constructor creates empty collection" );
                Log( "*** Actual:   {0} objects in \"empty\" collection", empty.Count );
            }

            PopulateCollection();
            DrawingCollection theirAnswer = NewDrawingCollection( _collection );

            if ( theirAnswer.Count != _objects.Count || failOnPurpose )
            {
                AddFailure( "DrawingCollection.ctor( IEnumerable<Drawing> ) failed" );
            ...
            TestConstructorWith( new List<Drawing>() );
        ...
        private void            TestConstructorWith( IEnumerable<Drawing> e )
        {
            DrawingCollection theirAnswer = NewDrawingCollection( e );
        ...
        {
            DrawingCollection theirAnswer = NewDrawingCollection( capacity );

            if ( theirAnswer.Count != 0 || failOnPurpose )
            {
                AddFailure( "DrawingCollection.ctor( int ) failed" );
                Log( "*** Expected: Count == 0" );
                Log( "*** Actual:   Count == {0}", theirAnswer.Count );
            }
        }
        ...
        private class           FakeEnumerable : IEnumerable<Drawing>, ICollection<Drawing>
        {
            public void         Add( Drawing p )
            {
            }
            public void         Clear()
            {
            }
            public bool         Contains( Drawing p )
            {
                return false;
            }
            public void         CopyTo( Drawing[] points, int index )
            {
            }
            public bool         Remove( Drawing p )
            {
                return false;
            }
            public int          Count
            ...
            private class       Enumerator : IEnumerator<Drawing>
            {
        ...
        private class           NonCollectionEnumerable : IEnumerable<Drawing>
        {
            public IEnumerator<Drawing> GetEnumerator()
            {
                return new Enumerator();
        ...
        {
            DrawingCollection c = NewDrawingCollection( null );
        }

        private void            ConstructWithNullInArray()
        {
            Drawing[] array = new Drawing[ _objects.Count+1 ];
            _objects.CopyTo( array );
            array[ _objects.Count ] = null;
            DrawingCollection c = NewDrawingCollection( array );
        }
        ...
        private void            TestAddWith( object o )
        {
            ...
            _collection.Add( (Drawing)o );

            if ( oldCount+1 != _collection.Count || position != oldCount ||
                    !_collection[ position ].Equals( (Drawing)o ) || failOnPurpose )
            {
                AddFailure( "DrawingCollection.Add failed" );
                Log( "*** Expected: count = {0}, position = {1}, Drawing added = {2}", oldCount+1, oldCount, o );
                Log( "***   Actual: count = {0}, position = {1}, Drawing added = {2}", _collection.Count, position, _collection[ position ] );
            }
        }
        ...
        private void            TestAdd2()
        {
            ...
            Log( "Testing DrawingCollection.Add with bad parameters..." );
        ...
        {
            Log( "Testing DrawingCollection.Changed..." );
        ...
        private void            TestChangedForConstructor()
        {
            ...
            DrawingCollection temp = _collection.Clone();
            temp = NewDrawingCollection( _collection );
            VerifyCallbacksProcessed();
        ...
        private void            TestContains()
        {
            ...
            Log( "Testing DrawingCollection.Contains..." );
            PopulateCollection();
            ...
            foreach ( Drawing obj in _objects )
            {
                ...
                {
                    AddFailure( "DrawingCollection.Contains failed" );
                    Log( "Could not locate {0} in my collection", obj );
                }
            }
        ...
        private void            TestCopyTo()
        {
            ...
            Log( "Testing DrawingCollection.CopyTo..." );
            PopulateCollection();
            Drawing[] typedArray = new Drawing[ _objects.Count ];
            ...
            for ( int i = 0; i < _collection.Count; i++ )
            {
                ...
                {
                    AddFailure( "CopyTo( Drawing[],int ) failed" );
        ...
        private void            TestCopyTo2()
        {
            ...
            Log( "Testing DrawingCollection.CopyTo with bad parameters..." );
        ...
        {
            PopulateList();
            Drawing[,] array = new Drawing[ _objects.Count,_objects.Count ];
            _list.CopyTo( array, 0 );
        }
        ...
        {
            Log( "Testing DrawingEnumerator..." );
            PopulateCollection();

            TestEnumeratorWith( NewDrawingCollection() );
            TestEnumeratorWith( _collection );
        }

        // - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

        private void            TestEnumeratorWith( DrawingCollection collection )
        {
        ...
        private void            TestMoveNextWith( IEnumerator enumerator, DrawingCollection collection )
        {
            int count = 0;
            while ( enumerator.MoveNext() )
            {
                Drawing current = (Drawing)enumerator.Current;
        ...
        private void            TestEnumerator2()
        {
            ...
            try
            {
                foreach ( Drawing o in _collection )
                {
            ...
            {
                PopulateCollection();
                enumerator = ((IEnumerable)NewDrawingCollection()).GetEnumerator();
                object foo = enumerator.Current;
        ...
        private void            TestGetAsFrozen()
        {
            ...
            DrawingCollection copy = _collection.Clone();
            ...
            DrawingCollection frozen = (DrawingCollection)copy.GetAsFrozen();
            ...
            copy.Freeze();
            frozen = (DrawingCollection)copy.GetAsFrozen();
        ...
        private void            TestGetCurrentValueAsFrozen()
        {
            ...
            DrawingCollection frozen = (DrawingCollection)copy.GetCurrentValueAsFrozen();
            if ( object.ReferenceEquals( frozen, copy ) ||
                 !ObjectUtils.DeepEqualsToAnimatable( frozen, copy ) ||
                 failOnPurpose )
            {
            ...
            copy.Freeze();
            frozen = (DrawingCollection)copy.GetCurrentValueAsFrozen();
            if ( !object.ReferenceEquals( frozen, copy ) || failOnPurpose )
            {
        ...
        private void            TestInsert()
        {
            ...
            Log( "Testing DrawingCollection.Insert..." );
            PopulateCollection();
        ...
        private void            TestInsert2()
        {
            ...
            Log( "Testing DrawingCollection.Insert with bad parameters..." );
            PopulateCollection();
        ...
        private void            TestItem()
        {
            ...
            Log( "Testing DrawingCollection.get/set_Item..." );
            PopulateCollection();

            _collection[ 0 ] = _objects[ 3 ];
            if ( !_collection[ 0 ].Equals( _objects[ 3 ] ) || failOnPurpose )
            {
                AddFailure( "DrawingCollection.get/set_Item failed" );
                Log( "*** Expected: item[0] = {0}", _objects[ 3 ] );
        ...
        private void            TestItem2()
        {
            ...
            Log( "Testing DrawingCollection.get/set_Item with bad parameters..." );
            PopulateCollection();
        ...
        {
            Drawing item = _collection[ -1 ];
        }

        private void            CollectionGetItemIndexTooLarge()
        {
            Drawing item = _collection[ _objects.Count ];
        }

        private void            CollectionSetItemNegativeIndex()
        {
            _collection[ -1 ] = (Drawing)_objects[ 0 ];
        }

        private void            CollectionSetItemIndexTooLarge()
        {
            _collection[ _objects.Count ] = (Drawing)_objects[ 0 ];
        }


        private void            CollectionSetItemNull()
        {
            _collection[ 0 ] = (Drawing)null;
        }
        ...
        private void            TestRemove()
        {
            ...
            Log( "Testing DrawingCollection.Remove..." );
            PopulateCollection();
        ...
        private void            TestRemove2()
        {
            ...
            Log( "Testing DrawingCollection.Remove with bad parameters..." );
        ...
        {
            PopulateCollection();
            _collection.Remove( (Drawing)_objects[ 0 ] );
            _collection.Remove( (Drawing)_objects[ 0 ] );
        }
        ...
        private void            RemoveFromEmptyCollection()
        {
            ...
            {
                AddFailure( "DrawingCollection.Remove failed with empty collection" );
        ...
        {
            Log( "Testing DrawingCollection.RemoveAt with bad parameters..." );
        ...
        private DrawingCollection _collection;
        private IList _list;
        private List<Drawing> _objects;
