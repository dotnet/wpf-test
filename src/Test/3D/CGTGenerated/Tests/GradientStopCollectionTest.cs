...
/***********************************************************
*
*   Program:   Tests for GradientStopCollection (generated)
*
...
namespace                       Microsoft.Test.Graphics.Generated
{
    ...
    public partial class        GradientStopCollectionTest : CoreGraphicsTest
    {
        // - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        ...
        public override void    Init( Variation v )
        {
            ...
            _collection      = NewGradientStopCollection();
            _list            = NewGradientStopCollection();
            _objects         = new List<GradientStop>();
            _bogusCollection = new ArrayList();

            _objects.Add( new GradientStop() );
            _objects.Add( new GradientStop( Colors.White, 0.0 ) );
            _objects.Add( new GradientStop( Colors.Red, 1.0 ) );
            _objects.Add( new GradientStop( Colors.Black, 0.5 ) );
            _objects.Add( new GradientStop( Colors.Green, 0.25 ) );
            _objects.Add( new GradientStop( Colors.Orange, 0.75 ) );
        ...
        private GradientStopCollection NewGradientStopCollection()
        {
        ...
        private GradientStopCollection NewGradientStopCollection( IEnumerable<GradientStop> collection )
        {
            return new GradientStopCollection( collection );
        }

        // - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

        private GradientStopCollection NewGradientStopCollection( int capacity )
        {
            return new GradientStopCollection( capacity );
        }
        ...
        {
            _collection = NewGradientStopCollection();
            foreach ( GradientStop obj in _objects )
            {
                _collection.Add( obj );
            }
        }
        ...
        private GradientStop[]  GradientStopArray
        {
            get
            {
                GradientStop[] array = new GradientStop[ _objects.Count ];
        ...
        private void            TestConstructor()
        {
            ...
            GradientStopCollection empty = NewGradientStopCollection();
            if ( empty.Count != 0 || failOnPurpose )
            {
                AddFailure( "GradientStopCollection.ctor() failed" );
                Log( "*** Expected: Default constructor creates empty collection" );
                Log( "*** Actual:   {0} objects in \"empty\" collection", empty.Count );
            }

            PopulateCollection();
            GradientStopCollection theirAnswer = NewGradientStopCollection( _collection );

            if ( theirAnswer.Count != _objects.Count || failOnPurpose )
            {
                AddFailure( "GradientStopCollection.ctor( IEnumerable<GradientStop> ) failed" );
            ...
            TestConstructorWith( new List<GradientStop>() );
        ...
        private void            TestConstructorWith( IEnumerable<GradientStop> e )
        {
            GradientStopCollection theirAnswer = NewGradientStopCollection( e );
        ...
        {
            GradientStopCollection theirAnswer = NewGradientStopCollection( capacity );

            if ( theirAnswer.Count != 0 || failOnPurpose )
            {
                AddFailure( "GradientStopCollection.ctor( int ) failed" );
                Log( "*** Expected: Count == 0" );
                Log( "*** Actual:   Count == {0}", theirAnswer.Count );
            }
        }
        ...
        private class           FakeEnumerable : IEnumerable<GradientStop>, ICollection<GradientStop>
        {
            public void         Add( GradientStop p )
            {
            }
            public void         Clear()
            {
            }
            public bool         Contains( GradientStop p )
            {
                return false;
            }
            public void         CopyTo( GradientStop[] points, int index )
            {
            }
            public bool         Remove( GradientStop p )
            {
                return false;
            }
            public int          Count
            ...
            private class       Enumerator : IEnumerator<GradientStop>
            {
        ...
        private class           NonCollectionEnumerable : IEnumerable<GradientStop>
        {
            public IEnumerator<GradientStop> GetEnumerator()
            {
                return new Enumerator();
        ...
        {
            GradientStopCollection c = NewGradientStopCollection( null );
        }

        private void            ConstructWithNullInArray()
        {
            GradientStop[] array = new GradientStop[ _objects.Count+1 ];
            _objects.CopyTo( array );
            array[ _objects.Count ] = null;
            GradientStopCollection c = NewGradientStopCollection( array );
        }
        ...
        private void            TestAddWith( object o )
        {
            ...
            _collection.Add( (GradientStop)o );

            if ( oldCount+1 != _collection.Count || position != oldCount ||
                    !_collection[ position ].Equals( (GradientStop)o ) || failOnPurpose )
            {
                AddFailure( "GradientStopCollection.Add failed" );
                Log( "*** Expected: count = {0}, position = {1}, GradientStop added = {2}", oldCount+1, oldCount, o );
                Log( "***   Actual: count = {0}, position = {1}, GradientStop added = {2}", _collection.Count, position, _collection[ position ] );
            }
        }
        ...
        private void            TestAdd2()
        {
            ...
            foreach ( object o in _bogusCollection )
            {
                ...
                Type addType = typeof( GradientStop );
                try
                {
                    _list.Add( o );
            ...
            Log( "Testing GradientStopCollection.Add with bad parameters..." );
        ...
        {
            Log( "Testing GradientStopCollection.Changed..." );
        ...
        private void            TestChangedForConstructor()
        {
            ...
            GradientStopCollection temp = _collection.Clone();
            temp = NewGradientStopCollection( _collection );
            VerifyCallbacksProcessed();
        ...
        private void            TestContains()
        {
            ...
            Log( "Testing GradientStopCollection.Contains..." );
            PopulateCollection();
            ...
            foreach ( GradientStop obj in _objects )
            {
                ...
                {
                    AddFailure( "GradientStopCollection.Contains failed" );
                    Log( "Could not locate {0} in my collection", obj );
                }
            }
        ...
        private void            TestContains2()
        {
            ...
            {
                Type itemType = ( o == null ) ? null : o.GetType();
                Type listType = typeof( GradientStop );
        ...
        private void            TestCopyTo()
        {
            ...
            Log( "Testing GradientStopCollection.CopyTo..." );
            PopulateCollection();
            GradientStop[] typedArray = new GradientStop[ _objects.Count ];
            ...
            for ( int i = 0; i < _collection.Count; i++ )
            {
                ...
                {
                    AddFailure( "CopyTo( GradientStop[],int ) failed" );
        ...
        private void            TestCopyTo2()
        {
            ...
            Log( "Testing GradientStopCollection.CopyTo with bad parameters..." );
        ...
        {
            PopulateList();
            GradientStop[,] array = new GradientStop[ _objects.Count,_objects.Count ];
            _list.CopyTo( array, 0 );
        }
        ...
        {
            Log( "Testing GradientStopEnumerator..." );
            PopulateCollection();

            TestEnumeratorWith( NewGradientStopCollection() );
            TestEnumeratorWith( _collection );
        }

        // - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

        private void            TestEnumeratorWith( GradientStopCollection collection )
        {
        ...
        private void            TestMoveNextWith( IEnumerator enumerator, GradientStopCollection collection )
        {
            int count = 0;
            while ( enumerator.MoveNext() )
            {
                GradientStop current = (GradientStop)enumerator.Current;
        ...
        private void            TestEnumerator2()
        {
            ...
            try
            {
                foreach ( GradientStop o in _collection )
                {
            ...
            {
                PopulateCollection();
                enumerator = ((IEnumerable)NewGradientStopCollection()).GetEnumerator();
                object foo = enumerator.Current;
        ...
        private void            TestGetAsFrozen()
        {
            ...
            GradientStopCollection copy = _collection.Clone();
            ...
            GradientStopCollection frozen = (GradientStopCollection)copy.GetAsFrozen();
            ...
            copy.Freeze();
            frozen = (GradientStopCollection)copy.GetAsFrozen();
        ...
        private void            TestGetCurrentValueAsFrozen()
        {
            ...
            GradientStopCollection frozen = (GradientStopCollection)copy.GetCurrentValueAsFrozen();
            if ( object.ReferenceEquals( frozen, copy ) ||
                 !ObjectUtils.DeepEqualsToAnimatable( frozen, copy ) ||
                 failOnPurpose )
            {
            ...
            copy.Freeze();
            frozen = (GradientStopCollection)copy.GetCurrentValueAsFrozen();
            if ( !object.ReferenceEquals( frozen, copy ) || failOnPurpose )
            {
        ...
        private void            TestInsert()
        {
            ...
            Log( "Testing GradientStopCollection.Insert..." );
            PopulateCollection();
        ...
        private void            TestInsert2()
        {
            ...
            Log( "Testing GradientStopCollection.Insert with bad parameters..." );
            PopulateCollection();
        ...
        private void            TestItem()
        {
            ...
            Log( "Testing GradientStopCollection.get/set_Item..." );
            PopulateCollection();

            _collection[ 0 ] = _objects[ 3 ];
            if ( !_collection[ 0 ].Equals( _objects[ 3 ] ) || failOnPurpose )
            {
                AddFailure( "GradientStopCollection.get/set_Item failed" );
                Log( "*** Expected: item[0] = {0}", _objects[ 3 ] );
        ...
        private void            TestItem2()
        {
            ...
            Log( "Testing GradientStopCollection.get/set_Item with bad parameters..." );
            PopulateCollection();
        ...
        {
            GradientStop item = _collection[ -1 ];
        }

        private void            CollectionGetItemIndexTooLarge()
        {
            GradientStop item = _collection[ _objects.Count ];
        }

        private void            CollectionSetItemNegativeIndex()
        {
            _collection[ -1 ] = (GradientStop)_objects[ 0 ];
        }

        private void            CollectionSetItemIndexTooLarge()
        {
            _collection[ _objects.Count ] = (GradientStop)_objects[ 0 ];
        }


        private void            CollectionSetItemNull()
        {
            _collection[ 0 ] = (GradientStop)null;
        }
        ...
        private void            TestRemove()
        {
            ...
            Log( "Testing GradientStopCollection.Remove..." );
            PopulateCollection();
        ...
        private void            TestRemove2()
        {
            ...
            Log( "Testing GradientStopCollection.Remove with bad parameters..." );
        ...
        {
            PopulateCollection();
            _collection.Remove( (GradientStop)_objects[ 0 ] );
            _collection.Remove( (GradientStop)_objects[ 0 ] );
        }
        ...
        private void            RemoveFromEmptyCollection()
        {
            ...
            {
                AddFailure( "GradientStopCollection.Remove failed with empty collection" );
        ...
        {
            Log( "Testing GradientStopCollection.RemoveAt with bad parameters..." );
        ...
        private void            TestToStringWith( GradientStopCollection collection )
        {
            ...
            foreach ( GradientStop obj in collection )
            {
                // I don't like calling into another Avalon method, but since test coverage
                //  for ToString should already exist for GradientStop, this is okay.
        ...
        private void            TestToString2()
        {
            ...
            TestToStringWith( NewGradientStopCollection() );
        }

        // - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

        private GradientStopCollection _collection;
        private IList _list;
        private List<GradientStop> _objects;
