...
/***********************************************************
*
*   Program:   Tests for TransformCollection (generated)
*
...
namespace                       Microsoft.Test.Graphics.Generated
{
    ...
    public partial class        TransformCollectionTest : CoreGraphicsTest
    {
        // - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        ...
        public override void    Init( Variation v )
        {
            ...
            _collection      = NewTransformCollection();
            _list            = NewTransformCollection();
        ...
        private TransformCollection NewTransformCollection()
        {
        ...
        private TransformCollection NewTransformCollection( IEnumerable<Transform> collection )
        {
            return new TransformCollection( collection );
        }

        // - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

        private TransformCollection NewTransformCollection( int capacity )
        {
            return new TransformCollection( capacity );
        }
        ...
        private Transform[]     TransformArray
        {
            get
            {
                Transform[] array = new Transform[ _objects.Count ];
        ...
        private void            TestConstructor()
        {
            ...
            TransformCollection empty = NewTransformCollection();
            if ( empty.Count != 0 || failOnPurpose )
            {
                AddFailure( "TransformCollection.ctor() failed" );
                Log( "*** Expected: Default constructor creates empty collection" );
                Log( "*** Actual:   {0} objects in \"empty\" collection", empty.Count );
            }

            PopulateCollection();
            TransformCollection theirAnswer = NewTransformCollection( _collection );

            if ( theirAnswer.Count != _objects.Count || failOnPurpose )
            {
                AddFailure( "TransformCollection.ctor( IEnumerable<Transform> ) failed" );
            ...
            TestConstructorWith( new List<Transform>() );
        ...
        private void            TestConstructorWith( IEnumerable<Transform> e )
        {
            TransformCollection theirAnswer = NewTransformCollection( e );
        ...
        {
            TransformCollection theirAnswer = NewTransformCollection( capacity );

            if ( theirAnswer.Count != 0 || failOnPurpose )
            {
                AddFailure( "TransformCollection.ctor( int ) failed" );
                Log( "*** Expected: Count == 0" );
                Log( "*** Actual:   Count == {0}", theirAnswer.Count );
            }
        }
        ...
        private class           FakeEnumerable : IEnumerable<Transform>, ICollection<Transform>
        {
            ...
            public bool         Contains( Transform p )
            {
                return false;
            }
            public void         CopyTo( Transform[] points, int index )
            {
            }
            public bool         Remove( Transform p )
            {
                return false;
            }
            public int          Count
        ...
        private class           NonCollectionEnumerable : IEnumerable<Transform>
        {
            public IEnumerator<Transform> GetEnumerator()
            {
                return new Enumerator();
        ...
        {
            TransformCollection c = NewTransformCollection( null );
        }

        private void            ConstructWithNullInArray()
        {
            Transform[] array = new Transform[ _objects.Count+1 ];
            _objects.CopyTo( array );
            array[ _objects.Count ] = null;
            TransformCollection c = NewTransformCollection( array );
        }
        ...
        private void            TestAddWith( object o )
        {
            ...
            _collection.Add( (Transform)o );

            if ( oldCount+1 != _collection.Count || position != oldCount ||
                    !_collection[ position ].Equals( (Transform)o ) || failOnPurpose )
            {
                AddFailure( "TransformCollection.Add failed" );
                Log( "*** Expected: count = {0}, position = {1}, Transform added = {2}", oldCount+1, oldCount, o );
                Log( "***   Actual: count = {0}, position = {1}, Transform added = {2}", _collection.Count, position, _collection[ position ] );
            }
        }
        ...
        private void            TestAdd2()
        {
            ...
            Log( "Testing TransformCollection.Add with bad parameters..." );
        ...
        {
            Log( "Testing TransformCollection.Changed..." );
        ...
        private void            TestChangedForConstructor()
        {
            ...
            TransformCollection temp = _collection.Clone();
            temp = NewTransformCollection( _collection );
            VerifyCallbacksProcessed();
        ...
        private void            TestContains()
        {
            ...
            Log( "Testing TransformCollection.Contains..." );
            PopulateCollection();
            ...
            foreach ( Transform obj in _objects )
            {
                ...
                {
                    AddFailure( "TransformCollection.Contains failed" );
                    Log( "Could not locate {0} in my collection", obj );
                }
            }
        ...
        private void            TestContains2()
        {
            ...
            {
                Type itemType = ( o == null ) ? null : o.GetType();
                Type listType = typeof( Transform );
        ...
        private void            TestCopyTo()
        {
            ...
            Log( "Testing TransformCollection.CopyTo..." );
            PopulateCollection();
            Transform[] typedArray = new Transform[ _objects.Count ];
            ...
            for ( int i = 0; i < _collection.Count; i++ )
            {
                ...
                {
                    AddFailure( "CopyTo( Transform[],int ) failed" );
        ...
        private void            TestCopyTo2()
        {
            ...
            Log( "Testing TransformCollection.CopyTo with bad parameters..." );
        ...
        {
            PopulateList();
            Transform[,] array = new Transform[ _objects.Count,_objects.Count ];
            _list.CopyTo( array, 0 );
        }
        ...
        {
            Log( "Testing TransformEnumerator..." );
            PopulateCollection();

            TestEnumeratorWith( NewTransformCollection() );
            TestEnumeratorWith( _collection );
        }

        // - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

        private void            TestEnumeratorWith( TransformCollection collection )
        {
        ...
        private void            TestMoveNextWith( IEnumerator enumerator, TransformCollection collection )
        {
            int count = 0;
        ...
        private void            TestEnumerator2()
        {
            ...
            try
            {
                foreach ( Transform o in _collection )
                {
            ...
            {
                PopulateCollection();
                enumerator = ((IEnumerable)NewTransformCollection()).GetEnumerator();
                object foo = enumerator.Current;
        ...
        private void            TestGetAsFrozen()
        {
            ...
            TransformCollection copy = _collection.Clone();
            ...
            TransformCollection frozen = (TransformCollection)copy.GetAsFrozen();
            ...
            copy.Freeze();
            frozen = (TransformCollection)copy.GetAsFrozen();
        ...
        private void            TestGetCurrentValueAsFrozen()
        {
            ...
            TransformCollection frozen = (TransformCollection)copy.GetCurrentValueAsFrozen();
            if ( object.ReferenceEquals( frozen, copy ) ||
                 !ObjectUtils.DeepEqualsToAnimatable( frozen, copy ) ||
                 failOnPurpose )
            {
            ...
            copy.Freeze();
            frozen = (TransformCollection)copy.GetCurrentValueAsFrozen();
            if ( !object.ReferenceEquals( frozen, copy ) || failOnPurpose )
            {
        ...
        private void            TestInsert()
        {
            ...
            Log( "Testing TransformCollection.Insert..." );
            PopulateCollection();
        ...
        private void            TestInsert2()
        {
            ...
            Log( "Testing TransformCollection.Insert with bad parameters..." );
            PopulateCollection();
        ...
        private void            TestItem()
        {
            ...
            Log( "Testing TransformCollection.get/set_Item..." );
            PopulateCollection();

            _collection[ 0 ] = _objects[ 3 ];
            if ( !_collection[ 0 ].Equals( _objects[ 3 ] ) || failOnPurpose )
            {
                AddFailure( "TransformCollection.get/set_Item failed" );
                Log( "*** Expected: item[0] = {0}", _objects[ 3 ] );
        ...
        private void            TestItem2()
        {
            ...
            Log( "Testing TransformCollection.get/set_Item with bad parameters..." );
            PopulateCollection();
        ...
        {
            Transform item = _collection[ -1 ];
        }

        private void            CollectionGetItemIndexTooLarge()
        {
            Transform item = _collection[ _objects.Count ];
        }

        private void            CollectionSetItemNegativeIndex()
        {
            _collection[ -1 ] = (Transform)_objects[ 0 ];
        }

        private void            CollectionSetItemIndexTooLarge()
        {
            _collection[ _objects.Count ] = (Transform)_objects[ 0 ];
        }


        private void            CollectionSetItemNull()
        {
            _collection[ 0 ] = (Transform)null;
        }
        ...
        private void            TestRemove()
        {
            ...
            Log( "Testing TransformCollection.Remove..." );
            PopulateCollection();
        ...
        private void            TestRemove2()
        {
            ...
            Log( "Testing TransformCollection.Remove with bad parameters..." );
        ...
        {
            PopulateCollection();
            _collection.Remove( (Transform)_objects[ 0 ] );
            _collection.Remove( (Transform)_objects[ 0 ] );
        }
        ...
        private void            RemoveFromEmptyCollection()
        {
            ...
            {
                AddFailure( "TransformCollection.Remove failed with empty collection" );
        ...
        {
            Log( "Testing TransformCollection.RemoveAt with bad parameters..." );
        ...
        private TransformCollection _collection;
        private IList _list;
