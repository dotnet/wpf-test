...
/***********************************************************
*
*   Program:   Tests for GeometryCollection (generated)
*
...
namespace                       Microsoft.Test.Graphics.Generated
{
    ...
    public partial class        GeometryCollectionTest : CoreGraphicsTest
    {
        // - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        ...
        public override void    Init( Variation v )
        {
            ...
            _collection      = NewGeometryCollection();
            _list            = NewGeometryCollection();
            _objects         = new List<Geometry>();
            _bogusCollection = new ArrayList();

            _objects.Add( Const2D.lineGeometry );
            _objects.Add( Const2D.rectangleGeometry );
            _objects.Add( Const2D.ellipseGeometry );
            _objects.Add( Const2D.combinedGeometry );
            _objects.Add( Const2D.geometryGroup );
            _objects.Add( Const2D.pathGeometry );
        ...
        private GeometryCollection NewGeometryCollection()
        {
        ...
        private GeometryCollection NewGeometryCollection( IEnumerable<Geometry> collection )
        {
        ...
        private GeometryCollection NewGeometryCollection( int capacity )
        {
            return new GeometryCollection( capacity );
        }
        ...
        {
            _collection = NewGeometryCollection();
            foreach ( Geometry obj in _objects )
            {
                _collection.Add( obj );
            }
        }
        ...
        private Geometry[]      GeometryArray
        {
            get
            {
                Geometry[] array = new Geometry[ _objects.Count ];
        ...
        private void            TestConstructor()
        {
            ...
            GeometryCollection empty = NewGeometryCollection();
            if ( empty.Count != 0 || failOnPurpose )
            {
                AddFailure( "GeometryCollection.ctor() failed" );
                Log( "*** Expected: Default constructor creates empty collection" );
                Log( "*** Actual:   {0} objects in \"empty\" collection", empty.Count );
            }

            PopulateCollection();
            GeometryCollection theirAnswer = NewGeometryCollection( _collection );

            if ( theirAnswer.Count != _objects.Count || failOnPurpose )
            {
                AddFailure( "GeometryCollection.ctor( IEnumerable<Geometry> ) failed" );
            ...
            TestConstructorWith( new List<Geometry>() );
        ...
        private void            TestConstructorWith( IEnumerable<Geometry> e )
        {
            GeometryCollection theirAnswer = NewGeometryCollection( e );
        ...
        {
            GeometryCollection theirAnswer = NewGeometryCollection( capacity );

            if ( theirAnswer.Count != 0 || failOnPurpose )
            {
                AddFailure( "GeometryCollection.ctor( int ) failed" );
                Log( "*** Expected: Count == 0" );
                Log( "*** Actual:   Count == {0}", theirAnswer.Count );
            }
        }
        ...
        private class           FakeEnumerable : IEnumerable<Geometry>, ICollection<Geometry>
        {
        ...
        private class           NonCollectionEnumerable : IEnumerable<Geometry>
        {
            public IEnumerator<Geometry> GetEnumerator()
            {
                return new Enumerator();
        ...
        {
            GeometryCollection c = NewGeometryCollection( null );
        }

        private void            ConstructWithNullInArray()
        {
            Geometry[] array = new Geometry[ _objects.Count+1 ];
            _objects.CopyTo( array );
            array[ _objects.Count ] = null;
            GeometryCollection c = NewGeometryCollection( array );
        }
        ...
        private void            TestAddWith( object o )
        {
            ...
            _collection.Add( (Geometry)o );

            if ( oldCount+1 != _collection.Count || position != oldCount ||
                    !_collection[ position ].Equals( (Geometry)o ) || failOnPurpose )
            {
                AddFailure( "GeometryCollection.Add failed" );
                Log( "*** Expected: count = {0}, position = {1}, Geometry added = {2}", oldCount+1, oldCount, o );
                Log( "***   Actual: count = {0}, position = {1}, Geometry added = {2}", _collection.Count, position, _collection[ position ] );
            }
        }
        ...
        private void            TestAdd2()
        {
            ...
            Log( "Testing GeometryCollection.Add with bad parameters..." );
        ...
        {
            Log( "Testing GeometryCollection.Changed..." );
        ...
        private void            TestChangedForConstructor()
        {
            ...
            GeometryCollection temp = _collection.Clone();
            temp = NewGeometryCollection( _collection );
            VerifyCallbacksProcessed();
        ...
        private void            TestContains()
        {
            ...
            Log( "Testing GeometryCollection.Contains..." );
            PopulateCollection();
            ...
            foreach ( Geometry obj in _objects )
            {
                ...
                {
                    AddFailure( "GeometryCollection.Contains failed" );
                    Log( "Could not locate {0} in my collection", obj );
                }
            }
        ...
        private void            TestCopyTo()
        {
            ...
            Log( "Testing GeometryCollection.CopyTo..." );
            PopulateCollection();
            Geometry[] typedArray = new Geometry[ _objects.Count ];
            ...
            for ( int i = 0; i < _collection.Count; i++ )
            {
                ...
                {
                    AddFailure( "CopyTo( Geometry[],int ) failed" );
        ...
        private void            TestCopyTo2()
        {
            ...
            Log( "Testing GeometryCollection.CopyTo with bad parameters..." );
        ...
        {
            PopulateList();
            Geometry[,] array = new Geometry[ _objects.Count,_objects.Count ];
            _list.CopyTo( array, 0 );
        }
        ...
        {
            Log( "Testing GeometryEnumerator..." );
            PopulateCollection();

            TestEnumeratorWith( NewGeometryCollection() );
            TestEnumeratorWith( _collection );
        }

        // - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

        private void            TestEnumeratorWith( GeometryCollection collection )
        {
        ...
        private void            TestMoveNextWith( IEnumerator enumerator, GeometryCollection collection )
        {
            int count = 0;
            while ( enumerator.MoveNext() )
            {
                Geometry current = (Geometry)enumerator.Current;
        ...
        private void            TestEnumerator2()
        {
            ...
            try
            {
                foreach ( Geometry o in _collection )
                {
            ...
            {
                PopulateCollection();
                enumerator = ((IEnumerable)NewGeometryCollection()).GetEnumerator();
                object foo = enumerator.Current;
        ...
        private void            TestGetAsFrozen()
        {
            ...
            GeometryCollection copy = _collection.Clone();
            ...
            GeometryCollection frozen = (GeometryCollection)copy.GetAsFrozen();
            ...
            copy.Freeze();
            frozen = (GeometryCollection)copy.GetAsFrozen();
        ...
        private void            TestGetCurrentValueAsFrozen()
        {
            ...
            GeometryCollection frozen = (GeometryCollection)copy.GetCurrentValueAsFrozen();
            if ( object.ReferenceEquals( frozen, copy ) ||
                 !ObjectUtils.DeepEqualsToAnimatable( frozen, copy ) ||
                 failOnPurpose )
            {
            ...
            copy.Freeze();
            frozen = (GeometryCollection)copy.GetCurrentValueAsFrozen();
            if ( !object.ReferenceEquals( frozen, copy ) || failOnPurpose )
            {
        ...
        private void            TestInsert()
        {
            ...
            Log( "Testing GeometryCollection.Insert..." );
            PopulateCollection();
        ...
        private void            TestInsert2()
        {
            ...
            Log( "Testing GeometryCollection.Insert with bad parameters..." );
            PopulateCollection();
        ...
        private void            TestItem()
        {
            ...
            Log( "Testing GeometryCollection.get/set_Item..." );
            PopulateCollection();

            _collection[ 0 ] = _objects[ 3 ];
            if ( !_collection[ 0 ].Equals( _objects[ 3 ] ) || failOnPurpose )
            {
                AddFailure( "GeometryCollection.get/set_Item failed" );
                Log( "*** Expected: item[0] = {0}", _objects[ 3 ] );
        ...
        private void            TestItem2()
        {
            ...
            Log( "Testing GeometryCollection.get/set_Item with bad parameters..." );
            PopulateCollection();
        ...
        {
            Geometry item = _collection[ -1 ];
        }

        private void            CollectionGetItemIndexTooLarge()
        {
            Geometry item = _collection[ _objects.Count ];
        }

        private void            CollectionSetItemNegativeIndex()
        {
            _collection[ -1 ] = (Geometry)_objects[ 0 ];
        }

        private void            CollectionSetItemIndexTooLarge()
        {
            _collection[ _objects.Count ] = (Geometry)_objects[ 0 ];
        }


        private void            CollectionSetItemNull()
        {
            _collection[ 0 ] = (Geometry)null;
        }
        ...
        private void            TestRemove()
        {
            ...
            Log( "Testing GeometryCollection.Remove..." );
            PopulateCollection();
        ...
        private void            TestRemove2()
        {
            ...
            Log( "Testing GeometryCollection.Remove with bad parameters..." );
        ...
        {
            PopulateCollection();
            _collection.Remove( (Geometry)_objects[ 0 ] );
            _collection.Remove( (Geometry)_objects[ 0 ] );
        }
        ...
        private void            RemoveFromEmptyCollection()
        {
            ...
            {
                AddFailure( "GeometryCollection.Remove failed with empty collection" );
        ...
        {
            Log( "Testing GeometryCollection.RemoveAt with bad parameters..." );
