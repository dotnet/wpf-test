...
//
// Description:
//  The set of Developer Regression Tests for the PackageDigitalSignature classes.
//
//
...
namespace DRT
{
    ...
    public sealed class PackSchemeTestSuite : DrtTestSuite
    {
        ...
        static String s_cacheFile = "cached_" + Title + ".container";

        // test specific
        System.Diagnostics.TraceListener _trace = null;
        private System.Uri _realUri = null;
        private Int32 _timeout = -1;
        private FileInfo    _fileInfo = new FileInfo(s_file);
        ...
        static string[] s_partNames = new string[] {
            "coastal.xaml", 
            "okanagan.xaml", 
            "kootenay.xaml", 
            "/cities/vernon.jpg", 
        ...
        static string[] s_contentTypes = new string[] {
        ...
        public override bool HandleCommandLineArgument(string arg, bool option, string[] args, ref int k)
        {
            if (option)
            {
                switch (arg)
                {
                    ...
                    case "trace":
                        _trace = new System.Diagnostics.ConsoleTraceListener();
                        System.Diagnostics.Trace.Listeners.Clear();
                        System.Diagnostics.Trace.Listeners.Add(_trace);
                        break;
...
#pragma warning restore SYSLIB0009 // Type or member is obsolete
        ...
        public void EnsureContainer()
        {
            if (!_fileInfo.Exists)
            {
                ...
                for (i = 0; i < s_partNames.Length; i++)
                {
                    c.CreatePart(PackUriHelper.CreatePartUri(new Uri(s_partNames[i], UriKind.Relative)), s_contentTypes[i]);
                }
        ...
        // RegressionTest1 - Cached PackWebRequest: if the second request is a cache-hit, 
        ...
        public void RegressionTest1(Uri uri)
        {
            ...
            // would be different.
            FileInfo fi = new FileInfo(s_cacheFile);
            if (fi.Exists)
                fi.Delete();
            File.Copy(_fileInfo.FullName, s_cacheFile);

            c = Package.Open(s_cacheFile, FileMode.Open, FileAccess.Read);
/*
...
//            addMethod.Invoke(null, new object[] { PackUriHelper.GetPackageUri(packUriNoPart), c });
        ...
        // RegressionTest2 - GetResponseStream.Close() will cause failure in ContentLength when
        ...
        //
        public void RegressionTest2(Uri uri)
        {
            Console.WriteLine("RegressionTest2 with Uri: " + uri);
            long length1 = 0;
            long length2 = 0;
            {
            ...
            // verify
            DRT.Assert(length1 == length2, "Different Lengths in test RegressionTest2.  " + length1 + " != " + length2);
        }
#endif

        // RegressionTest3 Close a ResponseStream (or the PackWebResponse) on a 
        // PackWebRequest without exercising the responsed stream will throw 
        // an ApplicationException
        public void RegressionTest3(Uri uri)
        {
            Console.WriteLine("\nRegressionTest3 with Uri: " + uri);
        ...
        // RegressionTest4 Dispose/Close in PackWebRequest and PackWebResponse behave 
        // differently from the standard WebRequest and WebResponse
        public void RegressionTest4(Uri uri)
        {
            Console.WriteLine("\nRegressionTest4 with Uri: " + uri);
        ...
        /// <summary>
        /// RegressionTest5 CompoundFileWebResponse.ContentType will throw ObjectNULLExp
        /// </summary>
        /// <param name="uri"></param>
        public void RegressionTest5(Uri uri)
        {
            Console.WriteLine("\nRegressionTest5 with Uri: " + uri);
        ...
        /// <summary>
        /// RegressionTest6 Missing public properties in CompoundFileWebResponse class
        /// </summary>
        /// <param name="uri"></param>
        public void RegressionTest6(Uri uri)
        {
            Console.WriteLine("\nRegressionTest6 with Uri: " + uri);
        ...
        /// <summary>
        /// RegressionTest7 Closing WebResponse should close Stream (but closing Stream should not close WebResponse)
        /// </summary>
        /// <param name="uri"></param>
        public void RegressionTest7(Uri uri)
        {
            Console.WriteLine("\nRegressionTest7 with Uri: " + uri);
            Console.WriteLine("\tCase 1: Close stream first");
        ...
        private void CachePolicyTests()
        {
            ...
            Package p = Package.Open(fi.FullName, FileMode.Create, FileAccess.ReadWrite);
            Uri packageUri = new Uri(fi.FullName, UriKind.Absolute);
            Uri partUri = new Uri("/Test_willy", UriKind.Relative);
            p.CreatePart(PackUriHelper.CreatePartUri(partUri), "Part/contenttype");
            p.Flush();
