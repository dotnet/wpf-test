...
{
    // @Review : Should nested archiver be returned as part of the "getter" interface ? or just files ?
    // @Review : Should the interface setter rebuild the archive on every call to set ? Or create a new Method on the interface to repackage the files ?
    // @Review : Trying to set a key that does not exist (interface) should throw or create a new entry (current implementation) ?
    // @Review : Should app bypass system setting (read only) when deleting file (see GenArchive private Method) ?
