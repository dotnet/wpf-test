...
namespace Microsoft.Test.Loaders
{
    ...
    public static class FireFoxAutomationHelper
    {
        ...
        public static void NavigateFireFox(IntPtr mainWindowHwnd, string urlToNavigate)
        {
            ...
            //// Normalize the window, just in case it started minimized or is minimized due to user interference.
            //AutomationElement fireFoxWindow = AutomationElement.FromHandle(mainWindowHwnd);
            //WindowPattern windowPattern = fireFoxWindow.GetCurrentPattern(WindowPattern.Pattern) as WindowPattern;
            //windowPattern.SetWindowVisualState(WindowVisualState.Normal);
            //MsaaUIHelper.AccessibleChild addressBar = new MsaaUIHelper.AccessibleChild();

            //// Find the address bar using the IAccessible APIs.  Needed for checking value.
            //IAccessible ffIAcc = MsaaUIHelper.IAccessibleFromHWnd(mainWindowHwnd);
            //MsaaUIHelper.AccessibleChild fireFoxWindowAccChild = new MsaaUIHelper.AccessibleChild(ffIAcc, MsaaUIHelper.CHILDID_SELF);

            //if (Microsoft.Test.Diagnostics.FirefoxHelper.FireFoxVersion.StartsWith("1") || Microsoft.Test.Diagnostics.FirefoxHelper.FireFoxVersion.StartsWith("2"))
            //{
            //    MsaaUIHelper.AccessibleChild locationComboBox = (FindAccessibleChildrenByName(fireFoxWindowAccChild, "Location"))[0];
            //    addressBar = MsaaUIHelper.GetAccessibleChildren(locationComboBox)[2];
            //}
            //else if (Microsoft.Test.Diagnostics.FirefoxHelper.FireFoxVersion.StartsWith("3.5") ||
            //        (Microsoft.Test.Diagnostics.FirefoxHelper.FireFoxVersion.StartsWith("3.0")))
            //{
            //    MsaaUIHelper.AccessibleChild locationComboBox = (FindAccessibleChildrenByName(fireFoxWindowAccChild, "Location"))[0];
            //    addressBar = MsaaUIHelper.GetAccessibleChildren(locationComboBox)[1];
            //}
            //else if (Microsoft.Test.Diagnostics.FirefoxHelper.FireFoxVersion.StartsWith("3.6"))
            //// This is tested to work on 3.6 at least
            //{
            //    MsaaUIHelper.AccessibleChild addressBarParent = (FindAccessibleChildrenByName(fireFoxWindowAccChild, "Search Bookmarks and History"))[0];
            //    addressBar = (FindAccessibleChildrenByName(addressBarParent, "Search Bookmarks and History"))[0];
            //}
            
            //int retryCount = 3;
            //string detectedAddressBar = "";

            //do
            //{
            //    fireFoxWindow.SetFocus();
            ...
            //    MTI.Input.SendUnicodeString(urlToNavigate, 1, 100);

            //    Thread.Sleep(150);

            //    // Sleep a bit to ensure that the browser has a chance to begin before following code starts.
            //    // Yes, I know, Sleep is evil, yada yada.  
            //    retryCount--;
            //    // If we're retrying, sleep progressively longer.  Default: 1 Second.
            //    Thread.Sleep((3 - retryCount) * 1000);
            //    detectedAddressBar = addressBar.IAccessible.get_accValue(((object)MsaaUIHelper.CHILDID_SELF));
            //}
            //while ((detectedAddressBar != urlToNavigate) && (retryCount > 0));
            ...
            //// Hit a 2nd return for IME enabled OSes (Japan, Korea, Chinese Simplified, Chinese Traditional)
            //if ( (keyboardLayout == 0x0411) || (keyboardLayout == 0x0412) || (keyboardLayout == 0x0404) || (keyboardLayout == 0x0804) )
            //{
            //    MTI.Input.SendKeyboardInput(Key.Enter, true);
            //    MTI.Input.SendKeyboardInput(Key.Enter, false);
            //}
