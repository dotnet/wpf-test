...
namespace Microsoft.Test.Input
{
    ...
    // see Input.SendMouseInputInternal
    internal enum MouseMoveMethod
    {
        None,           // awaiting lazy initialization
        WinForms,       // use WF.Cursor.set_Position (4.0-4.8)
        SetCursorPos,   // call SetCursorPos directly
        SendInput,      // call SendInput, with no-coalesce flag
        SendInputImpliedMove, // like SendInput, but Absolute implies Move
    }
    ...
    public sealed class Input
    {
        ...
        private static MouseMoveMethod _mouseMoveMethod = MouseMoveMethod.None;

        // Decide how we move the mouse.  Defaults to SendInput.
        // For debugging or back-compat, user can override by setting
        // environment variable WPF_Test_MouseMoveMethod
        private static MouseMoveMethod MouseMoveMethod
        {
            get
            {
                // lazy initialization
                if (_mouseMoveMethod == MouseMoveMethod.None)
                {
                    // assume default
                    _mouseMoveMethod = MouseMoveMethod.SendInput;

                    // see if user wants to override
                    string s = System.Environment.GetEnvironmentVariable("WPF_Test_MouseMoveMethod");
                    if (!String.IsNullOrWhiteSpace(s))
                    {
                        MouseMoveMethod t;
                        if (Enum.TryParse(s, false, out t))
                        {
                            _mouseMoveMethod = t;
                        }
                    }
                ...
                return _mouseMoveMethod;
            }
        }

        // separate method, to avoid loading WinForms.dll if not needed
        [System.Runtime.CompilerServices.MethodImpl(System.Runtime.CompilerServices.MethodImplOptions.NoInlining)]
        private static void UseWinFormsToMoveMouse(double x, double y)
        {
            System.Windows.Forms.Cursor.Position = new System.Drawing.Point((int)(x + 0.5), (int)(y + 0.5));
        }

        private static int _virtualDesktopXOffset;
        private static int _virtualDesktopYOffset;
        private static double _virtualDesktopXFactor = 0.0;
        private static double _virtualDesktopYFactor = 0.0;

        private static void NormalizeAbsoluteCoordinates(ref double x, ref double y)
        {
            if (_virtualDesktopXFactor == 0.0)
            {
                _virtualDesktopXOffset = GetSystemMetrics(SM_XVIRTUALSCREEN);
                _virtualDesktopYOffset = GetSystemMetrics(SM_YVIRTUALSCREEN);
                _virtualDesktopXFactor = 65536.0d / GetSystemMetrics(SM_CXVIRTUALSCREEN);
                _virtualDesktopYFactor = 65536.0d / GetSystemMetrics(SM_CYVIRTUALSCREEN);
            }

            x = (x - _virtualDesktopXOffset + 0.5) * _virtualDesktopXFactor;
            y = (y - _virtualDesktopYOffset + 0.5) * _virtualDesktopYFactor;
        }
