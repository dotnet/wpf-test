...
namespace Microsoft.Test.RenderingVerification
{
    ...
    public partial class ImageUtility
    {
        ...
        /// <summary>
        /// Returns the screen bounding rectangle (including non-client area) for a top-level window.
        /// </summary>
        /// <param name="target">UIElement to get screen bounding rectangle</param>
        /// <returns>the screen bounding rectangle relative to the screen origin for the specified Element</returns>
        [PermissionSet(SecurityAction.Assert, Name = "FullTrust")]
        public static System.Drawing.Rectangle GetScreenBoundingRectangleNonClient(Window target)
        {
            PresentationSource source = PresentationSource.FromVisual(target);
            if (source == null || target != source.RootVisual)
                throw new InvalidOperationException("The specified Window is not connected to a rendering Visual Tree.");

            // shortcut when target is a window.  The transforms are all trivial,
            // and we should not call ClientToScreen
            System.Drawing.Rectangle windowRect = new System.Drawing.Rectangle();
            User32.GetWindowRect(((System.Windows.Interop.HwndSource)source).Handle, ref windowRect);

            // GetWindowRect returns bottom-right corner.   Convert to width x height.
            windowRect.Width -= windowRect.Left;
            windowRect.Height -= windowRect.Top;
            return windowRect;
        }
