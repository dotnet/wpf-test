...
{
   // FullTrust will mess up AutoData (GetEntryAssembly would go too deep)
   // [PermissionSet(SecurityAction.Assert, Name="FullTrust")]
//   [PermissionSet(SecurityAction.Assert)]
   // Unrestricted = true will mess up AutoData (GetEntryAssembly would go too deep)
            ...
            foreach(XmlNode node in nodeList)
            {
               ...
               { 
                  if (paramNames.Length == 1)
                  {
                     ...
                     {
                         // BUGBUG: Filter out the comments
