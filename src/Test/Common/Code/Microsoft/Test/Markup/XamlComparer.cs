...
namespace Microsoft.Test.Markup
{
    ...
    public static class XamlComparer
    {
        ...
        public static XmlCompareResult Compare(Stream xml1, Stream xml2, string[] nodesShouldIgnoreChildrenSequence)
        {
            ...
            doc2.Load(xml2);
            //Go Compare XmlNodes
            result = Compare((XmlNode)doc1, (XmlNode)doc2, nodesShouldIgnoreChildrenSequence);
            if (CompareResult.Equivalent != result.Result)
                SendCompareMessage("I am an ---- for trying to log the text of each xaml when I'm already logging the files.");
                //SendCompareMessage("Different xml. First:\n"
                //    + doc1.OuterXml + "\nSecond:\n" + doc2.OuterXml);
