    ...
    public abstract partial class DrtBase
    {
        ...
        void RunNextTest()
        {
            ...
            else
            {
                ...
                {
                    // can't throw an exception here - it'll get ----ed by the
...
#if false
                //
                bool isClientSKU = IsRegDWordValuePresent(Registry.LocalMachine, @"SOFTWARE\Microsoft\NET Framework Setup\NDP\????\Setup", "InstallSuccess", 1);
#else
        ...
        private class StringAndFileWriter : StringWriter
        {
            ...
            private bool EnsureFileOpen()
            {
                if (!_isClosed)
                {
                    if (_logFile == null)
                    {
                        _logFile = System.IO.File.CreateText(_drtName + ".log");
                    }
                }
        ...
        private static Visual _rootElement;
        private static bool _topMost = false;
        private static AutomationElement _logicalRoot;
