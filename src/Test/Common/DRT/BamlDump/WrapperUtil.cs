...
namespace DRT
{
    ...
    public static class WrapperUtil
    {
        ...
        static CultureInfo InvariantEnglishUS = new CultureInfo("en-us", false);
        ...
        internal static Assembly FindAssemblyInDomain(string assemblyName)
        {
            ...
            // </SecurityNote>         
            {
                ...
                try
                {
                    ...
                    {
                        if (String.Compare(assemblies[i].FullName, assemblyName, false, InvariantEnglishUS) == 0 ||
                            String.Compare( assemblies[i].GetName().Name, assemblyName, false, InvariantEnglishUS) == 0 )
                        {
        ...
        internal static void ConvertToBaml(string filename)
        {
            ...
            object referenceAssembly = Activator.CreateInstance(typeReferenceAssembly,
                                         ...
                                         null, new object[2] { refWinFXAssemblyPath + "PresentationFramework.dll", "PresentationFramework" }, InvariantEnglishUS);
            refAssemblies.Add(referenceAssembly);
            referenceAssembly = Activator.CreateInstance(typeReferenceAssembly,
                                         BindingFlags.Instance | BindingFlags.NonPublic | BindingFlags.CreateInstance,
                                         null, new object[2] { refWinFXAssemblyPath + "PresentationCore.dll", "PresentationCore" }, InvariantEnglishUS);
            refAssemblies.Add(referenceAssembly);

            referenceAssembly = Activator.CreateInstance(typeReferenceAssembly,
                                         BindingFlags.Instance | BindingFlags.NonPublic | BindingFlags.CreateInstance,
                                         null, new object[2] { refWinFXAssemblyPath + "WindowsBase.dll", "WindowsBase" }, InvariantEnglishUS);
            refAssemblies.Add(referenceAssembly);

            referenceAssembly = Activator.CreateInstance(typeReferenceAssembly,
                                         BindingFlags.Instance | BindingFlags.NonPublic | BindingFlags.CreateInstance,
                                         null, new object[2] { refWinFXAssemblyPath + "UIAutomationProvider.dll", "UIAutomationProvider" }, InvariantEnglishUS);
            refAssemblies.Add(referenceAssembly);
