...
public class Tabs : WPFReflectBase
{
    ...
    // class vars
    private string appTitle = "Tabs TC";        // so that Mita can find it
    private DockPanel _dp;
    private string events = string.Empty;       // string of logged Focus events
	...
	Utilities.SleepDoEvents(10);
        ...
        // examine results
        // Note: Windows OS Bug 1540048 used to happen here
        string expResults = "wfTB1:wfTB2:wfTB4:wfTB3:wfTB1:wfTB2:";
        p.log.WriteLine("Expected events = '{0}'", expResults);
        p.log.WriteLine("Actual   events = '{0}'", events);
        WPFMiscUtils.IncCounters(sr, expResults, events, "Did not get expected events", p.log);
    ...
    [Scenario("Windows OS Bug 1546950 - Verify that WF controls in a WFH get Enter/GotFocus/LostFocus events")]
    public ScenarioResult Scenario7(TParams p)
    {
        ...
        p.log.WriteLine("Expected events = '{0}'", expResults);
        p.log.WriteLine("Actual   events = '{0}'", events);
        WPFMiscUtils.IncCounters(sr, expResults, events, "Did not get expected events Entering WF control", p.log);
        ...
        p.log.WriteLine("Expected events = '{0}'", expResults);
        p.log.WriteLine("Actual   events = '{0}'", events);
        WPFMiscUtils.IncCounters(sr, expResults, events, "Did not get expected events Leaving WF control", p.log);
    ...
    private void SetupScenario(TParams p, string scenario)
    {
        ...
        {
            // Windows OS Bug 1546950 - Verify that WF controls in a WFH get Enter/GotFocus/LostFocus events
