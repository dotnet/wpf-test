...
namespace ReflectTools
{
    ...
    ///
    /// Added : Tests event add/remove methods using the OnXxxx() methods to fire events.
    ...
    public abstract class WPFAutoTest : WPFAutoPME
    {
        ...
        protected WPFAutoTest(string[] args) : base(args) { }
        ...
        private void HandleException(TParams p, ScenarioResult result, Exception e)
        {
            ...
            if (e is SecurityException)
                ...
                WPFMiscUtils.IncCounters(result, p.log, false, "FAIL: caught unexpected exception " + e.GetType().ToString());
                                        ...
                                        bool value)
        {
            ...
            WPFMiscUtils.IncCounters(result, p.log, returned == value, "FAIL: (" + propertyName + ") set to " + value + "; get returned " + returned);
                                        ...
                                        Enum value)
        {
            ...
            WPFMiscUtils.IncCounters(result, p.log, returned.Equals(value), "FAIL: (" + 
                propertyName + ") set to " + value.ToString() + "; get returned " + returned.ToString());
                                            ...
                                            int value)
        {
            ...
            try
            {
                ...
                WPFMiscUtils.IncCounters(result, p.log, false, "FAIL: set to " + value + "; No exception thrown");
            ...
            catch (TargetInvocationException te)
            {
                ...
                WPFMiscUtils.IncCounters(result, p.log, passed, "FAIL: Failed set invalid enum value");
                                          ...
                                          string value)
        {
            ...
            if (value == null)
            {
                ...
                WPFMiscUtils.IncCounters(result, p.log, returned == value || returned == "", "FAIL: (" + propertyName + ") " + comment);
            ...
            else
            {
                ...
                WPFMiscUtils.IncCounters(result, p.log, returned == value, "FAIL: (" + propertyName + ") " + comment);
        ...
        private ScenarioResult AutoTestEventsActual(TParams p)
        {
            ...
            foreach (EventInfo info in _events)
            {
                ...
                else
                {
                    ...
                    if (onMethod == null)
                    {
                        WPFMiscUtils.IncCounters(result, p.log, false, "FAIL: No On" + info.Name + "() method found.  Exclude this event from AutoTest or define a Raise" + info.Name + "() method.");
                        ...
                        {                              // set up for invoking the On method
                            ...
                            for (int i = 0; i < pis.Length; i++)
                            {
                                if (pis[i].ParameterType == typeof(HtmlElementEventArgs))
                                    parameters[i] = new EventArgs();
                                else if (pis[i].ParameterType == typeof(RoutedEventArgs))
                                    parameters[i] = new RoutedEventArgs();
                                else
