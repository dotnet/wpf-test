...
public class MasterDetail : WPFReflectBase
{
    ...
    // class vars
    private Families fams;
    private ListBox avLB1;
    private System.Windows.Forms.TextBox wfTB1;
    private System.Windows.Forms.DataGridView wfDGV;
    private System.Windows.Forms.BindingSource bsM;
    private System.Windows.Forms.BindingSource bsD;
    ...
    public class Family : INotifyPropertyChanged
    {
        ...
        public override string ToString()
        {
            ...
            return familyName + " (" + members.Count + " members)";
        }
    ...
    public ScenarioResult Scenario1(TParams p)
    {
        ...
        avLB1.ItemsSource = bsM;
        FixCurrency();

        // set up detail
        // detail as textbox with "currently selected master"
        System.Windows.Forms.Binding b1 = new System.Windows.Forms.Binding("Text", bsM, "FamilyName");
        ...
        VerifyControlData(p, sr, avLB1, wfDGV, wfTB1);
    ...
    public ScenarioResult Scenario2(TParams p)
    {
        ...
        // detail as data grid
        //wfDGV.DataSource = bsD;
        Winform.dataGridView1.DataSource = bsD;

        //!!!Utilities.ActiveFreeze(currentScenario.Name);

        // check data binding to controls
        VerifyControlData(p, sr, avLB1, Winform.dataGridView1, Winform.textBox1);
    ...
    public ScenarioResult Scenario3(TParams p)
    {
        ...
        // set up master
        // master as data grid
        wfDGV.DataSource = bsM;

        // set up detail
        // detail as listbox
        avLB1.ItemsSource = bsD;
        ...
        // check data binding to controls
        VerifyControlData(p, sr, wfDGV, avLB1, wfTB1);
    ...
    public ScenarioResult Scenario4(TParams p)
    {
        ...
        // set up master
        Winform.dataGridView1.DataSource = bsM;
        ...
        // check data binding to controls
        VerifyControlData(p, sr, Winform.dataGridView1, avLB1, Winform.textBox1);
        Winform.Close();
    ...
    private void CreateListData()
    {
        ...
        bsM = new System.Windows.Forms.BindingSource();
        bsD = new System.Windows.Forms.BindingSource();
        bsM.DataSource = fams;
        bsD.DataSource = bsM;
        bsD.DataMember = "Members";
    }
    ...
    private string[] expFamilyName = { "Stooges", "Peanuts", "MASH", "Marx" };
    private int[] expMemberCount = { 3, 6, 5, 4 };
    private string[] expMemberList = { 
            "Larry:Moe:Curly:" ,
    ...
    private void FixCurrency()
    {
        ...
        BindingListCollectionView cv = (BindingListCollectionView)CollectionViewSource.GetDefaultView(bsM);
        cv.CurrentChanged += new EventHandler(cv_CurrentChanged);
    }

    /// <summary>
    /// Silly helper function that keeps data in sync with Avalon list !!!
    /// </summary>
    ...
    void cv_CurrentChanged(object sender, EventArgs e)
    {
        ...
        bsM.Position = cv.CurrentPosition;
    }
    ...
    private void CreateWinformsControls(StackPanel sp)
    {
        ...
        wfDGV = new System.Windows.Forms.DataGridView();
        wfDGV.Name = "wfDGV";
    ...
    private void VerifyControlData(TParams p, ScenarioResult sr, ListBox LBmaster, System.Windows.Forms.DataGridView DGVdetail, System.Windows.Forms.TextBox TBcursel)
    {
        ...
        for (int i = 0; i < fams.Count; i++)
        {
            ...
            if (debug) { p.log.WriteLine("Selected master is {0}", idxM); }
            ...
            if (debug) { p.log.WriteLine("Current FamilyName is '{0}'", curFam); }
            WPFMiscUtils.IncCounters(sr, expFamilyName[i], curFam, "Currently selected family is incorrect", p.log);

            // detail list count should match member count list corresponding to loop index
            if (debug) { p.log.WriteLine("Number of detail rows '{0}'", cntDet); }
            WPFMiscUtils.IncCounters(sr, expMemberCount[i], cntDet, "Detail list member count is incorrect", p.log);
            ...
            foreach (System.Windows.Forms.DataGridViewRow row in DGVdetail.Rows)
            {
                ...
                if (debug) { p.log.WriteLine("  Row in Detail list '{0}'", val); }
                actList += (val + ":");
            }

            // list of details should match list for loop index
            WPFMiscUtils.IncCounters(sr, expMemberList[i], actList, "List of detail members is incorrect", p.log);
