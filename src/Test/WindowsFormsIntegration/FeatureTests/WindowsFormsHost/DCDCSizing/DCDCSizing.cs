...
namespace WindowsFormsHostTests
{
    public class DCDCSizing : WPFReflectBase
    {
        ...
        private SWF.FlowLayoutPanel _wfFLP;
        private WindowsFormsHost _wfh1;
        private int _buttonCount = 15;
        private int _btnWidth;
        private int _btnHeight;
        private SD.Point _originPanel;           // upper left point of SP
        ...
        public ScenarioResult Scenario1(TParams p)
        {
            ...
            WPFMiscUtils.IncCounters(sr, HorizontalAlignment.Stretch, _wfh1.HorizontalAlignment,
            ...
            _wfh1.HorizontalAlignment = HorizontalAlignment.Stretch;
            _originPanel = _wfh1.Child.Parent.PointToScreen(new SD.Point(0, 0));
            p.log.WriteLine("Got SP Origin as ({0},{1})", _originPanel.X, _originPanel.Y);
        ...
        private void ExerciseWindow(TParams p, ScenarioResult sr)
        {
            ...
            p.log.WriteLine("Button Width   {0}", _btnWidth);
            p.log.WriteLine("Button Height  {0}", _btnHeight);
            ...
            for (int col = 2; col < 8; col++)
            {
                ...
                int width = col * (_btnWidth + 8);   // button width plus gap between columns
                width += (_btnWidth / 2);            // half a column to insure between columns
                ...
                int expRows = _buttonCount / col;    // integer math - truncate remainder
                // if is not evenly divisible, we will have an extra row
                if (_buttonCount % col > 0) { expRows++; }
                p.log.WriteLine("");
                ...
                int expPanelHeight = expRows * _btnHeight;   // height of single button
                expPanelHeight += expRows * 6;              // gap between rows of buttons
                p.log.WriteLine("Expect Panel Height to be {0}", expPanelHeight);

                // check window height
                p.log.WriteLine("Panel Width   {0}", _wfFLP.Size.Width);
                p.log.WriteLine("Panel Height  {0}", _wfFLP.Size.Height);

                WPFMiscUtils.IncCounters(sr, expPanelHeight, _wfFLP.Size.Height,
        ...
        {
            HorizontalAlignment ha = _wfh1.HorizontalAlignment;
            ...
            int originX = _originPanel.X;
            int originY = _originPanel.Y;
            ...
            int heightAll = (int)_wfh1.ActualHeight;
            ...
            switch (ha)
            {
                case HorizontalAlignment.Left:
                    ...
                    widthLeft = 0;
                    widthMiddle = (int)_wfh1.ActualWidth;
                    widthRight = (int)(_sp.ActualWidth - _wfh1.ActualWidth);
                    break;

                case HorizontalAlignment.Right:
                    // should have host right justified in panel, with extra space on left side
                    widthLeft = (int)(_sp.ActualWidth - _wfh1.ActualWidth);
                    widthMiddle = (int)_wfh1.ActualWidth;
                    widthRight = 0;
                    break;
                ...
                case HorizontalAlignment.Center:
                    ...
                    widthMiddle = (int)_wfh1.ActualWidth;
                    widthLeft = (int)(_sp.ActualWidth - _wfh1.ActualWidth) / 2;
                    widthRight = (int)(_sp.ActualWidth - _wfh1.ActualWidth) / 2;

                    // Note: when amount to be divided between left and right is odd, it
                    // appears that the left side gets the extra pixel width
                    if ((int)(_sp.ActualWidth - _wfh1.ActualWidth) % 2 != 0)
                    {
                        widthLeft++;
                    }
                    break;
                ...
                case HorizontalAlignment.Stretch:
                    ...
                    widthLeft = 0;
                    widthMiddle = (int)_sp.ActualWidth;
                    widthRight = 0;
                    break;
            ...
            WPFMiscUtils.IncCounters(sr, (int)_sp.ActualWidth, sum, "Panel sizes do not add up", p.log);
        ...
        private void CreateTestControls()
        {
            ...
            _wfFLP = new System.Windows.Forms.FlowLayoutPanel();
            _wfh1.Child = _wfFLP;

            // set colors so can see host background
            _wfFLP.Parent.Paint += new System.Windows.Forms.PaintEventHandler(Parent_Paint);
            _wfFLP.BackColor = SD.Color.LightBlue;
            ...
            for (int i = 0; i < _buttonCount; i++)
            {
                ...
                btn.Text = "Button " + i;
                _wfFLP.Controls.Add(btn);
                _btnWidth = btn.Width;
                _btnHeight = btn.Height;
            }
        ...
        public class Win : Window
        {
            ...
            public Win()
            {
                ...
                _label = new Label();
                _label.Content = "An Avalon UIElement";
                _label.Height = 200;
                _label.Background = Brushes.OrangeRed;
                _stackPanel.Children.Add(_label);
                _host = new WindowsFormsHost();
                _host.Background = Brushes.Blue;
                this.Background = Brushes.Beige;
                _flp = new SWF.FlowLayoutPanel();
                _flp.BackColor = SD.Color.Yellow;
                _flp.Size = new System.Drawing.Size(170, 300);
                _host.Child = _flp;
                for (int i = 0; i < _buttonCount; i++)
                {
                    SWF.Button btn = new SWF.Button();
