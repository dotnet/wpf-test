...
public class FontPropagation : WPFReflectBase
{
    ...
    private Button avb1;
    private bool debug = false;      // used for TC debugging
    ...
    private static MyFontItemType[] MyFontList = {
    ...
    public ScenarioResult Scenario1(TParams p)
    {
        ...
        foreach (MyFontItemType item in MyFontList)
        {
            ...
            VerifyFontInHost(p, sr, wfh1, item);
            VerifyFontInHost(p, sr, wfh2, item);
            VerifyFontInWFControl(p, sr, wfb1, item);
            VerifyFontInWFControl(p, sr, wfb2, item);
            VerifyFontInWFControl(p, sr, wfb3, item);
        }
        return sr;
    }
    ...
    public ScenarioResult Scenario3(TParams p)
    {
        ...
        foreach (MyFontItemType item in MyFontList)
        {
            ...
            // "Set WFH child"
            wfb1.Font = new SD.Font("Arial", 15, SD.FontStyle.Italic);
            wfb3.Font = new SD.Font("Impact", 15, SD.FontStyle.Italic | SD.FontStyle.Bold);
            MyPause();
            ...
            VerifyFontInHost(p, sr, wfh1, item);
            VerifyFontInHost(p, sr, wfh2, item);
            VerifyFontInWFControl(p, sr, wfb1, expFont1);
            VerifyFontInWFControl(p, sr, wfb2, item);
            VerifyFontInWFControl(p, sr, wfb3, expFont3);
        }
        return sr;
    }
    ...
    public ScenarioResult Scenario6(TParams p)
    {
        ...
        foreach (MyFontItemType item in MyFontList)
        {
            ...
            SetFontOnHost(wfh1,item);
            MyPause();

            // "verify that Fonts get propagated to the WFH and it's controls"
            VerifyFontInHost(p, sr, wfh1, item);
            VerifyFontInHost(p, sr, wfh2, origFont);
            VerifyFontInWFControl(p, sr, wfb1, item);
            VerifyFontInWFControl(p, sr, wfb2, item);
            VerifyFontInWFControl(p, sr, wfb3, origFont);
        }
        return sr;
    }
    ...
    public ScenarioResult Scenario7(TParams p)
    {
        ...
        foreach (MyFontItemType item in MyFontList)
        {
            ...
            SetFontOnHost(wfh2, item);
            MyPause();
            SetFontOnHost(wfh2, item);
            MyPause();

            // "verify that Fonts get propagated to the WFH and it's controls"
            VerifyFontInHost(p, sr, wfh1, origFont);
            VerifyFontInHost(p, sr, wfh2, item);
            VerifyFontInWFControl(p, sr, wfb1, origFont);
            VerifyFontInWFControl(p, sr, wfb2, origFont);
            VerifyFontInWFControl(p, sr, wfb3, item);
        }
        return sr;
    }
    ...
    public ScenarioResult Scenario8(TParams p)
    {
        ...
        foreach (MyFontItemType item in MyFontList)
        {
            ...
            // compare current font of WFH child with what we set it to
            VerifyFontInHost(p, sr, wfh1, item);                // we set this in test
            VerifyFontInHost(p, sr, wfh2, origFont);            // original window font
            VerifyFontInWFControl(p, sr, wfb1, expFont1);       // we set this
            VerifyFontInWFControl(p, sr, wfb2, item);           // should match parent (wfh1)
            VerifyFontInWFControl(p, sr, wfb3, expFont3);       // we set this
        }
        return sr;
    }
    ...
    public ScenarioResult Scenario9(TParams p)
    {
        ...
        foreach (MyFontItemType item in MyFontList)
        {
            ...
            // compare current font of WFH child with what we set it to
            VerifyFontInHost(p, sr, wfh1, item);                // we set this
            VerifyFontInHost(p, sr, wfh2, origFont);            // original window font
            VerifyFontInWFControl(p, sr, wfb1, expFont1);       // we set this
            VerifyFontInWFControl(p, sr, wfb2, item);           // should match parent (wfh1)
            VerifyFontInWFControl(p, sr, wfb3, expFont3);       // we set this
    ...
    private void VerifyFontInHost(TParams p, ScenarioResult sr, WindowsFormsHost wfh, MyFontItemType item)
    {
        ...
        if (debug) { p.log.WriteLine("  Checking font in '{0}'", wfh.Name); }
    ...
    private void VerifyFontInWFControl(TParams p, ScenarioResult sr, SWF.Control ctrl, MyFontItemType item)
    {
        ...
        if (debug) { p.log.WriteLine("  Checking font in '{0}'", ctrl.Name); }
    ...
    private void SetUpControls()
    {
        ...
        wfb1 = new SWF.Button();
        wfb1.Name = "WF Btn 1";
        wfb1.Text = "This is a WinForms Button";
        wfb1.AutoSize = true;
        wfb2 = new SWF.Button();
        wfb2.Name = "WF Btn 2";
        wfb2.Text = "Have a Nice Day!";
        wfb2.AutoSize = true;

        // put buttons in panel
        wfb2.Top = wfb1.Bottom * 2;
