...
namespace WindowsFormsHostTests
{
    public class OtherKeys : WPFReflectBase
    {
        ...
        // class vars
        private string _appTitle = "Other Keys TC";      // so that Mita can find it
        private DockPanel _dp;
        private System.Windows.Forms.VScrollBar _vScr;
        private System.Windows.Forms.HScrollBar _hScr;
        ...
        Utilities.SleepDoEvents(10);
            ...
            // End
            PressKeys("{END}");
            WPFMiscUtils.IncCounters(sr, 91, _vScr.Value, "After {END} Value not as expected", p.log);

            // Home
            PressKeys("{HOME}");
            WPFMiscUtils.IncCounters(sr, 0, _vScr.Value, "After {HOME} Value not as expected", p.log);
            ...
            PressKeys("{PGDN}");
            PressKeys("{PGDN}");
            expVal += 2* _vScr.LargeChange;
            WPFMiscUtils.IncCounters(sr, expVal, _vScr.Value, "After {PGDN} Value not as expected", p.log);

            // PG Up
            PressKeys("{PGUP}");
            expVal -= _vScr.LargeChange;
            WPFMiscUtils.IncCounters(sr, expVal, _vScr.Value, "After {PGUP} Value not as expected", p.log);

            // Arrow Down
            PressKeys("{DOWN}");
            PressKeys("{DOWN}");
            expVal += 2* _vScr.SmallChange;
            WPFMiscUtils.IncCounters(sr, expVal, _vScr.Value, "After {DOWN} Value not as expected", p.log);

            // Arrow Up
            PressKeys("{UP}");
            expVal -= 1;
            WPFMiscUtils.IncCounters(sr, expVal, _vScr.Value, "After {END} Value not as expected", p.log);
            ...
            // End
            PressKeys("{END}");
            WPFMiscUtils.IncCounters(sr, 91, _hScr.Value, "After {END} Value not as expected", p.log);

            // Home
            PressKeys("{HOME}");
            WPFMiscUtils.IncCounters(sr, 0, _hScr.Value, "After {HOME} Value not as expected", p.log);
            ...
            PressKeys("{PGDN}");
            PressKeys("{PGDN}");
            expVal += 2 * _hScr.LargeChange;
            WPFMiscUtils.IncCounters(sr, expVal, _hScr.Value, "After {PGDN} Value not as expected", p.log);

            // PG Up
            PressKeys("{PGUP}");
            expVal -= _hScr.LargeChange;
            WPFMiscUtils.IncCounters(sr, expVal, _hScr.Value, "After {PGUP} Value not as expected", p.log);

            // Arrow Down
            PressKeys("{DOWN}");
            PressKeys("{DOWN}");
            expVal += 2 * _hScr.SmallChange;
            WPFMiscUtils.IncCounters(sr, expVal, _hScr.Value, "After {DOWN} Value not as expected", p.log);

            // Arrow Up
            PressKeys("{UP}");
            expVal -= 1;
            WPFMiscUtils.IncCounters(sr, expVal, _hScr.Value, "After {END} Value not as expected", p.log);
        ...
        public ScenarioResult Scenario3(TParams p)
        {
            ...
            PressKeys("{HOME}");
            PressKeys("{DEL}");
            WPFMiscUtils.IncCounters(sr, "extBox1", _wfTB1.Text, "First char not deleted", p.log);

            // move three chars in, delete char
            PressKeys("{RIGHT 3}");
            PressKeys("{DEL}");
            WPFMiscUtils.IncCounters(sr, "extox1", _wfTB1.Text, "Middle char not deleted", p.log);

            // move to end, try to delete char (should not be able to)
            PressKeys("{END}");
            PressKeys("{DEL}");
            WPFMiscUtils.IncCounters(sr, "extox1", _wfTB1.Text, "String not as expected", p.log);

            // try backspace to delete last char (should be able to)
            PressKeys("{BKSP}");
            WPFMiscUtils.IncCounters(sr, "extox", _wfTB1.Text, "String not as expected", p.log);
        ...
        public ScenarioResult Scenario4(TParams p)
        {
            ...
            _WFEvents = "";
            PressKeys(" ");
            p.log.WriteLine("After: WFEvents = '{0}'", _WFEvents);
            WPFMiscUtils.IncCounters(sr, "wfCB_CheckedChanged:", _WFEvents, "Should be 1 hit", p.log);
            ...
            PressKeys(" ");
            p.log.WriteLine("After: WFEvents = '{0}'", _WFEvents);
            WPFMiscUtils.IncCounters(sr, "wfBtn_Click:", _WFEvents, "Should be 1 hit", p.log);
            ...
            Utilities.SleepDoEvents(5);
            _WFEvents = "";
            p.log.WriteLine("Before: WFEvents = '{0}'", _WFEvents);
            PressKeys(" ");
            p.log.WriteLine("After: WFEvents = '{0}'", _WFEvents);
            WPFMiscUtils.IncCounters(sr, p.log, EventInList("wfRad1_Click:", _WFEvents), "Did not get Click event");
            ...
            PressKeys(" ");
            p.log.WriteLine("After: WFEvents = '{0}'", _WFEvents);
            WPFMiscUtils.IncCounters(sr, p.log, EventInList("wfRad2_Click:", _WFEvents), "Did not get Click event");
        ...
        private void CreateHostWithScroll(TParams p)
        {
            ...
            _vScr = new System.Windows.Forms.VScrollBar();
            _vScr.Name = "vScroll";
            _vScr.TabStop = true;
            ...
            _hScr = new System.Windows.Forms.HScrollBar();
            _hScr.Name = "hScroll";
            _hScr.TabStop = true;
        ...
        private void CreateHostWithTextbox(TParams p, bool bMultiLine)
        {
            ...
            _wfTB1.Name = "wfTB1";
            _wfTB1.Text = "TextBox1";

            if (bMultiLine)
            {
                _wfTB1.Multiline = true;
                _wfTB1.Height = _wfTB1.Height * 4;
                //wfTB1.Text += "\nHave A\nNice Day";
            }

            // add TextBox to Panel
            System.Windows.Forms.Panel panel = new System.Windows.Forms.Panel();
            panel.Controls.Add(_wfTB1);
        ...
        void wfCB_CheckedChanged(object sender, EventArgs e)
        {
            ...
            _WFEvents += (name + "_CheckedChanged:");
        ...
        private int GetCursorPos(TParams p)
        {
            ...
            MyDelegateType MethodGetCursorPos = delegate
            {
                ...
                return _wfTB1.SelectionStart;
            };

            // call method on thread which created TextBox
            int idx = (int)_wfTB1.Invoke(MethodGetCursorPos);
