...
namespace WindowsFormsHostTests
{
    public class Enabled : WPFReflectBase
    {
        ...
        [Scenario("Regress Windows OS Regression_Bug25 - Exception when setting IsEnabled false")]
        public ScenarioResult Scenario5(TParams p)
        {
            ...
            catch (Exception e)
            {
                ...
                p.log.LogKnownBug(BugDb.WindowsOSBugs, 25, "Got exception setting IsEnabled = false");
        ...
        private void DoSingleControlTest(TParams p, ScenarioResult sr, ContainerType contType)
        {
            ...
            {
                if (_debug) { p.log.WriteLine("contFlag = '{0}'", contFlag); }

                // WF Host control
                foreach (bool wfhFlag in new bool[] { true, false })
                {
                    if (_debug) { p.log.WriteLine("\twfhFlag = '{0}'", wfhFlag); }

                    // WF control
                    foreach (bool wfFlag in new bool[] { true, false })
                    {
                        if (_debug) { p.log.WriteLine("\t\twfFlag = '{0}'", wfFlag); }
                        ...
                        if (_debug) { p.log.WriteLine("setting state to  {0} {1} {2}", contFlag, wfhFlag, wfFlag); }
                        ...
                        if (true)
                        {
                            ...
                            {
                                if (_debug) { p.log.WriteLine("Comparing with Enabled"); }
                                //!!!bMatch = BitmapsCloseEnough(bmpEnabled, bmpBtn);
                                bMatch = Utilities.BitmapsIdentical(bmpEnabled, bmpTB);
                            }
                            else
                            {
                                if (_debug) { p.log.WriteLine("Comparing with Disabled"); }
        ...
        private void DoComplexControlTest(TParams p, ScenarioResult sr, ContainerType contType)
        {
            ...
            foreach (bool contFlag in new bool[] { true, false })
            {
                ...
                foreach (bool wfhFlag in new bool[] { true, false })
                {
                    ...
                    {
                        if (_debug) { p.log.WriteLine("\t\tusrFlag = '{0}'", usrFlag); }
        ...
        private void DoContainerControlTest(TParams p, ScenarioResult sr, ContainerType contType)
        {
            ...
            foreach (bool contFlag in new bool[] { true, false })
            {
                ...
                foreach (bool wfhFlag in new bool[] { true, false })
                {
                    ...
                    {
                        if (_debug) { p.log.WriteLine("\t\tpanFlag = '{0}'", panFlag); }
...
//@ Regress Windows OS Regression_Bug25 - Exception when setting IsEnabled false
