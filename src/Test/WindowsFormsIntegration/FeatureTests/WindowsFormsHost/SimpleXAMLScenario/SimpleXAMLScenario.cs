...
public class SimpleXAMLScenario : WPFReflectBase
{
    ...
    // class vars
    private Window loadedWin;           // Avalon window build from Xaml file
    private SWF.Button wfBtn;           // handles to WF controls on window
    private SWF.TextBox wfTB;
    private SWF.MonthCalendar wfCal;
    private bool debug = true;
    private string events;
    private string fName = "MyScenario1.xaml";      // file on current directory
    ...
    private void SetupScenario(TParams p, string scenario)
    {
        ...
        {
            loadedWin = WindowFromXamlFile(fName);
            if (loadedWin != null)
            {
                loadedWin.Show();
                SetUpWindow();
    ...
    private void SetUpWindow()
    {
        ...
        AddAvalonClickHandler(loadedWin);
        AddWinformsClickHandler(loadedWin);
        ...
        StackPanel sp = (StackPanel)loadedWin.FindName("sp");

        // get handles to controls, assign to globals (er, I mean "Class Variables")
        wfBtn = (SWF.Button)loadedWin.FindName("wfBtn1");
        wfTB = (SWF.TextBox)loadedWin.FindName("wfTB1");
        wfCal = (SWF.MonthCalendar)loadedWin.FindName("wfCal");

        // make sure we found the WF controls
        if (wfBtn == null) { throw new ArgumentNullException("wfBtn"); }
        if (wfTB == null) { throw new ArgumentNullException("wfTB"); }
        if (wfCal == null) { throw new ArgumentNullException("wfCal"); }
    }
    ...
    public ScenarioResult Scenario1(TParams p)
    {
        ...
        SD.Bitmap bmpBtn = GetWinformBitmap(wfBtn);
        SD.Bitmap bmpTB = GetWinformBitmap(wfTB);
        SD.Bitmap bmpCal = GetWinformBitmap(wfCal);
        if (debug)
        {
        ...
        // Author's note: It used to be the case that the MonthCalendar was the exact same "discreet"
    ...
    public ScenarioResult Scenario2(TParams p)
    {
        ...
        uiAvBtn.Click();
        MyPause();
        p.log.WriteLine("Events string is '{0}'", events);
        WPFMiscUtils.IncCounters(sr, "avBtn:", events, "Did not get expected event", p.log);
        ...
        MyPause();
        p.log.WriteLine("Events string is '{0}'", events);
        WPFMiscUtils.IncCounters(sr, "B1:", events, "Did not get expected event", p.log);
