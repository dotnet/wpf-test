...
public class EventOrdering : WPFReflectBase
{
    ...
    private WindowsFormsHost wfh;                   // our Host control
    private System.Windows.Forms.Button wfBtn;      // our WF control
    private string expHostEvents;                   // expected WFH events
    private string expCtrlEvents;                   // expected WF Control events
    ...
    public ScenarioResult Scenario1(TParams p)
    {
        ...
        expCtrlEvents = "";
        WPFMiscUtils.IncCounters(sr, expCtrlEvents, WFBUtil.GetEvents(), "Did not get correct events", p.log);
        ...
        p.log.WriteLine("");
        expHostEvents = "Initialized:";
        WPFMiscUtils.IncCounters(sr, expHostEvents, WFHUtil.GetEvents(), "Did not get correct events", p.log);
        expCtrlEvents = "";
        WPFMiscUtils.IncCounters(sr, expCtrlEvents, WFBUtil.GetEvents(), "Did not get correct events", p.log);

        // Note about "ChangeUICues" message - Windows OS Bug 1546968 "By Design"
        // This message is received by a Windows Forms Control in a WF app when it is shown for the first time.
        // There is no corresponding message within Avalon for this.  However, the UI Cues are set up before the
        // WF control is shown (see Bug for details) so that's why we don't get this message in this case.
        ...
        p.log.WriteLine("");
        expHostEvents = "IsVisibleChanged:SizeChanged:LayoutUpdated:LayoutUpdated:Loaded:LayoutUpdated:";
        WPFMiscUtils.IncCounters(sr, expHostEvents, WFHUtil.GetEvents(), "Did not get correct events", p.log);
        expCtrlEvents = "HandleCreated:BindingContextChanged:VisibleChanged:Invalidated:Layout:Resize:SizeChanged:ClientSizeChanged:Invalidated:BackColorChanged:VisibleChanged:Paint:";
        WPFMiscUtils.IncCounters(sr, expCtrlEvents, WFBUtil.GetEvents(), "Did not get correct events", p.log);
        ...
        p.log.WriteLine("");
        expHostEvents = "IsVisibleChanged:Unloaded:";
        WPFMiscUtils.IncCounters(sr, expHostEvents, WFHUtil.GetEvents(), "Did not get correct events", p.log);
        expCtrlEvents = "VisibleChanged:";
        WPFMiscUtils.IncCounters(sr, expCtrlEvents, WFBUtil.GetEvents(), "Did not get correct events", p.log);
    ...
    public ScenarioResult Scenario2(TParams p)
    {
        ...
        p.log.WriteLine("");
        expHostEvents = "SizeChanged:LayoutUpdated:LayoutUpdated:";
        WPFMiscUtils.IncCounters(sr, expHostEvents, WFHUtil.GetEvents(), "Did not get correct events", p.log);
        expCtrlEvents = "ParentChanged:FontChanged:Layout:ForeColorChanged:BackColorChanged:HandleCreated:BindingContextChanged:MarginChanged:Invalidated:Layout:Resize:SizeChanged:ClientSizeChanged:Paint:";
        WPFMiscUtils.IncCounters(sr, expCtrlEvents, WFBUtil.GetEvents(), "Did not get correct events", p.log);
        ...
        // Note: VSWhidbey Bug 577659 used to happen here
        // (assigning child to null yields exception)
        wfh.Child = null;
        WPFReflectBase.DoEvents();
        ...
        expHostEvents = "SizeChanged:LayoutUpdated:LayoutUpdated:";
        WPFMiscUtils.IncCounters(sr, expHostEvents, WFHUtil.GetEvents(), "Did not get correct events", p.log);
        expCtrlEvents = "VisibleChanged:VisibleChanged:ParentChanged:Invalidated:FontChanged:Layout:Invalidated:ForeColorChanged:Invalidated:BackColorChanged:BindingContextChanged:";
        WPFMiscUtils.IncCounters(sr, expCtrlEvents, WFBUtil.GetEvents(), "Did not get correct events", p.log);
    ...
    public ScenarioResult Scenario3(TParams p)
    {
        ...
        p.log.WriteLine("Ctrl Events when adding WFH to DockPanel '{0}'", WFBUtil.GetEvents());
        p.log.WriteLine("");
        expHostEvents = "Initialized:IsVisibleChanged:SizeChanged:LayoutUpdated:LayoutUpdated:LayoutUpdated:Loaded:LayoutUpdated:LayoutUpdated:LayoutUpdated:";
        WPFMiscUtils.IncCounters(sr, expHostEvents, WFHUtil.GetEvents(), "Did not get correct events", p.log);
        expCtrlEvents = "HandleCreated:BindingContextChanged:VisibleChanged:Invalidated:Layout:Resize:SizeChanged:ClientSizeChanged:Invalidated:BackColorChanged:VisibleChanged:Paint:";
        WPFMiscUtils.IncCounters(sr, expCtrlEvents, WFBUtil.GetEvents(), "Did not get correct events", p.log);
    ...
    public ScenarioResult Scenario6(TParams p)
    {
        ...
        p.log.WriteLine("Ctrl Events when setting maximized '{0}'", WFBUtil.GetEvents());
        p.log.WriteLine("");
        expHostEvents = "SizeChanged:LayoutUpdated:LayoutUpdated:LayoutUpdated:LayoutUpdated:LayoutUpdated:LayoutUpdated:";
        WPFMiscUtils.IncCounters(sr, expHostEvents, WFHUtil.GetEvents(), "Did not get correct events", p.log);
        expCtrlEvents = "Invalidated:Layout:Resize:SizeChanged:ClientSizeChanged:Paint:";
        WPFMiscUtils.IncCounters(sr, expCtrlEvents, WFBUtil.GetEvents(), "Did not get correct events", p.log);
        ...
        p.log.WriteLine("Ctrl Events when setting normal '{0}'", WFBUtil.GetEvents());
        p.log.WriteLine("");
        expHostEvents = "LayoutUpdated:LayoutUpdated:LayoutUpdated:LayoutUpdated:LayoutUpdated:LayoutUpdated:";
        WPFMiscUtils.IncCounters(sr, expHostEvents, WFHUtil.GetEvents(), "Did not get correct events", p.log);
        expCtrlEvents = "Paint:";
        WPFMiscUtils.IncCounters(sr, expCtrlEvents, WFBUtil.GetEvents(), "Did not get correct events", p.log);
    ...
    public ScenarioResult Scenario7(TParams p)
    {
        ...
        Canvas.SetLeft(wfh, 100);
        Canvas.SetTop(wfh, 100);
        ...
        p.log.WriteLine("");
        expHostEvents = "LayoutUpdated:LayoutUpdated:LayoutUpdated:LayoutUpdated:LayoutUpdated:LayoutUpdated:LayoutUpdated:";
        WPFMiscUtils.IncCounters(sr, expHostEvents, WFHUtil.GetEvents(), "Did not get correct events", p.log);
        expCtrlEvents = "";
        WPFMiscUtils.IncCounters(sr, expCtrlEvents, WFBUtil.GetEvents(), "Did not get correct events", p.log);
    ...
    public ScenarioResult Scenario9(TParams p)
    {
        ...
        // need to update this part of the TC - a-wboyde !!!
        if (true)
        {
            ...
            // add some controls
            wfhValidate = new WindowsFormsHost();
            System.Windows.Forms.CheckBox checkBox = new System.Windows.Forms.CheckBox();
            checkBox.Checked = true;
            checkBox.Text = "Cancel closing";
            wfhValidate.Child = checkBox;
            w.Content = wfhValidate;
            ...
            ((System.Windows.Forms.ContainerControl)(wfhValidate.Child.Parent)).ActiveControl = checkBox;
            ...
            strEvents = "";
            w.Close();
            p.log.WriteLine("strEvents = '{0}'", strEvents);

            // check events - should get Validating, but not Validated; Closing, but not Closed
            expEvents = "w_Closing:checkBox_Validating:";
            WPFMiscUtils.IncCounters(sr, expEvents, strEvents, "Did not get expected events", p.log);
        ...
        // was playing with AppDomain to try to force Disposed - no such luck
        // remove if not needed !!! a-wboyde
        ...
        // force Garbage collector to run !!! per Andrew
        // another failed attempt - sigh - a-wboyde !!!
        GC.Collect();
        WPFReflectBase.DoEvents();
        GC.Collect(0);
        WPFReflectBase.DoEvents();
    ...
    void w_Closing(object sender, System.ComponentModel.CancelEventArgs e)
    {
        ...
        bool bVal = ((System.Windows.Forms.ContainerControl)wfhValidate.Child.Parent).Validate(true);
    ...
    private void TestBehavior(TParams p, ScenarioResult sr, Operation op)
    {
        ...
        switch (op)
        {
            ...
            case Operation.Resize:
                ...
                p.log.WriteLine("Ctrl Events when resizing window '{0}'", WFBUtil.GetEvents());
                p.log.WriteLine("");
                expHostEvents = "SizeChanged:LayoutUpdated:LayoutUpdated:LayoutUpdated:LayoutUpdated:LayoutUpdated:";
                WPFMiscUtils.IncCounters(sr, expHostEvents, WFHUtil.GetEvents(), "Did not get correct events", p.log);
                expCtrlEvents = "Invalidated:Layout:Resize:SizeChanged:ClientSizeChanged:Paint:";
                WPFMiscUtils.IncCounters(sr, expCtrlEvents, WFBUtil.GetEvents(), "Did not get correct events", p.log);
                break;
    ...
    // !!! This was added as part of creating new AppDomain for Disposed issue
    // can remove if is no longer useful - a-wboyde
    public class Win : Window
    {
        ...
        public Win()
        {
            ...
            avBtn = new Button();
            avBtn.Content = "Status";
            sp.Children.Add(avBtn);
        ...
        {
            avBtn.Content += " x";
            avBtn.Background = System.Windows.Media.Brushes.Yellow;
        }

        void wfBtn_LostFocus(object sender, EventArgs e)
        {
            avBtn.Background = System.Windows.Media.Brushes.Red;
        }

        void wfBtn_GotFocus(object sender, EventArgs e)
        {
            avBtn.Background = System.Windows.Media.Brushes.Green;
        }
    }
}
}
...
//@ WindowsOS Bug 1547402 - Verify Winforms Controls can still get Validating/Validated messages