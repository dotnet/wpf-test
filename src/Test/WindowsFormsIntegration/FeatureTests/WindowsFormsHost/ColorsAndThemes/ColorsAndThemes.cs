...
namespace WindowsFormsHostTests
{
    public class ColorsAndThemes : WPFReflectBase
    {
        ...
        // open issues !!!
        // Regression_Bug18 - WindowsFormsHost -- background color on a WF button (within a System.Windows.Controls.Page object) change after restore from minimize (fixed 6228?)
        // Regression_Bug19 - WindowsFormsHost:  Background transparency not working (active)
        ...
        private static RadialGradientBrush s_brRadGreenYellow = new RadialGradientBrush(Colors.Green, Colors.Yellow);
        private static RadialGradientBrush s_brRadBlueRed = new RadialGradientBrush(Colors.Blue, Colors.Red);
        ...
        private static MyColorMapType[] s_myColorList = {
            new MyColorMapType("Solid Yellow", myBrushType.Solid, s_brYellow, SD.Color.Yellow),
            new MyColorMapType("Solid Red", myBrushType.Solid, s_brRed, SD.Color.Red),
            new MyColorMapType("Solid Blue", myBrushType.Solid, s_brBlue, SD.Color.Blue),
            new MyColorMapType("Solid Green", myBrushType.Solid, s_brGreen, SD.Color.Green),
            new MyColorMapType("Linear Yellow/Blue", myBrushType.LinearYB, s_brLinYellowBlue, SD.Color.Yellow),
            new MyColorMapType("Linear Red/Green", myBrushType.LinearRG, s_brLinRedGreen, SD.Color.Red),
            new MyColorMapType("Radial Green/Yellow", myBrushType.RadialGY, s_brRadGreenYellow, SD.Color.Green),
            new MyColorMapType("Radial Blue/Red", myBrushType.RadialBR, s_brRadBlueRed, SD.Color.Blue),
        };
        ...
        public ScenarioResult Scenario1(TParams p)
        {
            ...
            // loop through colors
            foreach (MyColorMapType item in s_myColorList)
            {
                p.log.WriteLine("Testing '{0}'", item.testDesc);

                // "Set forecolor of the WFH"
                _wfh1.Foreground = item.avBrush;
                _wfh2.Foreground = item.avBrush;
                MyPause();

                // "verify that the child get's it"
                VerifyControlForeColor(p, sr, _wfb1, item.wfColor);
                VerifyControlForeColor(p, sr, _wfb2, item.wfColor);
                VerifyControlForeColor(p, sr, _wfb3, item.wfColor);
            }
            return sr;
        }
        ...
        public ScenarioResult Scenario2(TParams p)
        {
            ...
            foreach (MyColorMapType item in s_myColorList)
            {
                ...
                _wfh1.Background = item.avBrush;
                _wfh2.Background = item.avBrush;
                ...
                {
                    VerifyControlBackColor(p, sr, _wfb1, item.wfColor, item.brushType);
                    VerifyControlBackColor(p, sr, _wfb2, item.wfColor, item.brushType);
                }
                VerifyControlBackColor(p, sr, _wfb3, item.wfColor, item.brushType);
            }
            return sr;
        }
        ...
        public ScenarioResult Scenario3(TParams p)
        {
            ...
            foreach (MyColorMapType item in s_myColorList)
            {
                ...
                _sp.Background = item.avBrush;
                MyPause();

                // force repaint - see Windows OS Regression_Bug20 !!!
                ...
                WPFMiscUtils.IncCounters(sr, null, _wfh1.Background, "WFH1 not null", p.log);
                WPFMiscUtils.IncCounters(sr, null, _wfh2.Background, "WFH2 not null", p.log);
...
*/
        ...
        public ScenarioResult Scenario4(TParams p)
        {
            ...
            foreach (MyColorMapType item in s_myColorList)
            {
                ...
                WPFMiscUtils.IncCounters(sr, item.avBrush, _wfh1.Foreground, "WFH1 not correct ForeColor", p.log);
                WPFMiscUtils.IncCounters(sr, item.avBrush, _wfh2.Foreground, "WFH2 not correct ForeColor", p.log);
                VerifyControlForeColor(p, sr, _wfb1, item.wfColor);
                VerifyControlForeColor(p, sr, _wfb2, item.wfColor);
                VerifyControlForeColor(p, sr, _wfb3, item.wfColor);
            }
        ...
        [Scenario("Regression_Bug19 - Set WFH background transparency and verify it works as expected.")]
        public ScenarioResult Scenario5(TParams p)
        {
            ...
            _sp.Background = s_brBlue;

            // "Set WFH background transparency"
            _wfh1.Background = Brushes.Transparent;
            _wfh2.Background = Brushes.Transparent;
        ...
        private void TestOpacity(TParams p, ScenarioResult sr, double op, SD.Color expClr, double expPer)
        {
            ...
            _wfh1.PropertyMap.Apply("Background");
            _wfh2.PropertyMap.Apply("Background");
            MyPause();

            // get bitmap of control
            this.Topmost = true;
            SD.Bitmap bmpCtrl = Utilities.GetBitmapOfControl(_wfb3);
            this.Topmost = false;
...
//@ Regression_Bug19 - Set WFH background transparency and verify it works as expected.
