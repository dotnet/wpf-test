...
namespace WindowsFormsHostTests
{
    public class PropertyMappings : WPFReflectBase
    {
        ...
        private static Button s_avBtn;                        // our Avalon button
        private static WindowsFormsHost s_wfh;                // our Windows Forms Host control
        private static System.Windows.Forms.Button s_wfBtn;   // our Windows Forms button

        // (these all have to be declared static because they are used in array initializers)
        // define some color brushes to play with
        private static SolidColorBrush s_brBlue = Brushes.Blue;
        private static SolidColorBrush s_brRed = Brushes.Red;
        private static SolidColorBrush s_brGreen = Brushes.Green;
        private static SolidColorBrush s_brYellow = Brushes.Yellow;

        // define some gradient brushes
        private static LinearGradientBrush s_brLinYellowBlue = new LinearGradientBrush(Colors.Yellow, Colors.Blue, 90);
        private static LinearGradientBrush s_brLinRedGreen = new LinearGradientBrush(Colors.Red, Colors.Green, 0);
        private static RadialGradientBrush s_brRadYellowBlue = new RadialGradientBrush(Colors.Yellow, Colors.Blue);
        private static RadialGradientBrush s_brRadRedGreen = new RadialGradientBrush(Colors.Red, Colors.Green);
        ...
        private static string s_bitmapPath = "beany.bmp";
        private static BitmapImage s_imgBeany = new BitmapImage(new Uri(s_bitmapPath, UriKind.Relative));
        private static ImageBrush s_brImgBeany = new ImageBrush(s_imgBeany);
        ...
        // this function is only called from ScenarioRegression_Bug32()
        private void Regression_Bug32(TParams p)
        {
            ...
            s_avBtn.FontFamily = testFont;
            s_avBtn.Content = str;

            try
            {
                // set Font to WFH
                s_wfh.FontFamily = testFont;
            }
        ...
        private void TestMappedProperty(ScenarioResult sr, TParams p, string prop, VerifyMappingDelegate VerFunc)
        {
            ...
            bool bExists = s_wfh.PropertyMap.Contains(prop);
            ...
            if (VerFunc != null) { VerFunc(sr, p, s_wfh, true); }

            // verify removing a property map works (Scenario 2)
            PropertyTranslator pt = GetPropertyTranslator(s_wfh, prop);
            s_wfh.PropertyMap.Remove(prop);
        ...
        /// <summary>
        /// Helper function used to bypass 
        ...
        private static MyBackgroundType[] s_myBackgroundList = {
            new MyBackgroundType("Solid Blue", s_brBlue, System.Drawing.Color.Blue, myBrushType.Solid),
            new MyBackgroundType("Solid Red", s_brRed, System.Drawing.Color.Red, myBrushType.Solid),
            new MyBackgroundType("Solid Green", s_brGreen, System.Drawing.Color.Green, myBrushType.Solid),
            new MyBackgroundType("Yellow/Blue linear", s_brLinYellowBlue, System.Drawing.Color.Empty, myBrushType.LinearYB),
            new MyBackgroundType("Red/Green linear", s_brLinRedGreen, System.Drawing.Color.Empty, myBrushType.LinearRG),
            new MyBackgroundType("Yellow/Blue radial", s_brRadYellowBlue, System.Drawing.Color.Empty, myBrushType.RadialYB),
            new MyBackgroundType("Red/Green radial", s_brRadRedGreen, System.Drawing.Color.Empty, myBrushType.RadialRG),
            new MyBackgroundType("Bitmap image", s_brImgBeany, System.Drawing.Color.Empty, myBrushType.Bitmap),
        };
        ...
        private static void VerifyMappingBackground(ScenarioResult sr, TParams p, WindowsFormsHost wfh, bool bIsMapped)
        {
            ...
            System.Windows.Media.Brush origBackBtn = s_avBtn.Background;
            System.Drawing.Color origColor = s_wfBtn.BackColor;
            System.Drawing.Image origImage = s_wfBtn.BackgroundImage;
            ...
            foreach (MyBackgroundType bt in s_myBackgroundList)
            {
                p.log.WriteLine("Checking '{0}'", bt.testDesc.ToString());

                // set property in WFH
                wfh.Background = bt.avalonBack;
                s_avBtn.Background = bt.avalonBack;
                MyPause();

                // check settings in WF control
                System.Drawing.Color wfColor = s_wfBtn.BackColor;
                System.Drawing.Image wfImage = s_wfBtn.BackgroundImage;
                if (bIsMapped)
                {
                    ...
                    if (bt.winformsColor != System.Drawing.Color.Empty)
                    {
                        ...
                        if (bt.winformsColor.ToArgb() != wfColor.ToArgb())
                        {
                            ...
                            p.log.WriteLine(msg);
                            //p.log.LogKnownBug(BugDb.VSWhidbey, 34, msg);
                            p.log.LogKnownBug(BugDb.WindowsOSBugs, 19, msg);
                        }
                    }
            ...
            s_avBtn.Background = origBackBtn;
        origImage = s_wfBtn.BackgroundImage;
            MyPause();

            // verify WF control follows suit
            // BackColor
            // Note: VSWhidbey Regression_Bug35 / WindowsOS Regression_Bug36 used to happen here
            WPFMiscUtils.IncCounters(sr, origColor.ToArgb(), s_wfBtn.BackColor.ToArgb(),
                "BackColor values don't match (restoring original)", p.log);

            // BackgroundImage
            WPFMiscUtils.IncCounters(sr, origImage, s_wfBtn.BackgroundImage,
        ...
        private static bool VerifyBackground(TParams p, myBrushType brushType)
        {
            ...
            System.Drawing.Rectangle wfRect = new System.Drawing.Rectangle(2, 2,
                s_wfBtn.ClientRectangle.Width - 4, s_wfBtn.ClientRectangle.Height - 4);

            // convert to screen coordinated, get bitmap of WF control
            System.Drawing.Rectangle wfRectSc = s_wfBtn.RectangleToScreen(wfRect);
        ...
        private static MyCursorType[] s_myCursorList = {
        ...
        private static void VerifyMappingCursor(ScenarioResult sr, TParams p, WindowsFormsHost wfh, bool bIsMapped)
        {
            ...
            System.Windows.Forms.Cursor origCur2 = s_wfBtn.Cursor;

            foreach (MyCursorType curType in s_myCursorList)
            {
                p.log.WriteLine("Checking '{0}'", curType.avalon.ToString());

                // set (Avalon) property in WFH
                wfh.Cursor = curType.avalon;
                s_avBtn.Cursor = curType.avalon;
                MyPause();

                // check setting in WF control (WinForms)
                System.Windows.Forms.Cursor actCur2 = s_wfBtn.Cursor;
                if (bIsMapped)
                {
                    // mapping, so should have changed
            ...
            // restore original value
            wfh.Cursor = origCur;
            s_avBtn.Cursor = origCur;
            MyPause();

            // verify WF control follows suit
            // Note: VSWhidbey Regression_Bug37 used to happen here
            WPFMiscUtils.IncCounters(sr, origCur2, s_wfBtn.Cursor,
        ...
        private static void VerifyMappingFlowDirection(ScenarioResult sr, TParams p, WindowsFormsHost wfh, bool bIsMapped)
        {
            ...
            System.Windows.Forms.RightToLeft origChild = s_wfBtn.RightToLeft;
            ...
            foreach (FlowDirection testVal in Enum.GetValues(typeof(FlowDirection)))
            {
                ...
                s_avBtn.FlowDirection = testVal;
                MyPause();

                // check setting in WF control
                System.Windows.Forms.RightToLeft valChild = s_wfBtn.RightToLeft;
                if (bIsMapped)
                {
                    // mapping, so should have changed
            ...
            s_avBtn.FlowDirection = orig;
            MyPause();

            // verify WF control follows suit
            WPFMiscUtils.IncCounters(sr, origChild, s_wfBtn.RightToLeft,
        ...
        checkedFonts.Add("Wingdings");
            ...
            System.Drawing.Font origChildFamily = s_wfBtn.Font;
            ...
            if (!checkedFonts.Contains(curFont.Source))
                ...
                s_avBtn.FontFamily = curFont;
                WPFReflectBase.DoEvents();

                // check setting in WF control
                System.Drawing.Font valChild = s_wfBtn.Font;
                if (bIsMapped)
                {
                    // mapping, so should have changed
                    ...
                    // Note: VSWhidbey Regression_Bug38 used to happen here
            ...
            s_avBtn.FontFamily = origHostFamily;
            WPFReflectBase.DoEvents();

            // verify WF control follows suit
            // Note: VSWhidbey Regression_Bug39 / WindowsOSBugs Regression_Bug41 used to happen here
            WPFMiscUtils.IncCounters(sr, origChildFamily, s_wfBtn.Font,
        ...
        private static void VerifyMappingFontSize(ScenarioResult sr, TParams p, WindowsFormsHost wfh, bool bIsMapped)
        {
            ...
            float origChildVal = s_wfBtn.Font.Size;
            ...
            for (double d = 2; d<60; d+=2)
            {
                ...
                // check setting in WF control
                float vChild = s_wfBtn.Font.Size;
                if (bIsMapped)
                {
                    // unit should be point
                    System.Drawing.GraphicsUnit unit = s_wfBtn.Font.Unit;
                    WPFMiscUtils.IncCounters(sr, s_wfBtn.Font.Unit, System.Drawing.GraphicsUnit.Point,
            ...
            s_avBtn.FontSize = origHostVal;
            MyPause();

            // verify WF control follows suit
            WPFMiscUtils.IncCounters(sr, origChildVal, s_wfBtn.Font.Size,
        ...
        // this is a list of all defined "input" values and their corresponsing "output" values
        // This list is derived from the "WindowsFormsHost Property Mapping" spec
        private static MyFontStyleType[] s_myFontStyleList = {
        ...
        private static void VerifyMappingFontStyle(ScenarioResult sr, TParams p, WindowsFormsHost wfh, bool bIsMapped)
        {
            ...
            System.Drawing.Font origChildStyle = s_wfBtn.Font;

            // loop through each available font style
            foreach (MyFontStyleType curStyle in s_myFontStyleList)
            {
                p.log.WriteLine("Checking '{0}'", curStyle.avalon.ToString());

                // set property in WFH
                wfh.FontStyle = curStyle.avalon;
                s_avBtn.FontStyle = curStyle.avalon;
                MyPause();
            ...
            s_avBtn.FontStyle = origHostStyle;
            MyPause();

            // verify WF control follows suit
            // Note: VSWhidbey Regression_Bug39 / WindowsOSBugs Regression_Bug41 used to happen here
            WPFMiscUtils.IncCounters(sr, origChildStyle.Style, s_wfBtn.Font.Style,
        ...
        // this is a list of all defined "input" values and their corresponsing "output" values
        // This list is derived from the "WindowsFormsHost Property Mapping" spec
        private static MyFontWeightType[] s_myFontWeightList = {
        ...
        private static void VerifyMappingFontWeight(ScenarioResult sr, TParams p, WindowsFormsHost wfh, bool bIsMapped)
        {
            ...
            System.Drawing.Font origChildWeight = s_wfBtn.Font;

            // loop through each available font !!!
            //foreach (bool testVal in new bool[] { true, false, true })
            foreach (MyFontWeightType curWeight in s_myFontWeightList)
            {
                p.log.WriteLine("Checking '{0}'", curWeight.avalon.ToString());

                // set property in WFH
                wfh.FontWeight = curWeight.avalon;
                s_avBtn.FontWeight = curWeight.avalon;
                MyPause();
                ...
                {
                    // mapping, so should have changed
                    // Note: VSWhidbey Regression_Bug40 used to happen here
            ...
            s_avBtn.FontWeight = origHostWeight;
            MyPause();

            // verify WF control follows suit
            // Note: VSWhidbey Regression_Bug39 / WindowsOSBugs Regression_Bug41 used to happen here
            WPFMiscUtils.IncCounters(sr, origChildWeight.Bold, s_wfBtn.Font.Bold,
        ...
        // this is a list of all defined "input" values and their corresponsing "output" values
        private static MyForceCursorType[] s_myForceCursorList = {
        ...
        private static void VerifyMappingForceCursor(ScenarioResult sr, TParams p, WindowsFormsHost wfh, bool bIsMapped)
        {
            ...
            foreach (MyForceCursorType curTest in s_myForceCursorList)
            {
                p.log.WriteLine("Checking '{0}'", curTest.testDesc);

                // save current state "before" each test
                bool origUseWaitCursor = s_wfBtn.UseWaitCursor;
                System.Windows.Forms.Cursor origWFcursor = s_wfBtn.Cursor;
                LogCursorStats(p);
                ...
                p.log.WriteLine("  wfBtn.UseWaitCursor is '{0}'", s_wfBtn.UseWaitCursor.ToString());
                p.log.WriteLine("  wfBtn.Cursor is '{0}'", s_wfBtn.Cursor.ToString());
                ...
                if (bIsMapped)
                {
                    ...
                    WPFMiscUtils.IncCounters(sr, curTest.expUseWaitCursor, s_wfBtn.UseWaitCursor,
                        "UseWaitCursor values not as expected (mapping)", p.log);

                    // debug !!!
                    if (curTest.expUseWaitCursor != s_wfBtn.UseWaitCursor)
                    {
                        p.log.LogKnownBug(BugDb.VSWhidbey, 42, "ForceCursor issues");
                    ...
                    if (curTest.expCursor != null)
                    {
                        ...
                        WPFMiscUtils.IncCounters(sr, curTest.expCursor, s_wfBtn.Cursor,
                            "WF cursor not as expected (mapping)", p.log);

                        // debug !!!
                        if (curTest.expCursor != s_wfBtn.Cursor)
                        {
                            p.log.LogKnownBug(BugDb.VSWhidbey, 42, "ForceCursor issues");
                ...
                else
                {
                    ...
                    WPFMiscUtils.IncCounters(sr, origUseWaitCursor, s_wfBtn.UseWaitCursor,
        ...
        // this is a list of defined (Avalon) "input" values and their corresponding (WinForms) "output" values
        private static MyForegroundType[] s_myForegroundList = {
            new MyForegroundType("Blue", s_brBlue, System.Drawing.Color.Blue),
            new MyForegroundType("Red", s_brRed, System.Drawing.Color.Red),
            new MyForegroundType("Green", s_brGreen, System.Drawing.Color.Green),
            new MyForegroundType("Yellow/Blue linear", s_brLinYellowBlue, System.Drawing.Color.Yellow),
            new MyForegroundType("Red/Green linear", s_brLinRedGreen, System.Drawing.Color.Red),
            new MyForegroundType("Yellow/Blue radial", s_brRadYellowBlue, System.Drawing.Color.Yellow),
            new MyForegroundType("Red/Green radial", s_brRadRedGreen, System.Drawing.Color.Red),
        };
        ...
        private static void VerifyMappingForeground(ScenarioResult sr, TParams p, WindowsFormsHost wfh, bool bIsMapped)
        {
            ...
            System.Windows.Media.Brush origForeBtn = s_avBtn.Foreground;
            System.Drawing.Color origColor = s_wfBtn.ForeColor;
            ...
            // loop through test sets
            foreach (MyForegroundType bt in s_myForegroundList)
            {
                p.log.WriteLine("Checking '{0}'", bt.testDesc.ToString());

                // set property in WFH
                wfh.Foreground = bt.avalonFore;
                s_avBtn.Foreground = bt.avalonFore;
                MyPause();

                // check settings in WF control
                System.Drawing.Color wfColor = s_wfBtn.ForeColor;
                if (bIsMapped)
                {
                    // mapping, so should have changed
            ...
            s_avBtn.Foreground = origForeBtn;
            MyPause();

            // verify hosted WF control follows suit
            WPFMiscUtils.IncCounters(sr, s_wfBtn.ForeColor.ToArgb(), origColor.ToArgb(),
                "ForeColor values don't match (restoring original)", p.log);
            // debug !!!
            if (s_wfBtn.ForeColor.ToArgb() != origColor.ToArgb())
            {
                //MessageBox.Show(string.Format("ForeColor values don't match A={0} E={1}",
        ...
        private static void VerifyMappingIsEnabled(ScenarioResult sr, TParams p, WindowsFormsHost wfh, bool bIsMapped)
        {
            ...
            bool origChildEnabled = s_wfBtn.Enabled;
            ...
            foreach (bool testVal in new bool[] { true, false, true })
            {
                ...
                {
                    // mapping, so should have changed
                    WPFMiscUtils.IncCounters(sr, testVal, s_wfBtn.Enabled,
                        "IsEnabled values don't match (mapping)", p.log);
                }
                else
                {
                    // not mapping, so should remain as original
                    WPFMiscUtils.IncCounters(sr, origChildEnabled, s_wfBtn.Enabled,
            ...
            s_avBtn.IsEnabled = origIsEnabled;
            MyPause();

            // verify WF control follows suit
            WPFMiscUtils.IncCounters(sr, origIsEnabled, s_wfBtn.Enabled,
        ...
        private static void VerifyMappingVisibility(ScenarioResult sr, TParams p, WindowsFormsHost wfh, bool bIsMapped)
        {
            ...
            bool origChildVisible = s_wfBtn.Visible;
            ...
            foreach (Visibility testVal in new Visibility[] { Visibility.Collapsed, Visibility.Hidden, Visibility.Visible, Visibility.Hidden })
            {
                ...
                if (bIsMapped)
                {
                    ...
                    WPFMiscUtils.IncCounters(sr, expVal, s_wfBtn.Visible,
                        "Visible values don't match (mapping)", p.log);
                }
                else
                {
                    // not mapping, so should remain as original
                    WPFMiscUtils.IncCounters(sr, origChildVisible, s_wfBtn.Visible,
            ...
            s_avBtn.Visibility = origVisibility;
            MyPause();
