...
public class DCSCSizing : WPFReflectBase
{
    ...
    private SWF.MonthCalendar wfCal;
    private int wfCalUnitWidth;         // screen width of calendar for single month
    private int wfCalUnitHeight;        // screen height of calendar for single month
    ...
    protected override void InitTest(TParams p)
    {
        ...
        wfCalUnitWidth = mon.Width;
        wfCalUnitHeight = mon.Height;
    ...
    public ScenarioResult Scenario1(TParams p)
    {
        ...
        int origBtnHeight = wfBtn.Height;
        ...
        for (int i = 0; i < 10; i++)
        {
            ...
            WPFMiscUtils.IncCounters(sr, (int)Monitor.ConvertScreenToLogical(Dimension.Width, wfBtn.Width), (int)sp.ActualWidth, "WF Button and Panel not same Width", p.log);
            WPFMiscUtils.IncCounters(sr, wfBtn.Height, origBtnHeight, "WF Button Height should be unchanged", p.log);
        }
    ...
    public ScenarioResult Scenario2(TParams p)
    {
        ...
        p.log.WriteLine("Autosize defaults to '{0}'", wfBtn.AutoSize);
        WPFMiscUtils.IncCounters(sr, false, wfBtn.AutoSize, "Button.AutoSize should default to false", p.log);

        // save current sizes
        SD.Size origBtnSize = wfBtn.Size;
        double origPanelWidth = sp.ActualWidth;
        double origPanelHeight = sp.ActualHeight;

        // play with padding
        int padDelta = 10;
        wfh1.Padding = new Thickness(padDelta);
        MyPause();
        ...
        WPFMiscUtils.IncCounters(sr, origBtnSize.Width + 2 * padDelta, wfBtn.Size.Width, "Width not correct", p.log);
        WPFMiscUtils.IncCounters(sr, origBtnSize.Height + 2 * padDelta, wfBtn.Size.Height, "Height not correct", p.log);
        WPFMiscUtils.IncCounters(sr, (int)origPanelWidth + (int)Monitor.ConvertScreenToLogical(Dimension.Width, 2 * padDelta), (int)sp.ActualWidth, "Panel Width not correct", p.log);
        WPFMiscUtils.IncCounters(sr, (int)origPanelHeight + (int)Monitor.ConvertScreenToLogical(Dimension.Height, 2 * padDelta), (int)sp.ActualHeight, "Panel Height not correct", p.log);
        CheckForUnderpants(p, sr);

        // reset padding (need AutoSizeMode.GrowAndShrink)
        wfh1.Padding = new Thickness(0);
        MyPause();
        LogSizes(p, ControlType.Button, "after reset padding");

        // play with margins
        int marDelta = 25;
        wfh1.Margin = new Thickness(marDelta);
        MyPause();
        LogSizes(p, ControlType.Button, "after set margins");

        // button size should remain the same
        WPFMiscUtils.IncCounters(sr, origBtnSize.Width, wfBtn.Size.Width, "Width not correct", p.log);
        WPFMiscUtils.IncCounters(sr, origBtnSize.Height - (int)Monitor.ConvertLogicalToScreen(Dimension.Height, 0.9), wfBtn.Size.Height, "Height not correct", p.log);

        // panel size should have increased by twice the margin (both sides)
        WPFMiscUtils.IncCounters(sr, origPanelWidth + 2 * marDelta, sp.ActualWidth, "Panel Width not correct", p.log);
        WPFMiscUtils.IncCounters(sr, (int)origPanelHeight + 2 * marDelta, (int)sp.ActualHeight, "Panel Height not correct", p.log);
        CheckForUnderpants(p, sr);
    ...
    public ScenarioResult Scenario3(TParams p)
    {
        ...
        int origWidth = wfCal.Width;
        int origHeight = wfCal.Height;
        ...
        for (int i = 0; i < 10; i++)
        {
            ...
            // (scoberry says its not a bug - any questions, ask him - but don't make him cry) :-)
            //int avCalUnitWidth = wfCalUnitWidth - 7;
            //SD.Size calDim = wfCal.CalendarDimensions;
            //p.log.WriteLine("avCalUnitWidth: {0}, wfCalUnitHeight: {1}", avCalUnitWidth, wfCalUnitHeight);
            //int expCalWidth = avCalUnitWidth * calDim.Width + (calDim.Width - 1) * 4;
            //int expCalHeight = wfCalUnitHeight * calDim.Height + (calDim.Height - 1) * 4;
            ...
            p.log.WriteLine("StackPanel = {0} x {1}", sp.ActualWidth, sp.ActualHeight);
            p.log.WriteLine("Calendar = {0} x {1}", wfCal.Width, wfCal.Height);

            // Note: WindowsOSBugs 1553099 "MonthCalendar wider than panel" used to happen here
            // Bug was resolved as "By Design"
            ...
            if (wfCal.Width > (int)sp.ActualWidth)
            {
                p.log.WriteLine("-- MonthCalendar is wider than panel, but that's ok - WindowsOSBugs 1553099");
                p.log.WriteLine("-- wfCal.Width = '{0}' sp.ActualWidth = '{1}'", wfCal.Width, sp.ActualWidth);
                //p.log.LogKnownBug(BugDb.WindowsOSBugs, 1553099, "MonthCalendar wider than panel");
            }

            // calendar should be expected size (this may be testing too much?)
            WPFMiscUtils.IncCounters(sr, expCalWidth, wfCal.Width, "Calendar not correct width", p.log);
            WPFMiscUtils.IncCounters(sr, expCalHeight, wfCal.Height, "Calendar not correct Height", p.log);
    ...
    public ScenarioResult Scenario4(TParams p)
    {
        ...
        // Note: AutoSize defaults to false
        p.log.WriteLine("Autosize defaults to '{0}'", wfCal.AutoSize);
        WPFMiscUtils.IncCounters(sr, false, wfCal.AutoSize, "MonthCalendar.AutoSize should default to false", p.log);

        // save current sizes
        SD.Size origCalSize = wfCal.Size;
        double origPanelWidth = sp.ActualWidth;
        double origPanelHeight = sp.ActualHeight;
        ...
        WPFMiscUtils.IncCounters(sr, origCalSize.Width, wfCal.Size.Width, "Width not correct", p.log);
        WPFMiscUtils.IncCounters(sr, origCalSize.Height, wfCal.Size.Height, "Height not correct", p.log);
        WPFMiscUtils.IncCounters(sr, origPanelWidth, sp.ActualWidth, "Panel Width not correct", p.log);
        WPFMiscUtils.IncCounters(sr, origPanelHeight, sp.ActualHeight, "Panel Height not correct", p.log);
        CheckForUnderpants(p, sr);
    ...
    public ScenarioResult Scenario5(TParams p)
    {
        ...
        foreach (SWF.DockStyle ds in Enum.GetValues(typeof(SWF.DockStyle)))
        {
            ...
            SD.Bitmap bmpCur = Utilities.GetBitmapOfControl(wfBtn, true);
            // debug !!!
    ...
    private void CreateTestControls(ControlType ctrl)
    {
        ...
        if (ctrl == ControlType.Button)
        {
            ...
            wfBtn.Text = "Winforms Test Button";
            wfh1.Child = wfBtn;

            // set colors so can see host background
            wfBtn.Parent.Paint += new System.Windows.Forms.PaintEventHandler(Parent_Paint);
            wfBtn.BackColor = SD.Color.LightBlue;
        }
        else if (ctrl == ControlType.Calendar)
        {
            // create WF Calendar
            wfCal = new System.Windows.Forms.MonthCalendar();
            wfh1.Child = wfCal;

            // set colors so can see host background
            wfCal.Parent.Paint += new System.Windows.Forms.PaintEventHandler(Parent_Paint);
        }
        else
        {
    ...
    private void LogSizes(TParams p, ControlType ctrl, string desc)
    {
        if (ctrl == ControlType.Button)
        {
            p.log.WriteLine("Button = ({0},{1}) Host = ({2},{3}) Panel = ({4},{5}) - {6}",
                wfBtn.Size.Width, wfBtn.Size.Height,
                wfh1.ActualWidth, wfh1.ActualHeight,
                sp.ActualWidth, sp.ActualHeight, desc);
        }
        else if (ctrl == ControlType.Calendar)
        {
            p.log.WriteLine("Calendar = ({0},{1}) Host = ({2},{3}) Panel = ({4},{5}) - {6}",
                wfCal.Size.Width, wfCal.Size.Height,
                wfh1.ActualWidth, wfh1.ActualHeight,
                sp.ActualWidth, sp.ActualHeight, desc);
        }
    }
    ...
    public class Win : Window
    {
        ...
        public Win()
        {
            ...
            avButton.Click += delegate { host.Child = new SWF.MonthCalendar(); };
            stackPanel.Children.Add(avButton);
