...
namespace Microsoft.Test.Xaml.Types.XamlTemplate
{
    ...
    public interface IXamlTemplateInterface<T> : IXamlNodeContainer
    {
        T Evaluate();
        T Evaluate(object context);
    }

    public class XamlTemplateInterfaceImpl<T> : IXamlTemplateInterface<T>
    {
        private readonly XamlNodeList _nodes;
        private readonly IXamlObjectWriterFactory _loader;

        public XamlTemplateInterfaceImpl(IXamlObjectWriterFactory loader,
                                         XamlReader reader)
        {
            this._loader = loader;
            if (reader == null)
