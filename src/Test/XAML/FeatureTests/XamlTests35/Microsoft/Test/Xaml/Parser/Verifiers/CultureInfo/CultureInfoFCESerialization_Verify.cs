...
{
    /// <summary/>
    public static class CultureInfoFCESerialization_Verify
    {
        ...
        {
            bool result = true;
            GlobalLog.LogStatus("Inside CultureInfoSerializationVerifier.VerifyFCE()...");

            Bold defaultb = (Bold) LogicalTreeHelper.FindLogicalNode(rootElement, "Default");
            ...
            foreach (string cultureStr in s_allCultures)
            {
                ...
                FrameworkContentElement fce = (FrameworkContentElement) LogicalTreeHelper.FindLogicalNode(rootElement, id);
                VerifyElement.VerifyString(fce.Language.IetfLanguageTag.ToLowerInvariant(), cultureStr.ToLowerInvariant(), ref result);
            }
            return result;
        }
    }
