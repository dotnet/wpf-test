...
//
//  Program:  HitTest tests for CombinedGeometry
//  Author:   Microsoft
//
using System;
...
{
    internal class CombinedGeometryHitTest : HitTestBase
    {
        public CombinedGeometryHitTest( double left, double top, double width, double height)
        ...
        {
            testedGeometry = new CombinedGeometry(GeometryCombineMode.Intersect,
                                                  new RectangleGeometry(new Rect(100, 100, 100, 100)),
                                                  new RectangleGeometry(new Rect(80, 50, 220, 250))
                                                  );
            RunBVT();

            CommonLib.LogTest("CombinedGeometry HitTesting Result:");
        }

        private void RunBVT()
        {
            CommonLib.LogStatus("The hit geometry is in the target CombinedGeometry, so expect True.");
            ...
            CommonLib.LogStatus("The Point(120, 120) is in the CombinedGeometry, so the result should be True");
            TestContains2(new Point(120, 120), true);

            CommonLib.LogStatus("The Poine(1000, 10000) is NOT in the CombinedGeometry, so the result should be False");
            TestContains2(new Point(1000, 10000), false);

            CommonLib.LogStatus("The Poine(120, 210) is NOT in the CombinedGeometry, but it falls on the PenWidth, so the result should be True");
            TestContains3(new Pen(Brushes.Black, 40), new Point(120, 210), true);

            CommonLib.LogStatus("The Poine(-20, 90) is NOT in the CombinedGeometry, nor it falls in the penwidth, so the result should be False");
            TestContains3(new Pen(Brushes.Black, 20), new Point(-20, 90), false);

            CommonLib.LogStatus("The hit geometry is inside of the target CombinedGeometry with Absolute Tolerance, so the result should be True");
            TestContains4(new RectangleGeometry(new Rect(100, 100, 20, 20)), 100, ToleranceType.Absolute, true);

            CommonLib.LogStatus("The hit RectangleGeometry is inside of the target CombinedGeometry with Relative Tolerance, so the result should be True");
            TestContains4(new RectangleGeometry(new Rect(120, 120, 20, 20)), 0, ToleranceType.Relative, true);

            CommonLib.LogStatus("The hit Point(110,200) is inside of the target CombinedGeometry with Absolute Tolerance, so the result should be True");
            TestContains5(new Point(110, 200), 0, ToleranceType.Absolute, true);

            CommonLib.LogStatus("The hit Point(110,200) is inside of the target CombinedGeometry with Relative Tolerance, so the result should be True");
            TestContains5(new Point(110, 200), 0, ToleranceType.Relative, true);

            CommonLib.LogStatus("The hit Point(205,200) is NOT inside of the target CombinedGeometry with Absolute Tolerance, but it is on the PenWidth, so the result should be True");
            ...
            CommonLib.LogStatus("The target CombinedGeometry fully contains the hit Geometry, so the expect IntersectionDetail will be FullyContains");
            ...
            CommonLib.LogStatus("The hitGeometry is fully inside of the CombinedGeometry with Absolute tolerance, so the expect IntersectionDetail will be FullyContains");
            TestContainsWithDetail3(hitGeometry, 0, ToleranceType.Absolute, IntersectionDetail.FullyContains);

            CommonLib.LogStatus("The hitGeometry is fully inside of the CombinedGeometry with Relative tolerance, so the expect IntersectionDetail will be FullyContains");
