...
{
    /******************************************************************************
    * CLASS:          ActionSequenceTestEngine
    ******************************************************************************/
    /// <summary>
    ...
    public class ActionSequenceTestEngine : IHostedTest
    {
        ...
        ******************************************************************************/
        /// <summary>
        /// Look at each "TEST" node for the requested test case; if found, run the test case.
        /// </summary>
        public void RunTestAction()
        {
            ...
            // Get Test Name.
            //
            string testName = DriverState.TestName;


            if (testName == null)
            {
                throw new Microsoft.Test.TestSetupException("'TestName' could not be retrieved.");
            }
            ...
            TestContainer = CommonStorage.Current.Get("TestContainer") as ITestContainer;

            //
            // Get XML TEST node.
            //
            XmlTextReader xmlTextReader = new XmlTextReader("AllInputScenarios.xtc");
            XmlDocument doc = new XmlDocument();
            doc.Load(xmlTextReader);

            XmlNodeList testNodes = doc.DocumentElement.ChildNodes;
            for (int j=0; j<testNodes.Count; j++)
            {
                if (testNodes[j].Name == "TEST")
                {
                    if (FindRequestedTestCase(testNodes[j], testName))
                    {
                        _testNode = testNodes[j];
                        break;
                    }
                }
            }
            ...
            if (_testNode == null)
            {
                throw new Microsoft.Test.TestSetupException("ERROR: The requested 'TEST' node named : " + testName + " was not found.");
            }
        ...
        /// <summary>
        /// Return true if the "Title" attribute of the "TEST" node contains the requested test case;
        /// </summary>
        private bool FindRequestedTestCase(XmlNode testNode, string requestedTest)
        {
            bool result = false;

            for(int i=0; i<testNode.Attributes.Count; i++)
            {
                if (testNode.Attributes.Item(i).Name == "Title")
                {
                    //if (testNode.Attributes.Item(i).Value == requestedTest)
                    //{
                    //    result = true;
                    //}
                    if (requestedTest.Contains(testNode.Attributes.Item(i).Value))
                    {
                        result = true;
                    }
                }
        ...
        ******************************************************************************/
        ...
        private bool _VerifySequences(List<SequenceAction> expectedSequence, List<SequenceAction> actualSequence)
        {
            ...
            if (!same)
            {
                ...
                throw new TestValidationException(failureMsg);
            }
...
#if TARGET_NET3_5
            s_types.AddRange(Utility.GetAssemblyTypes(typeof(UIElement3D).Assembly));
#endif
