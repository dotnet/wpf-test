...
{
    /// <summary>
    /// Verify ModifierKeysConverter ConvertTo method.
    /// </summary>
    /// <description>
    ...
    public class ModifierKeysConverterConvertToApp : TestApp
    {
        /// <summary>
        /// Launch our test.
        /// </summary>
        [TestCase("1", @"CoreInput\TypeConverter", "HwndSource", @"Verify ModifierKeysConverter ConvertTo method in HwndSource.")]
        public static void LaunchTest()
        {
            HostType hostType = (HostType)Enum.Parse(typeof(HostType), DriverState.DriverParameters["TestParameters"]);

            ExeStubContainerFramework exe = new ExeStubContainerFramework(hostType);
            exe.Run(new ModifierKeysConverterConvertToApp(), "Run");
        ...
        public override object DoValidate(object arg)
        {
            ...
            for (int i = 0; i < arrayLength; i++)
            {
                _convertedKeys[i] = (_converter.ConvertTo(_typeDescriptorContext, CultureInfo.InvariantCulture,
                    (ModifierKeys)(KeysAndExpectedStrings[i, 0]), typeof(string)));

                if (_convertedKeys[i] == null)
                {
                    // Not a valid conversion
                    CoreLogger.LogStatus("X " + (ModifierKeys)KeysAndExpectedStrings[i, 0]);
                    nConversionFails++;
                }
                else
                {
                    ...
                    {
                        CoreLogger.LogStatus("Conversion fails: expected Key='" + kExpected + "' (Key='" + k + "')");
                        nConversionFails++;
                    }
                }
            }
