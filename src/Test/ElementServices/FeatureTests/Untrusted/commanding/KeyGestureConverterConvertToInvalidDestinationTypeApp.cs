...
{
    /// <summary>
    /// Verify KeyGestureConverter ConvertTo method for invalid destination type.
    /// </summary>
    /// <description>
    ...
    [TestCaseTitle(@"Verify KeyGestureConverter ConvertTo method for invalid destination type.")]
    [TestCasePriority("2")]
    [TestCaseArea(@"Commanding\TypeConverter")]
    [TestCaseMethod("LaunchTest")]
    [TestCaseDisabled("0")]
    public class KeyGestureConverterConvertToInvalidDestinationTypeApp : TestApp
    {
        /// <summary>
        /// Launch our test.
        /// </summary>
        public static void LaunchTest()
        {
            ...
            TestApp app = new KeyGestureConverterConvertToInvalidDestinationTypeApp();
        ...
        public override object DoValidate(object arg)
        {
            ...
            for (int i = 0; i < arrayLength; i++)
            {
                try
                {
                    _convertedKeys[i] = (_converter.ConvertTo(_typeDescriptorContext, CultureInfo.InvariantCulture,
                        "Shift+F2", DestinationTypes[i]));

                    CoreLogger.LogStatus("O " + DestinationTypes[i].ToString());
                    nConversionPasses++;
                }
                catch (ArgumentNullException)
                {
                    CoreLogger.LogStatus("NS (null): " + DestinationTypes[i]);
                    nConversionFails++;
                }
                catch (NotSupportedException)
                {
                    CoreLogger.LogStatus("NS: " + DestinationTypes[i]);
                    nConversionFails++;
                }
            }
            ...
            bool actual = (nConversionFails == arrayLength);
            bool expected = true;
        ...
        /// </summary>
        private object[] _convertedKeys = new object[DestinationTypes.GetUpperBound(0) + 1];

        /// <summary>
        /// Strings representing invalid destination Type values.
        /// </summary>
        public static Type[] DestinationTypes = new Type[] {
            null,
            typeof(Object),
            typeof(Key),
            typeof(KeyGesture),
