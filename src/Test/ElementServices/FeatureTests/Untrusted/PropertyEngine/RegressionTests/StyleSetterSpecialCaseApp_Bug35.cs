...
{
    /******************************************************************************
    * CLASS:          StyleSetterSpecialCaseApp_Bug35
    ******************************************************************************/
    ...
    [Test(1, "PropertyEngine.RegressionTests", TestCaseSecurityLevel.FullTrust, "DependencyObjectProviderFullTrustTests")]
    public class StyleSetterSpecialCaseApp_Bug35 : AvalonTest
    {
        #region Private Data
        private static string s_testName = "";
        private static bool s_testPassed = false;
        #endregion

        #region Constructor

        [Variation("StyleSetterSpecialCaseApp_Bug35_Style")] 
        [Variation("StyleSetterSpecialCaseApp_Bug35_Factory")]

        /******************************************************************************
        * Function:          StyleSetterSpecialCaseApp_Bug35 Constructor
        ******************************************************************************/
        public StyleSetterSpecialCaseApp_Bug35(string arg)
        {
            s_testName = arg;
        ...
        ******************************************************************************/
        ...
        TestResult StartTest()
        {
            switch (s_testName)
            {
                case "StyleSetterSpecialCaseApp_Bug35_Style":
                    TestStylingHelper_via_Style_Seal();
                    break;
                case "StyleSetterSpecialCaseApp_Bug35_Factory":
                    TestStylingHelper_via_FrameworkElementFactory_SetValue();
                    break;
                default:
        ...
        /******************************************************************************
        * Function:          TestStylingHelper_via_Style_Seal
        ******************************************************************************/
        /// <summary>
        /// Invoke Style.Seal() to target changes that relax constraint of StyleHelper.IsStylingLogicalTree()
        ...
        /// </summary>
        public static void TestStylingHelper_via_Style_Seal()
        {
            ...
            GlobalLog.LogStatus("Test: Setter Seal() for Setter.Property == ToolTipProperty && Setter.Value is ContentElement.");
            styleA1.Seal();
            if (!styleA1.IsSealed)
            {
                s_testPassed = false;
                GlobalLog.LogEvidence("FAIL!: Setter.Seal() for Setter.Property == ToolTipProperty && Setter.Value is ContentElement.");
                return;
            }
            ...
            GlobalLog.LogStatus("Test: Setter.Seal() for Setter.Property == ContextMenuProperty && Setter.Value is ContextMenu, a Visual.");
            styleA2.Seal();
            if (!styleA2.IsSealed)
            {
                s_testPassed = false;
                GlobalLog.LogEvidence("FAIL!: Setter.Seal() for Setter.Property == ContextMenuProperty && Setter.Value is ContextMenu, a Visual.");
                return;
            }
            ...
            GlobalLog.LogStatus("Test: Setter.Seal() for Setter.Property == ItemsPanelProperty && Setter.Value is ItemsPanelTemplate.");
            styleA3.Seal();
            if (!styleA3.IsSealed)
            {
                s_testPassed = false;
                GlobalLog.LogEvidence("FAIL!: Setter.Seal() for Setter.Property == ItemsPanelProperty && Setter.Value is ItemsPanelTemplate.");
                return;
            }
            ...
            // Per Microsoft: test was previously verifying the overly stringent rules we were applying to value
            // set via Setters and started to fail because we relaxed the rules. This relaxation in the rules 
            // were introduced during the XAML parser changes. Specifically it came via this RI from Rob. 
            // I think this does not really count as a regression because this is an intentional relaxation 
            // of the rules. So the test should be changed to account for this.
#if TESTBUILD_CLR20
            GlobalLog.LogStatus("Test: Setter.Seal() throws NotSupportedException for DP == TagProperty && VALUE is ContentElement.");
            ExceptionHelper.ExpectException<NotSupportedException>( delegate() { styleA4.Seal(); },
                                                                    delegate(NotSupportedException e) { ;});
#endif
            s_testPassed = true;
            GlobalLog.LogStatus("Finished test: " + s_testName);
        }

        /******************************************************************************
        * Function:          TestStylingHelper_via_FrameworkElementFactory_SetValue
        ******************************************************************************/
        /// <summary>
        ...
        /// </summary>
        public static void TestStylingHelper_via_FrameworkElementFactory_SetValue()
        {
            GlobalLog.LogStatus("Starting test: " + s_testName);
            ...
            GlobalLog.LogStatus("Test: FrameworkElementFactory.SetValue( DP, VALUE) for DP == ContextMenuProperty && VALUE is ContextMenu, a Visual.");
            ...
            //
            GlobalLog.LogStatus("Test: FrameworkElementFactory.SetValue( DP, VALUE) for DP == ToolTipProperty && VALUE is ContentElement.");
            ...
            // Note: Extra-Special case since ItemsPanelTemplate is neither a Visual nor a ContentElement
            //
            GlobalLog.LogStatus("Test: FrameworkElementFactory.SetValue( DP, VALUE) for DP == ItemsPanelProperty && VALUE is ItemsPanelTemplate.");
            ...
            //
            GlobalLog.LogStatus("Test: FrameworkElementFactory.SetValue( DP, VALUE) for DP == TagProperty && VALUE is Object.");
            ...
            //    for Visual value
            //
            GlobalLog.LogStatus("Test: FrameworkElementFactory.SetValue( DP, VALUE) throws NotSupportedException for DP == TagProperty && VALUE is Menu, a Visual.");
            ...
            //    for Visual value
            //
            GlobalLog.LogStatus("Test: FrameworkElementFactory.SetValue( DP, VALUE) throws NotSupportedException for DP == TagProperty && VALUE is ContentElement.");
        ...
        ******************************************************************************/
        ...
        /// <returns></returns>
        public static TestResult TestSetter_SetValue( out Setter setterA1,
                                                       out Setter setterA2,
                                                       out Setter setterA3,
                                                       out Setter setterA4,
                                                       ...
                                                       out ContentElement contentElementA4)
        {
            ...
            GlobalLog.LogStatus("Test: Setter.Value must accept assignment of ContentElement instance (#1).");
            setterA1.Value = contentElementA1;

            if (setterA1.Value != contentElementA1)
            {
                GlobalLog.LogEvidence("FAIL!: Setter.Value did not get set to ContentElement instance (#1).");
                return TestResult.Fail;
            }

            // Verify bugfix now unconditionally allows setting Setter.Value to a Visual
            GlobalLog.LogStatus("Test: Setter.Value must accept assignment of UIElement instance.");
            setterA2.Value = contextMenuA2;

            if (setterA2.Value != contextMenuA2)
            {
                GlobalLog.LogEvidence("FAIL!: Setter.Value property did not get set to UIElement instance.");
                return TestResult.Fail;
            }

            // Verify bugfix still unconditionally allows setting Setter.Value to an ItemsPanelTemplate
            GlobalLog.LogStatus("Test: Setter.Value must accept assignment of ItemsPanelTemplate instance.");
            setterA3.Value = panelTemplateA3;

            if (setterA3.Value != panelTemplateA3)
            {
                GlobalLog.LogEvidence("FAIL!: Setter.Value property did not get set to ItemsPanelTemplate instance.");
                return TestResult.Fail;
            }

            // Verify bugfix now unconditionally allows setting Setter.Value to a ContentElement
            // This test is redundant with #1 but implemented for uniformity for caller.
            GlobalLog.LogStatus("Test: Setter.Value must accept assignment of ContentElement instance (#4).");
            setterA4.Value = contentElementA4;

            if (setterA4.Value != contentElementA4)
            {
                GlobalLog.LogEvidence("FAIL!: Setter.Value property did not get set to ContentElement instance (#4).");
                return TestResult.Fail;
            }
