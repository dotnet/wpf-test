...
namespace BamlLocalization
{
    ...
    internal class TranslationDictionariesReader
    {
        ...
        internal TranslationDictionariesReader(ResourceTextReader reader)
        {
            ...
            while (reader.ReadRow())
            {
                ...
                else
                {
                    ...
                    // field #3: Category
                    resource.Category = (LocalizationCategory) s_stringCatConverter.ConvertFrom(categoryString);
                    
                    // field #4: Readable
                    resource.Readable     = (bool) s_boolTypeConverter.ConvertFrom(reader.GetColumn(3));

                    // field #5: Modifiable
                    resource.Modifiable   = (bool) s_boolTypeConverter.ConvertFrom(reader.GetColumn(4));
        ...
        private static TypeConverter s_boolTypeConverter  = TypeDescriptor.GetConverter(true);
        private static TypeConverter s_stringCatConverter = TypeDescriptor.GetConverter(LocalizationCategory.Text);
    }
}
