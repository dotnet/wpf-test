...
namespace Microsoft.Test.Logging
{
    ...
    public static class LogContract
    {
        ...
        public static LoggingClient ConnectClient()
        {
            ...
            var logServer = AttachInternal(_clientStream);
        ...
        {
            var jsonrpc = AttachInternal(serverPipe, listener);
            await jsonrpc.Completion;
        }

        private static JsonRpc AttachInternal(Stream serverPipe, ILogger listener = null)
        {
            if (serverPipe == null)
            {
                throw new ArgumentException(nameof(serverPipe));
            }

            var rpc = CreateJsonRpc(serverPipe, serverPipe, listener);
            try
            {
                if (serverPipe != null)
                {
