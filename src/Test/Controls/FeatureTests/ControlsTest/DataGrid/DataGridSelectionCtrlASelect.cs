...
{
    /// <summary>
    /// Test press control+A to select cells in a DataGrid.
    /// </summary>
    [Test(0, "DataGrid", "DataGridSelectionCtrlASelect", SecurityLevel = TestCaseSecurityLevel.FullTrust)]  
    public class DataGridSelectionCtrlASelect : XamlTest
    {
        public DataGridSelectionCtrlASelect()
            : base(@"DataGridSelection.xaml")
        {
            InitializeSteps += new TestStep(Setup);
            CleanUpSteps += new TestStep(CleanUp);
            RunSteps += new TestStep(TestCtrlASelect);
        }
        ...
        TestResult TestCtrlASelect()
        {
            LogComment("TestCtrlASelect started");
            ...
            foreach (DataGridSelectionMode dataGridSelectionMode in Enum.GetValues(typeof(DataGridSelectionMode)))
            {
                foreach (DataGridSelectionUnit dataGridSelectionUnit in Enum.GetValues(typeof(DataGridSelectionUnit)))
                {
                    ...
                    using (DataGridSelectionValidator validator = new DataGridSelectionValidator(dataGrid))
                    {
                        ...
                        if (!validator.KeyboardCtrlASelect())
                        {
                            LogComment("CtrlASelect failed.");
