...
{
    /// <summary>
    /// Test press pageup and pagedown arrow keys to select cells in a DataGrid.
    /// NOTE: This test is disabled because it was unstable. Coverage for this scenario is being added in the ItemsControlVirtualization Tests
    /// </summary>
    [Test(0, "DataGrid", "DataGridSelectionPageUpAndPageDownArrowKeySelect", SecurityLevel = TestCaseSecurityLevel.FullTrust, Disabled = true)]  
    public class DataGridSelectionPageUpAndPageDownArrowKeySelect : XamlTest
    {
        public DataGridSelectionPageUpAndPageDownArrowKeySelect()
            : base(@"DataGridSelection.xaml")
        {
            InitializeSteps += new TestStep(Setup);
            CleanUpSteps += new TestStep(CleanUp);
            RunSteps += new TestStep(TestPageUpAndPageDownArrowKeySelect);
        }
        ...
        private bool PressPageUpAndPageDownArrowKeySelect(NavigationKey navigationKey)
        {
        ...
        TestResult TestPageUpAndPageDownArrowKeySelect()
        {
            LogComment("TestPageUpAndPageDownArrowKeySelect started");

            if (!PressPageUpAndPageDownArrowKeySelect(NavigationKey.PageUp))
            {
                LogComment("NavigationKey.PageUp failed.");
                return TestResult.Fail;
            }

            if (!PressPageUpAndPageDownArrowKeySelect(NavigationKey.PageDown))
            {
                LogComment("NavigationKey.PageDown failed.");
                return TestResult.Fail;
            }
