...
namespace RtfXamlView
{
   ...
   public struct sCommand
   {
      ...
      /// 
      /// </summary>
      public int argParam;//where in the args list to find string to go with param
   };
   ...
   public class RtfXamlViewApp : Application
   {
      ...
      public RtfXamlViewApp(string[] args/*string filename*/)
      {
         //_filename = filename;
         _args = args;
         _pathFileIndex = 0;
            ...
            else// try to treat it like XAML
            {
               ...
               else
               {
                  ...
                  try
                  {
                     try
                     {
                        //((XamlWindow)_xamlPanel.GetWindow()).LoadXamlFile(fileContent);
      ...
      private void OnFileMenuOpen(object sender, RoutedEventArgs e)
      {
         ...
         openDialog.Filter = "RTF files (*.rtf)|*.rtf|XAML files (*.xaml)|*.xaml|All files (*.*)|*.*";
      ...
      //the new tests are not entirely working yet
      ...
      //TOD0: Resolve exception when setting caret position
      ...
      /************************************************
      *  This function specificly takes the richedit
      *          content from the richedit control,
