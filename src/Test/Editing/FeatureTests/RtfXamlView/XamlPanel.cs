...
namespace RtfXamlView
{
    class XamlPanel : TextViewPanel
    {
        ...
        public override ConverterError SetPlainText(string textToSet, bool bSync)
        {
            ...
            else if (textToSet.Contains("<FlowDocument"))
            {
                _textBox.Text = textToSet;
            }
            else//the RichTextBox needs a FlowDocument, if execution gets to here, we add the FlowDocument Tag
            { 
        ...
        //Forces the control to do a copy and thus convert the XAML 
       ...
       // This doesn't sync, it simply does the same things that
       //  SetPlainText would do, sans sync and possibly logging.
