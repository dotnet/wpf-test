...
public class Trackball
{
    ...
    /// <summary>
    /// AttachAndFindViewports -
    /// Attaches this Trackball to the FrameworkElement provided *if* there are Viewport3D's
    /// in the tree.  If so, it will populate the Slaves collection.  If not, it will not
    /// attach to the FrameworkElement.
    /// </summary>
    /// <returns>
    /// bool - true if Viewport3D's were found, false if not.
    /// </returns>
    public bool AttachAndFindViewports(FrameworkElement element)
    {
        Slaves.Clear();
        Reset();
        ...
        // Perform a simple breadth-first search of the tree
        while (queue.Count > 0)
        {
            ...
            if (current != null)
            {
                if (current is Viewport3D)
                {
                    Viewport3D viewport = (Viewport3D)current;

                    // We can only handle ProjectionCameras for the time being
                    if (viewport.Camera is ProjectionCamera)
                    {
                        Slaves.Add(new WeakReference<Viewport3D>((Viewport3D)current));
                    }
                }
                else
                {
                    ...
                    {
                        DependencyObject visual = VisualTreeHelper.GetChild(current, i);

                        Debug.Assert(visual is Visual,
                            "The Viewport3D special case above should prevent us from getting here with a Visual3D.");
                        
                        queue.Enqueue((Visual) visual);
                    }
                }
            }
        }
    ...
    public List<WeakReference<Viewport3D> > Slaves
    {
        get
        {
            if (_slaves == null)
            {
                _slaves = new List<WeakReference<Viewport3D> >();
            }
    ...
    private void UpdateSlaves(Quaternion q, double s)
    {
        ...
        if (_slaves != null)
        {
            foreach (WeakReference<Viewport3D> i in _slaves)
            {
                Viewport3D viewport = i.Target;

                if (viewport != null)
                {
                    foreach(ModelVisual3D mv in viewport.Children)
                    {
    ...
    private List<WeakReference<Viewport3D> > _slaves;
