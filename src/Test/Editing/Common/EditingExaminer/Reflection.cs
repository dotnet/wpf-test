...
/*******************************************************************************
 *
 * Description: Reflection.cs implement the immediate window with the following
 * features:
 *      1. implement the reflection tools.
 *
 *******************************************************************************/
    ...
    public static class ReflectionUtils
    {
        ...
        public static Type FindType(string typeName)
        {
            ...
            new System.Security.Permissions.ReflectionPermission(
                System.Security.Permissions.PermissionState.Unrestricted)
                .Assert();
            Assembly[] domanassemblies = AppDomain.CurrentDomain.GetAssemblies();
            Assembly officeassebmly = Assembly.Load("OfficeLateBoundWrapper");
            Assembly[] assemblies; 
            if (officeassebmly != null)
            {
                assemblies = new Assembly[domanassemblies.Length + 1];
                for (int i = 0; i < domanassemblies.Length; i++)
                {
                    assemblies[i] = domanassemblies[i]; 
                }
                assemblies[domanassemblies.Length] = officeassebmly; 
            }
            else
            {
                assemblies = domanassemblies; 
            }
