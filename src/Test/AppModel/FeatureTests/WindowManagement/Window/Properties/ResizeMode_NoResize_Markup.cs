...
namespace WindowTest
{
    /// <summary>
    /// 
    /// Test for setting ResizeMode.NoResize property in markup
    ///
    /// </summary>
    public partial class ResizeMode_NoResize_Markup
    {                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                
        void OnContentRendered(object sender, EventArgs e)
        {
            // Expected Value
            ResizeMode expectedValue = ResizeMode.NoResize;
            Logger.Status("Expected ResizeMode = " + expectedValue.ToString());
            
            // Validate ResizeMode
            if (this.ResizeMode == expectedValue)
            {
                Logger.Status("[VALIDATION PASSED] Default ResizeMode validated");
            }
            else
            {
            ...
            // Set expectedValue to ResizeMode.CanResizeWithGrip
            expectedValue = ResizeMode.CanResizeWithGrip;
            Logger.Status("Expected ResizeMode = " + expectedValue.ToString());
