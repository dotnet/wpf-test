...
/* 
 * Description: Verify the internal method IAddChild.AddText within class Timeline

 */

//Instructions:
//  1. Create a Time Container
//  2. Add a string as child to the Time Container using 
//  3. Verify the property of the added child Timeline

//Pass Condition:
//  Test passes when added string child exists.

//Pass Verification:
//  The output of this test should match the expected output in TimelineAddTextExpect.txt.

//Warnings:
//  Any changes made in the output should be reflected TimelineAddTextExpect.txt file
...
{
     class TimelineAddText : ITimeBVT
     {
        Timeline _tContainer;
        Clock      _tClock;
        ...
        */
        ...
        public override void PostTick( int i )
        {
            if ( i == 5 )
            {
                //Add a string as child to the TimeContainer        
                string s = "StringChild";
                try
                {
                    ((IAddChild)_tContainer).AddText(s); 
                }
                catch (Exception e)
                {
