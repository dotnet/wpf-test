...
/* 
 * Description: Verify Seeking to a negavite time (TimeSeekOrigin.Duration)
 */

//Pass Verification:
//   The output of this test should match the expected output in ICSeekMinusExpect.txt

//Warnings:
//  Any changes made in the output should be reflected ICSeekMinusExpect.txt file
    ...
    class ICSeekMinus :ITimeBVT
    {
        ...
        public override void PostTick( int i )
        {
            if ( i == 2 )
            {
                try
                {
                    _tClock.Controller.Seek( TimeSpan.FromMilliseconds(-7), TimeSeekOrigin.Duration );
                }
                catch (Exception e)
                {
