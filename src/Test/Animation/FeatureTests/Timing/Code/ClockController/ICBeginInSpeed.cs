...
/* 
 * Description: Verify invoking Begin() via the CurrentGlobalSpeedInvalidated event
 */
...
//Warnings:
//  Any changes made in the output should be reflected in ICBeginInSpeedExpect.txt file
...
{
    class ICBeginInSpeed :ITimeBVT
    {
        private Clock         _clock;
        private int           _speedFired        = 0;
        ...
        public override void OnCurrentGlobalSpeedInvalidated(object subject, EventArgs args)
        {
            ...
            if (_speedFired == 0)
            {
                outString += "-----CurrentGlobalSpeedInvalidated-----Invoking Begin()" + "\n";
