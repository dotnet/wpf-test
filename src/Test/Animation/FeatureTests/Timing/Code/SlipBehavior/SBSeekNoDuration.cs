...
/* 
 * Description: Verify seeking when the child Timeline has a NaturalDuration but no Duration set - Slip
...
//Pass Verification:
//  The output of this test should match the expected output in SBSeekNoDurationExpect.txt.

//Warnings:
//  Any changes made in the output should be reflected SBSeekNoDurationExpect.txt file
...
{
    class SBSeekNoDuration :ITimeBVT
    {
        ...
        */
        public override string Test()
        {
            ...
            slipBegin           = 0d;
            slipDuration        = 0d;
            syncDurationTime    = TimeSpan.FromMilliseconds(10); //Simulating an implicit media Duration.
            ...
            // Run the Timer
            TimeGenericWrappers.EXECUTE( this, _clock, tManager, 0, 26, 1, ref outString);
        ...
        public override void PreTick( int i )
        {
            if ( i == 5 )
            {
                _clock.Controller.Seek( TimeSpan.FromMilliseconds(15), TimeSeekOrigin.BeginTime );
            }
        }
